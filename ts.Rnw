...
et définition d'un modèle, estimation des paramètres, calcul de
prévisions et simulation de séries.
...
\texttt{MASS}\index{package!MASS@\texttt{MASS}} de \cite{MASS}, déjà cité à
la section \ref{optimisation:MASS}. Rappelons que pour accéder aux
fonctions du package, il suffit de le charger en mémoire avec la
commande
...
nécessaires.  S-Plus propose également les classes \Splus
\classe{timeSeries} et \classe{signalSeries} (et les fonctions du même
nom pour créer les objets), mais celles-ci n'ajoutent pas de
...
\texttt{MASS}\index{package!MASS@\texttt{MASS}} (\code{arima}, entre
...
\texttt{MASS}\index{package!MASS@\texttt{MASS}} permet de juger rapidement
...
\texttt{MASS}\index{package!MASS@\texttt{MASS}} fournit une nouvelle
...
Le package \texttt{MASS}\index{package!MASS@\texttt{MASS}} contient également
