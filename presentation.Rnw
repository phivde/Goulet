...
\item Savoir démarrer une session R et exécuter des commandes simples.
\item Comprendre l’utilité des fichiers de script R et savoir les
  utiliser de manière interactive.
\item Savoir créer, modifier et sauvegarder ses propres fichiers de
  script R.
\end{objectifs}
...
\item Consulter l'annexe~\ref{emacs+ess} pour en savoir plus sur
...
\item Démarrer une session R en cliquant sur l'icône de l'application
  ...
  procédure expliquée à l'annexe~\ref{emacs+ess} si l'on utilise
  GNU~Emacs.
...
\item Quitter R en tapant \code{q()}\indexfonction{q} à la ligne de
  ...
  différente dans GNU~Emacs; voir l'annexe~\ref{emacs+ess}.
\end{enumerate}
...
\item Avec GNU~Emacs, la situation est un peu plus simple puisque l'on
  ...
  un processus R; voir l'annexe~\ref{emacs+ess}.
\end{itemize}
