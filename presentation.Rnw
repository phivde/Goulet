...
R est un environnement intégré de manipulation de données, de
calcul et de préparation de graphiques. Toutefois, ce n'est pas
...
\item programmes généralement courts, en général quelques lignes de
  code seulement;
...
figure~\ref{fig:presentation:console}. En soi, cela n'est pas si
...
\item L'interface graphique de R sous Mac OS~X est la plus élaborée.
  ...
  figure~\ref{fig:presentation:console}, l'application \code{R.app}
...
@
  ...
  calcul. Ces objets sont stockés dans la mémoire de l'ordinateur:
<<echo=TRUE>>=
x <- exp(2)
x
@
  Lorsque la session de travail est terminée, on sauvegarde une image
  de la mémoire sur le disque dur de l'ordinateur afin de pouvoir
...
@ %
  ...
  section~\ref{presentation:workspace}) et cette image est
...
\end{verbatim}
  ...
  valeur \Sexpr{format(x, dec = ",")} représente au juste?
...
\item L'approche dite de «code réel et objets virtuels» considère que
  ...
  R. La figure~\ref{fig:presentation:script} illustre schématiquement
...
\item GNU Emacs\index{Emacs} est un très ancien, mais aussi très
  ...
  riche qu'efficace. Entre autres fonctionnalités uniques à Emacs, le
  fichier de script et l'invite de commandes R sont regroupés dans la
  même fenêtre, comme on peut le voir à la
  figure~\ref{fig:presentation:emacs}.
  ...
  \begin{figure}[t]
    ...
    \caption{Fenêtre de GNU Emacs sous OS~X en mode d'édition de code
      ...
      la figure~\ref{fig:presentation:script} et dans la partie du
      bas, l'invite de commandes R.}
    \label{fig:presentation:emacs}
  \end{figure}

  Logiciel porte étendard du mouvement libre, Emacs est disponible
  gratuitement et à l'identique sur toutes les plateformes supportées
  par R, dont Windows, OS~X et Linux.
\item Le présent auteur distribue des versions de GNU Emacs pour
  Windows et OS~X simples à installer et livrées avec le mode ESS et
  quelques autres extensions. Consulter le site
  \url{http://vgoulet.act.ulaval.ca/emacs/}.
\item Il existe plusieurs autres options plus faciles à apprivoiser
  que Emacs pour éditer du code R. Nous recommandons:
  \begin{itemize}
  \item sous Windows, l'éditeur Notepad++\index{Notepad++}
    muni de l'extension NppToR \citep{NppToR}, tous deux des
    logiciels libres;
...
Dans ses grandes lignes, toute session de travail R se réduit aux
étapes ci-dessous.
\begin{enumerate}
...
\item Démarrer une session R en cliquant sur l'icône de l'application
  ...
  procédure expliquée à l'annexe~\ref{ess} si l'on utilise GNU~Emacs.
...
\item Si nécessaire --- et c'est rarement le cas --- sauvegarder
  l'image mémoire de la session R avec
...
\item Quitter R en tapant \code{q()}\indexfonction{q} à la ligne de
  ...
  différente dans GNU~Emacs; voir l'annexe~\ref{ess}.
\end{enumerate}
Évidemment, les étapes 1 et 2 sont interchageables, tout comme les
étapes 4, 5 et 6.


\section{Répertoire de travail}
\index{Répertoire de travail|(}
...
\item va sauvegarder l'image mémoire dans le fichier \code{.RData}.
\end{inparaenum}
...
Les rubriques d'aide des diverses fonctions disponibles dans R
contiennent une foule d'informations ainsi que des exemples
...
\begin{exercice}
  Démarrer un processus S-Plus ou R à l'intérieur de Emacs.
\end{exercice}
