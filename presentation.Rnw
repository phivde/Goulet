...
mais aussi un %
\capsule{https://youtu.be/FnoMjJ9Knl0}{langage de programmation} %
complet et autonome.
...
interprété. Pensez-y: nous utilisons le langage de programmation
lorsque nous entrons des commandes dans une cellule d'une feuille de
calcul, puis l'interpréteur exécute les commandes et affiche les
résultats dans la cellule.
...
\gotorbox{C'est le temps de vous lancer dans une première session de
  travail R. %
  \hypertarget{goto:presentation}\relax
  \hypertarget{goto:presentation:fin}\relax
  ...
  puis exécutez les lignes une par une en prenant bien soin de lire
  les commentaires et d'examiner les résultats.

  Poursuivez la lecture une fois la session de travail terminée. (En
  consultation électronique, les symboles {\gotorsign} insérés dans le
  code informatique permettent de revenir ici directement.)}
...
\begin{exercice}[nosol]
  ...
  {\scriptfilename} reproduit à la section précédente. Exécuter le
  code ligne par ligne et observer les résultats. Répéter l'exercice
  avec un ou deux autres éditeurs de texte afin de les comparer et de
  vous permettre d'en choisir un pour la suite.
\end{exercice}
