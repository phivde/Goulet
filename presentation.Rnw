...
sous macOS --- nous recommandons d'interagir avec R par le biais d'un
...
\item Un éditeur de texte est différent d'un traitement de texte en ce
  ...
  Bloc-notes sous Windows ou TextEdit sous macOS sont deux exemples
...
\item RStudio\index{RStudio} est un environnement de développement
  ...
  RStudio est disponible sur les plateformes Windows, macOS et Linux.
...
\item Les interfaces graphiques de R démarrent avec un répertoire de
  ...
  dans le menu \texttt{Fichier} (Windows) ou \texttt{Divers} (macOS).
