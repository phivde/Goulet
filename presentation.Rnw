...
surprenante puisqu'il précisément été développé dans ce but. Parmi ses
...
\tipbox{Toute analogie est boîteuse, mais celle-ci peut néanmoins
...
Un éditeur de texte pour programmeur saura en plus reconnaître la
...
que d'autres. Voici quelques bons choix.
...
\item RStudio\index{RStudio} est un environnement de développement
  ...
  est disponible à l'identique sur les plateformes Windows, macOS et
  Linux. L'\autoref{chap:rstudio} propose une introduction à RStudio.
...
par défaut. Pour le changer, utiliser l'entrée appropriée dans le menu
\texttt{Fichier} (Windows) ou \texttt{Divers} (macOS). Consulter aussi
les foires aux questions spécifiques aux interfaces graphiques
\citep{Rw-FAQ,RMacOSX-FAQ} pour des détails additionnels sur la
...
spécifier un répertoire de travail au lancement d'un processus R; voir
l'\autoref{chap:emacs+ess}.
...
Pour consulter la rubrique d'aide de la fonction \code{foo}, on entre
...
\end{Sinput}
\end{Schunk}
Pour chercher de l'aide sur un sujet \code{bar}, la commande est:
\begin{Schunk}
\begin{Sinput}
help.search("bar")
\end{Sinput}
...
suite de fonctions. Pour consulter la vignette sur \code{sujet},
entrer à la ligne de commande:
\begin{Schunk}
\begin{lstlisting}
vignette("sujet")
\end{lstlisting}
...
\lstinputlisting[firstline=13]{presentation.R}
