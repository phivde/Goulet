...
\item RStudio\index{RStudio} est un environnement de développement
  ...
  \begin{figure}[t]
    \centering
    \includegraphics{rstudio-screenshot}
    \caption{Fenêtre de RStudio sous OS~X dans sa configuration par
      défaut. Dans la partie de gauche, on retrouve en haut le fichier
      de script de la \autoref{fig:presentation:script} et, en bas,
      l'invite de commandes R. Dans la partie de droite, la fenêtre du
      haut dresse la liste des objets de l'environnement (vide, ici)
      et la fenêtre du bas offre une interface pour charger des
      paquetages (la liste peut différer d'un poste de travail à un
      autre).}
