...
\begin{figure}
\begin{lstlisting}[frame=single,language=lisp,numbers=none]
(define factorial (lambda (n)
  (if (= n 1)
      1
    (* n (factorial (- n 1))))))
\end{lstlisting}
\begin{lstlisting}[frame=single,language=R,numbers=none]
...
\tipbox{Toute analogie est boiteuse, mais celle-ci peut néanmoins
  ...
  \item ne sauvegarder que les formules d'une feuille de calcul,
    ...
    correspondantes.
  \end{enumerate}
  La seconde, assurément{\dots} Or, sauvegarder l'espace de travail R
...
%% exceptionnellement, le nom du fichier de script est défini plus
%% haut, à la fin de la section «Anatomie d'une session de travail»
