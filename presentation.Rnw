...
qu'il importe de conserver au terme d'une session de travail, ce ne
sont pas tant les objets que le code qui a servi à les créer. Ainsi,
on sauvegardera dans ce que l'on nommera des \emph{fichiers de script}
nos expressions R et le code de nos fonctions. Par convention, on
donne aux fichiers de script un nom se terminant avec l'extension
\code{.R}.
...
\tipbox{Toute analogie est boiteuse, mais celle-ci peut néanmoins
  ...
  virtuels». Avec un tableur comme Excel, que choisiriez-vous si l'on
  vous proposait les options suivantes:
  \begin{enumerate}
  ...
  \item Ne sauvegarder que les formules d'une feuille de calcul,
    ...
    correspondantes.
  \end{enumerate}
  La seconde, assurément{\dots} Or, c'est justement à celle-ci que
  correspond l'approche «code réel, objets virtuels». La raison pour
  laquelle on n'a jamais à se poser la question avec un tableur, c'est
  que le progiciel évalue constamment et en temps réel les formules
  pour afficher les résultats dans les cellules.}

\importantbox{Dans les fichiers de script comme à la ligne de
...
de fichiers textes bruts, c'est-à-dire dépourvus d'information de
...
Il est possible de changer le répertoire de travail depuis la ligne de
commande avec \Icode{setwd}:
\begin{Schunk}
\begin{Sinput}
...
Les interfaces graphiques de R et les environnements de développement
intégrés démarrent généralement avec un répertoire de travail par
défaut. On change le répertoire de travail à partir du menu \texttt{Fichier} (Windows) ou
\texttt{Divers} (macOS) des interfaces graphiques. Dans
\index{RStudio}RStudio, c'est via le menu \code{Session|Set Working Directory}.

Les foires aux questions spécifiques aux interfaces graphiques
\citep{Rw-FAQ,RMacOSX-FAQ} contiennent des détails additionnels sur la
gestion du répertoire de travail.

Avec \index{Emacs}Emacs, la situation est un peu plus simple puisqu'on
spécifie le répertoire de travail dès le lancement d'un processus R; ceci
...
\item Si nécessaire, changer le répertoire de travail de R pour le
  répertoire contenant le fichier de script.
...
\gotorbox{C'est le temps de vous lancer dans une première session de
  ...
  votre éditeur de texte, puis exécutez les lignes une par une en
...
\begin{exercice}[nosol]
  ...
  --- puis exécuter quelques expressions tirées des sections
  d'exemples.
\end{exercice}
