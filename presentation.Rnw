...
\item Dans la catégorie des éditeurs de texte, je recommande le
  ...
  \warningbox{La puissance de Emacs a toutefois un coût: c'est un
    logiciel difficile à apprivoiser, surtout pour les personnes moins
    à l'aise avec l'informatique. C'est pourquoi RStudio demeure un
    meilleur choix pour débuter à programmer avec R.}
