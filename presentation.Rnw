...
@
  ...
  \autoref{sec:presentation:workspace}) et cette image est
...
\item Dans la catégorie des éditeurs de texte, nous recommandons le
  ...
  Consulter l'\autoref{chap:emacs+ess} pour en savoir plus sur GNU~Emacs et
...
\item Avec GNU~Emacs, la situation est un peu plus simple puisque l'on
  ...
  un processus R; voir l'\autoref{chap:emacs+ess}.
\end{itemize}
