...
Comme la majorité des langages de programmation, le S contient
comporte les concepts de variable locale et de variable globale.

\begin{itemize}
\item Toute variable définie dans une fonction est
  locale\index{variable!locale} à cette fonction, c'est-à-dire qu'elle:
  \begin{itemize}
...
Nous n'abordons ici que les techniques les plus simples.
\begin{itemize}
\item Les erreurs de syntaxe sont les plus fréquentes (en particulier
  ...
  vérification de la syntaxe est effectuée par l'interpréteur S.
...
\item Avec Emacs\index{Emacs} et le mode ESS\index{ESS}, la principale
  ...
  \textsf{R} empêchent d'avoir recours à une telle procédure puisque
  la fenêtre d'édition de fonctions bloque l'accès à l'interface de
  commande.
\end{itemize}
...
\begin{exercice}
  ...
  d'étudiants.  Vous décidez donc de modifier la fonction afin d'en
  réduire le nombre d'opérations et faire en sorte qu'elle n'utilise
  aucune boucle.
\begin{verbatim}
...
\end{Schunk}
  ...
  \item Appliquer les corrections nécessaires à la fonction pour que
    ...
    \emph{Astuce}: tirer profit du moteur d'indentation de Emacs.
  \end{enumerate}
  \begin{rep}
\begin{verbatim}
param <- function (moyenne, variance, loi)
