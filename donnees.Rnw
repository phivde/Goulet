...
\code{read.csv} si c'est une virgule. Les nombreux arguments de ces
fonctions permettent de spécifier si les titres des lignes ou des
colonnes apparaissent ou non dans le fichier, le mode de chaque
colonne, le séparateur décimal, l'encodage, etc. La rubrique
d'aide fournit les détails.
<<echo=TRUE>>=
...
\begin{exercice}
  Effectuer une mise en œuvre de l'algorithme \emph{Bucketgsort} de
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    importer directement depuis Internet en spécifiant une url comme
    nom de fichier.
<<eval=FALSE>>=
matrix(scan("http://lib.stat.cmu.edu/datasets/boston",
            skip = 22),
       ncol = 14, byrow = TRUE)
@
  \end{sol}
