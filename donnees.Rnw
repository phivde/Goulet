...
\capsule{https://youtu.be/indicage}{matrices et tableaux}.
...
La généralisation d'une matrice à plus de deux dimensions est un
\Index{tableau}tableau (\emph{array}). Le nombre de dimensions du
tableau est toujours égal à la longueur de l'attribut \icode{dim}. La
fonction de base pour créer des tableaux est \Icode{array}:
<<echo=TRUE>>=
...
\importantbox{%
  ...
  vous habituer à la convention de R qu'essayer de constamment la
  contourner.}
...
avec les crochets \verb=[ ]=\icode{[}, comme pour les vecteurs
...
matrices tableaux, sinon qu'il faut préciser maintenant préciser des
...
La fonction \Icode{cbind} (\emph{column bind}), quant à elle,
fusionne horizontalement (colle) deux matrices (ou plus) ayant le même
nombre de lignes.
<<echo=TRUE>>=
n <- matrix(1:4, nrow = 2)
cbind(m, n)
