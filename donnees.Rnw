...
Plus formellement, un \index{tableau de données}tableau de données est
...
\section{Dates}
\label{sec:donnees:dates}

Vous devez travailler avec des dates. Leur traitement est complexe
dans n'importe quel langage ou outil d'analyse. Pensez seulement: il
faut tenir compte du nombre variable de jours par mois, des années
bissextiles, des secondes intercalaires, des fuseaux horaires, du
type de calendrier. R contient dans le système de base tout ce dont
vous aurez besoin pour manipuler les dates: stockage dans divers
formats, tri, comparaison, génération de suites, etc. Comme point de
départ, consultez la rubrique d'aide de DateTimeClasses. Plusieurs
paquetages externes offrent des outils pour faciliter le
traitement des dates, mais aucun n'est requis pour compléter le
travail.


\gotorbox{Étudiez les lignes \reflines{application:dates}
  du fichier de script \code{\scriptfilename}
  reproduit à la \autoref{sec:donnees:exemples}.}
