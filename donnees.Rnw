...
\capsule{https://youtu.be/jDUQlClI_XA}{matrices et tableaux}. Dans la
...
\tipbox{%
  Nommez les éléments des listes puisqu'il peut s'avérer difficile
  autrement de les identifier. De plus, comme nous le verrons
  ci-dessous, il est très simple d'extraire les éléments d'une liste
  par leur étiquette.}
...
inutile de définir des catégories s'il y a en une différente pour
...
\tipbox{On remarquera, ci-dessus, que les données de la colonne
...
De par sa nature
ouverte, R offre de multiples autres façons d'interagir avec des
données externes, notamment l'importation directe de fichiers de
tableurs comme Excel. Il ne faut toutefois pas négliger que, parfois,
un système de base de données demeure le meilleur outil pour préparer
les données avant une analyse statistique. R peut ensuite se connecter
à la base de données pour de manipulation des données demeure une  ou de la connection directe à des bases de
données. Quand ce ne sont pas les
...
un vecteur ou une matrice de données toutes du même mode. Pour de
...
Les fonctions \Icode{read.table} et \Icode{read.csv} constituent les
principaux outils d'importation de tableaux de données dans R. On
choisira l'une ou l'autre selon le format du fichier de données:
\code{read.table} si le séparateur des champs est une espace et
\code{read.csv} si c'est plutôt une virgule. Les nombreux arguments de
ces fonctions permettent de spécifier si les titres des lignes ou des
colonnes apparaissent ou non dans le fichier, le mode de chaque
colonne, le séparateur décimal, l'encodage, etc. Consulter la rubrique
...
\begin{table}
  \centering
  \caption{Principales caractéristiques des fonctions de la famille
    \code{read.table}}
