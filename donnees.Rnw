...
\notebox{Que signifient ces noms \code{foo}, \code{bar}, \code{foobar}
  et autres variantes que l'on rencontre souvent dans la documentation
  informatique? Rien de particulier, justement! Ce sont des
  \link{https://fr.wikipedia.org/wiki/Variable_m\%C3\%A9tasyntaxique}{variables
    métasyntaxiques} sans signification propre. Chez les francophones,
  l'équivalent le plus répandu est sans doute \code{toto}.}
...
Les objets de mode \code{numeric}, \code{complex}, \code{logical} et
\code{character} sont des objets \index{vecteur!simple}\emph{simples}
(\emph{atomic}) qui contiennent des données d'un seul type. En
revanche, les objets de mode \code{list} ou \code{expression} sont des
objets \index{vecteur!récursif}\emph{récursifs} qui peuvent contenir
d'autres objets. Par exemple, une liste peut contenir une ou plusieurs
autres listes; nous y reviendrons plus loin.
...
Vous devez connaitre les caractéristiques spéciales de ces objets
...
\code{NA} n'est égale à aucune autre, \emph{pas même à elle-même}! Pour
tester si une valeur est manquante, vous devez nécessairement avoir
recours à la fonction \Icode{is.na}.
<<echo=TRUE>>=
x <- NA
x == NA
is.na(NA)
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    \icode{dim}      &
    \index{dimension}dimensions des matrices et tableaux \\
    \icode{dimnames} &
    \index{etiquette@étiquette}étiquettes des dimensions des matrices
    et tableaux \\
    \icode{names}    &
    \index{etiquette@étiquette}étiquettes des éléments d'un objet \\
    \bottomrule
  \end{tabular}
\end{table}
...
\importantbox{%
  Vous aurez remarqué que R remplit les matrices et tableaux en
  ...
  elle a ses avantages dans plusieurs applications. La fonction
  \icode{matrix} possède un argument \code{byrow} qui permet
  d'inverser l'ordre de remplissage, mais il vaut mieux vous habituer
  à la convention de R que d'essayer constamment de la contourner.}

L'ordre inhabituel de remplissage des matrices et des tableaux vous
causera des maux de tête si vous ne les visualisez pas correctement.
Les tableaux à trois dimensions sont des prismes rectangulaires formés
de matrices (remplies par colonne!) placées les unes \emph{derrière}
les autres. L'ajout d'une quatrième dimension revient à aligner
...
des matrices et des tableaux à trois et à quatre dimensions.
...
\begin{figure}[t]
  ...
  \begin{minipage}{0.4\linewidth}
    \begin{picture}(5.5,4.5)(1,0)
      \put(1,1){\usebox{\matrice}}
      \put(0.5,3.5){\circle{0.7}}\put(0.5,3.5){\makebox(0,0){\footnotesize 1}}
      \put(0.5,3.0){\vector(0,-1){1.75}}
      \put(1.5,0.5){\circle{0.7}}\put(1.5,0.5){\makebox(0,0){\footnotesize 2}}
      \put(2.0,0.5){\vector(1,0){3.75}}
    \end{picture} \newline
    \begin{picture}(7,6.5)(1,0)
      \put(1,1){\usebox{\tableau}}
      \put(0.5,3.5){\circle{0.7}}\put(0.5,3.5){\makebox(0,0){\footnotesize 1}}
      \put(0.5,3.0){\vector(0,-1){1.75}}
      \put(1.5,0.5){\circle{0.7}}\put(1.5,0.5){\makebox(0,0){\footnotesize 2}}
      \put(2.0,0.5){\vector(1,0){3.75}}
      \put(6.4,0.5){\circle{0.7}}\put(6.4,0.5){\makebox(0,0){\footnotesize 3}}
      \put(6.8,0.8){\vector(5,3){1.2}}
    \end{picture}
  \end{minipage}
  \hfill
  \begin{minipage}{0.45\linewidth}
    \begin{picture}(10.4,7.1)(1,0)
      \put(4.2,2.92){\usebox{\tableau}}
      \put(1,1){\usebox{\tableau}}
      \put(0.5,3.5){\circle{0.7}}\put(0.5,3.5){\makebox(0,0){\footnotesize 1}}
      \put(0.5,3.0){\vector(0,-1){1.75}}
      \put(1.5,0.5){\circle{0.7}}\put(1.5,0.5){\makebox(0,0){\footnotesize 2}}
      ...
      \put(6.4,0.5){\circle{0.7}}\put(6.4,0.5){\makebox(0,0){\footnotesize 3}}
      \put(6.8,0.8){\vector(5,3){1.2}}
      \put(7.6,0.5){\circle{0.7}}\put(7.6,0.5){\makebox(0,0){\footnotesize 4}}
      \put(8.0,0.8){\vector(5,3){3.2}}
    \end{picture}
  \end{minipage}
  \caption[Représentations de tableaux à trois et à quatre
  dimensions]{%
    Représentations d'une matrice (en haut à gauche), d'un tableau à
    trois dimensions (en bas à gauche) et d'un tableau à quatre
    dimensions (à droite). Les chiffres encerclés identifient l'ordre
    de remplissage des dimensions.}
  \label{fig:donnees:tableaux}
\end{figure}

L'indiçage dans les matrices\Index{indiçage!matrice} et dans les
tableaux\Index{indiçage!tableau} se fait avec les crochets
\code{[~]}\indexcode{[}, comme pour les vecteurs simples. D'ailleurs,
l'ensemble des règles d'indiçage de la
\autoref{sec:bases:vecteurs:indicage} s'appliquent à l'identique aux
matrices et tableaux, sinon qu'il faut maintenant préciser des indices
pour chaque dimension, séparés par des virgules. Par exemple, on
extrait un élément d'une matrice en précisant sa position dans chaque
...
Dans un tableau à trois dimensions, l'indiçage sur une seule dimension
sélectionne des \Index{tranche}«tranches» du tableau, alors que
l'indiçage sur deux dimensions simultanément sélectionne des
\Index{carotte}«carottes»\footnote{%
  Un terme que j'emprunte à la géologie, où une carotte est un
  échantillon de roche cylindrique extrait d'un sol par forage.} %
ou des \index{tube|see{carotte}}tubes. Ces idées se généralisent
aisément à des tableaux à plus de trois dimensions. Les
figures~\ref{fig:donnees:indicage:matrice} et
\ref{fig:donnees:indicage:tableau} proposent respectivement des
représentations schématiques des opérations d'indiçage d'une matrice
et d'un tableau à trois dimensions.
...
\begin{figure}
  ...
  \begin{minipage}{0.22\linewidth}
    \centering
    \code{X} \\
    \begin{picture}(5.5,3.5)
      %% matrice 3 x 5
      \put(0,0){\usebox{\matrice}}
    \end{picture}
  \end{minipage}
  \hfill
  \begin{minipage}{0.22\linewidth}
    ...
    \begin{picture}(5.5,3.5)
      %% indiçage ligne 1
      \multiput(0,0)(0,1){2}{\usebox{\ligne}}
      \put(0,2){\usebox{\cligne}}
    \end{picture}
  \end{minipage}
  \hfill
  \begin{minipage}{0.22\linewidth}
    ...
    \begin{picture}(5.5,3.5)
      %% indiçage colonne 3
      \multiput(0,0)(1,0){2}{\usebox{\colonne}}
      \put(2,0){\usebox{\ccolonne}}
      \multiput(3,0)(1,0){2}{\usebox{\colonne}}
    \end{picture}
  \end{minipage}
  \hfill
  \begin{minipage}{0.22\linewidth}
    ...
    \code{X[1, 3]} \\
    \begin{picture}(5.5,3.5)
      %% indiçage élément (1, 3)
      \multiput(0,0)(0,1){2}{\usebox{\ligne}}
      \multiput(0,2)(1,0){2}{\usebox{\cube}}
      \put(2,2){\usebox{\ccube}}
      \multiput(3,2)(1,0){2}{\usebox{\cube}}
    \end{picture}
  \end{minipage}
  \caption[Représentations d'opérations d'indiçage sur une matrice]{%
    Représentations d'opérations d'indiçage sur une matrice. De gauche
    à droite: matrice complète; sélection d'une ligne; sélection d'une
    colonne; sélection d'un élément unique.}
  \label{fig:donnees:indicage:matrice}
\end{figure}
...
\begin{figure}
  ...
  \begin{minipage}{0.22\linewidth}
    ...
    \code{X[1, , ]} \\
    \begin{picture}(7,4.5)
      %% indiçage 1ère tranche horizontale
      \multiput(0.0,0.0)(0,1){2}{\usebox{\trancheh}}
      \put(0.0,2.0){\usebox{\ctrancheh}}
    \end{picture}
  \end{minipage}
  \hfill
  \begin{minipage}{0.22\linewidth}
    ...
    \code{X[1, , 2]} \\
    \begin{picture}(7,4.5)
      %% indiçage carotte transversale (1, 2)
      \multiput(0.0,0.0)(0,1){2}{\usebox{\trancheh}}
      \multiput(1.5,2.9)(-0.5,-0.3){2}{\usebox{\ligne}}
      \put(0.5,2.3){\usebox{\cligne}}
      \put(0.0,2.0){\usebox{\ligne}}
    \end{picture}
  \end{minipage}
  \hfill
  \begin{minipage}{0.22\linewidth}
    ...
    \code{X[1, 5, 2]} \\
    \begin{picture}(7,4.5)
      %% indiçage élément (1, 5, 2)
      \multiput(0.0,0.0)(0,1){2}{\usebox{\trancheh}}
      \multiput(1.5,2.9)(-0.5,-0.3){2}{\usebox{\ligne}}
      \multiput(0.5,2.3)(1,0){4}{\usebox{\cube}}
      \put(4.5,2.3){\usebox{\ccube}}
      \put(0.0,2.0){\usebox{\ligne}}
    \end{picture}
  \end{minipage}
  \caption[Représentations d'opérations d'indiçage sur un tableau à
  trois dimensions]{%
    Représentations d'opérations d'indiçage sur un tableau à trois
    dimensions. De gauche à droite: tableau complet; sélection d'une
    tranche horizontale; sélection d'une carotte transversale;
    sélection d'un élément unique.}
  \label{fig:donnees:indicage:tableau}
\end{figure}
...
\videobox{\link{https://youtu.be/6P2cFe3DBfw}{Matrices et tableaux}}{%
  Quand il s'agit de bien comprendre les opérations de création et
  d'indiçage des matrices et des tableaux, une image vaut mille mots.
  Visionnez donc la vidéo sur les
  \link{https://youtu.be/6P2cFe3DBfw}{matrices et tableaux} en
  complément de la présentation ci-dessus.}
...
## Certaines des opérations qui en résultent ne sont pas
...
raisonnement, vous avez compris le concept d'application, soit:
utiliser à répétition une même fonction sur les sous-ensembles d'une
structure de données.

La fonction d'application pour les matrices et les tableaux est
\icode{apply}. Utilisée sur une matrice, elle permet d'effectuer des
calculs sur les lignes ou les colonnes de celle-ci.
<<echo=TRUE>>=
apply(x, 1, sum)
apply(x, 2, sum)
@
...
La \autoref{fig:donnees:application:matrice} représente
schématiquement des opérations d'applications sur les lignes et les
...
\begin{figure}
  ...
  \begin{tabularx}{\linewidth}{cXcXc}
    \code{X} & & \code{apply(X, 1, FUN)} \\
    \begin{picture}(5,3.5)
      %% matrice
      \put(0.0,0.0){\usebox{\matrice}}
    \end{picture}
    & \quad\raisebox{1.25\unitlength}{\faArrowRight} &
    \begin{picture}(8.5,4.0)(-3.5,0.0)
      \put(-3.5,1.5){\makebox(3.5,0)[l]{\code{FUN} $\mapsto$}}
      %% tranches horizontales
      \multiput(0.0,-0.5)(0,1.5){3}{\usebox{\cligne}}
    \end{picture}
    & \quad\raisebox{1.25\unitlength}{\faArrowRight} &
    \begin{picture}(5.5,3.5)
      %% vecteur de 3 éléments
      \multiput(0,1)(1,0){3}{\usebox{\cube}}
    \end{picture}
    \\[6mm]
    \code{X} & & \code{apply(X, 2, FUN)} \\
    \begin{picture}(5,3.5)
      %% matrice
      \put(0.0,0.0){\usebox{\matrice}}
    \end{picture}
    & \quad\raisebox{1.25\unitlength}{\faArrowRight} &
    \begin{picture}(10.5,3.5)(-3.5,0)
      \put(-3.5,1.5){\makebox(3.5,0)[l]{\code{FUN} $\mapsto$}}
      %% tranches verticales
      \multiput(-0.5,0.0)(1.5,0){5}{\usebox{\ccolonne}}
    \end{picture}
    & \quad\raisebox{1.25\unitlength}{\faArrowRight} &
    \begin{picture}(5.5,3.5)
      %% vecteur de 5 éléments
      \put(0,1){\usebox{\ligne}}
    \end{picture}
  \end{tabularx}
  \caption{Représentations de la fonction d'application \code{apply}
    avec une matrice}
  \label{fig:donnees:application:matrice}
\end{figure}
...
ou un tableau, selon le contenu de l'argument \code{MARGIN}. Tel que
mentionné à la section précédente lorsque nous discutions d'indiçage
des tableaux, si \code{X} est un tableau à trois dimensions et que la
longueur de \code{MARGIN} est $1$, on applique \code{FUN} sur les
\index{tranche}tranches de \code{X} (des matrices). Si la longueur de
\code{MARGIN} est $2$, on applique plutôt \code{FUN} aux
\index{carotte}carottes tirées de \code{X} (des vecteurs). La
\autoref{fig:donnees:application:tableau} illustre ces principes
schématiquement.
...
\begin{figure}
  ...
  \begin{tabularx}{\linewidth}{cXcXc}
    ...
    \begin{picture}(7,4.5)
      ...
      \put(0.0,0.0){\usebox{\tableau}}
    \end{picture}
    & \raisebox{1.5\unitlength}{\faArrowRight} &
    \begin{picture}(10.5,5.0)(-3.5,0)
      \put(-3.5,1.5){\makebox(4.5,0)[l]{\code{FUN} $\mapsto$}}
      %% tranches horizontales
      \multiput(0.0,-0.5)(0,1.5){3}{\usebox{\ctrancheh}}
    \end{picture}
    & \raisebox{1.5\unitlength}{\faArrowRight} &
    \begin{picture}(5.5,4.5)
      %% vecteur de 3 éléments
      \multiput(0,1.5)(1,0){3}{\usebox{\cube}}
    \end{picture}
    \\[6mm]
    \code{X} & & \code{apply(X, c(1, 2), FUN)} \\
    \begin{picture}(7,4.5)
      %% tableau complet
      \put(0.0,0.0){\usebox{\tableau}}
    \end{picture}
    & \raisebox{1.5\unitlength}{\faArrowRight} &
    \begin{picture}(12.5,5.0)(-3.5,0)
      \put(-3.5,1.5){\makebox(4.5,0)[l]{\code{FUN} $\mapsto$}}
      %% carottes horizontales
      \multiput(0.0,-0.5)(0,1.5){3}{\usebox{\ccarotte}}
      \multiput(1.5,-0.5)(0,1.5){3}{\usebox{\ccarotte}}
      \multiput(3.0,-0.5)(0,1.5){3}{\usebox{\ccarotte}}
      \multiput(4.5,-0.5)(0,1.5){3}{\usebox{\ccarotte}}
      \multiput(6.0,-0.5)(0,1.5){3}{\usebox{\ccarotte}}
    \end{picture}
    \rule[-0.5\unitlength]{0pt}{5\unitlength} % éloigner légende
    & \raisebox{1.5\unitlength}{\faArrowRight} &
    \begin{picture}(5.5,4.5)
      %% matrice 3 x 5
      \put(0,0){\usebox{\matrice}}
  ...
  \end{tabularx}
  \caption{Représentations de la fonction d'application \code{apply}
    avec un tableau à trois dimensions}
  \label{fig:donnees:application:tableau}
\end{figure}
...
\begin{enumerate}
\item Les sommes des trois \index{tranche}tranches horizontales (de
  l'avant vers l'arrière) dans le tableau;
\item Les sommes des quatre \index{tranche}tranches verticales (du
  haut vers le bas);
\item les sommes des deux \index{tranche}tranches transversales (de la
  gauche vers la droite);
\item Les sommes des douze \index{carotte}carottes horizontales;
\item Les sommes des huit \index{carotte}carottes verticales;
\item Les sommes des six \index{carotte}carottes transversales.
\end{enumerate}
<<echo=TRUE>>=
apply(x, 1, sum)
apply(x, 2, sum)
...
polyvalente du langage R. Si les éléments d'un
\index{vecteur!simple}vecteur simple doivent tous être du même mode,
ceux de la liste peuvent être de n'importe quel mode, y compris le
mode \icode{list}. Par conséquent, il est possible d'emboîter des
listes, d'où le qualificatif de
\index{vecteur!récursif}\emph{récursif} pour ce type d'objet.
...
Dans l'exemple ci-dessus, la liste compte trois éléments. Le premier,
étiqueté \code{score}, est un vecteur de mode \code{numeric}; le
second, étiqueté \code{user}, est un vecteur de mode \code{character};
le troisième, étiqueté \code{new} est un vecteur de mode
\code{logical}. La \autoref{fig:donnees:liste} propose une
représentation schématique de cette liste.
...
\begin{figure}
  ...
  \input{include/cube}
  \centering
  \begin{picture}(39.25,6)
    \ttfamily
    %% liste
    \multiput(0,0)(12.5,0){3}{\usebox{\gpolygone}}

    %% premier élément
    \multiput(3,0.5)(2,0){3}{\usebox{\cubedouble}}
    \put( 4,1.5){\makebox(0,0){1}}
    \put( 6,1.5){\makebox(0,0){5}}
    \put( 8,1.5){\makebox(0,0){2}}
    \put(3.5,4.0){\usebox{\nametag}}
    \put(7.5,5.0){\makebox(0,0){score}}

    %% deuxième élément
    \put(16.25,0.5){\usebox{\polygone}}
    \put(18.75,1.5){\makebox(0,0){Joe}}
    \put(16.0,4.0){\usebox{\nametag}}
    \put(20.0,5.0){\makebox(0,0){user}}

    %% troisième élément
    \put(28.75,0.5){\usebox{\polygone}}
    \put(31.25,1.5){\makebox(0,0){TRUE}}
    \put(28.5,4.0){\usebox{\nametag}}
    \put(32.5,5.0){\makebox(0,0){new}}
  \end{picture}
  \caption[Représentation d'une liste]{%
    Représentation d'une liste composée d'un vecteur numérique, d'un
    vecteur caractère et d'un vecteur booléen, chaque élément de la
    liste étant étiqueté}
  \label{fig:donnees:liste}
\end{figure}

\tipbox{Étiquetez les éléments des listes puisqu'il peut s'avérer
  ...
  liste par leur \index{etiquette@étiquette}étiquette.}
...
«\code{[~]}», les crochets doubles «\code{[[~]]}» et le signe de
dollar «\code{\$}» (ces deux derniers opérateurs figurent au
...
\item Les crochets simples\indexcode{[} fonctionnent avec la liste
  ...
  lorsque l'on indice une liste avec un vecteur. Toutefois, cette
  règle fait en sorte que le résultat de l'extraction d'un seul
  élément d'une liste avec les crochets simples est non pas l'objet en
  question, mais bien une liste d'un élément contenant l'objet. C'est
  rarement ce que l'on souhaite obtenir.
<<echo=TRUE>>=
x[1]
x[-2]
x[c(1, 3)]
...
Consultez la \autoref{fig:donnees:liste:indicage} pour une
illustration de l'indiçage d'une liste.
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.4\linewidth}
    \centering
    \code{x[1]} \\
    \begin{picture}(14.25,7.0)
      \ttfamily
      \put(0,0){\usebox{\gpolygone}}
      \multiput(3,0.5)(2,0){3}{\usebox{\cubedouble}}
      \put( 4,1.5){\makebox(0,0){1}}
      \put( 6,1.5){\makebox(0,0){5}}
      \put( 8,1.5){\makebox(0,0){2}}
  ...
  \begin{minipage}[t]{0.4\linewidth}
    \centering
    \code{x[[1]]} ou \code{x\$score} \\
    \begin{picture}(7,3.5)
      \ttfamily
      \multiput(0,0)(2,0){3}{\usebox{\cubedouble}}
      \put(1,1){\makebox(0,0){1}}
      \put(3,1){\makebox(0,0){5}}
      \put(5,1){\makebox(0,0){2}}
    \end{picture}
  \end{minipage}
  \caption{Représentations d'opérations d'indiçage sur une liste}
  \label{fig:donnees:liste:indicage}
\end{figure}
...
La \autoref{fig:donnees:application:lapply+sapply} illustre l'effet
respectif des fonctions d'application \code{lapply} et \code{sapply}
sur une liste.
...
\begin{figure}
  ...
  \begin{tabular}{c}
    \code{X} \\
    \begin{picture}(16,2.9)
      %% liste
      \multiput(0,0)(5,0){3}{\usebox{\polygone}}
      %% premier élément
      \multiput(0.75,0.5)(1,0){3}{\usebox{\cube}}
      %% deuxième élément
      \put(6.75,0.5){\usebox{\cube}}
      %% troisième élément
      \multiput(11.25,0.5)(1,0){2}{\usebox{\cube}}
    \end{picture}
  \end{tabular} \\[4mm]
  \begin{tabularx}{0.92\linewidth}{cXc}
    \code{lapply(X, FUN)} \\
    \begin{picture}(12.5,2.3)(-4.0,0)
      \put(-4.0,1.0){\makebox(4.0,0)[l]{\code{FUN} $\mapsto$}}
      \multiput(0,0.5)(1,0){3}{\usebox{\ccube}}
      \put(4,0.5){\usebox{\ccube}}
      \multiput(6,0.5)(1,0){2}{\usebox{\ccube}}
    \end{picture}
    & \raisebox{0.5\unitlength}{\faArrowRight} &
    \begin{picture}(16,2.3)
      %% liste
      \multiput(0,0)(5,0){3}{\usebox{\polygone}}
      %% résultats
      \multiput(1.75,0.5)(5,0){3}{\usebox{\cube}}
    \end{picture}
    \\[4mm]
    \code{sapply(X, FUN)} \\
    \begin{picture}(12.5,2.3)(-4.0,0)
    ...
    \end{picture}
    & \raisebox{0.5\unitlength}{\faArrowRight} &
    \begin{picture}(3.5,1.8)
      %% résultats
      \multiput(0,0.5)(1,0){3}{\usebox{\cube}}
    \end{picture}
  \end{tabularx}
  \caption{Représentations des fonctions d'application \code{lapply}
    et \code{sapply}}
  \label{fig:donnees:application:lapply+sapply}
\end{figure}

\tipbox{La fonction \icode{sapply} est souvent utile pour rendre
  vectorielle une fonction \code{f} qui ne le serait pas autrement. La
  stratégie consiste généralement à vectoriser les calculs en
  intégrant à la fonction \code{f} un appel à \code{sapply}.}

\importantbox{Outre %
  l'\index{arithmetique@arithmétique vectorielle}arithmétique
  vectorielle, l'application avec \icode{lapply} et \icode{sapply}
  constitue le principal moyen d'effectuer des calculs répétitifs dans
  R. On ne saurait donc trop insister sur l'importance de ces deux
  fonctions.}
...
de matrice, selon le cas, \code{FUN(v[1], w[1])}, \code{FUN(v[2],
  w[2])}, \code{FUN(v[3], w[3])}, etc. La
\autoref{fig:donnees:application:mapply} offre une représentation de
...
\begin{figure}
  ...
  \input{include/cube}
  \centering
  \begin{tabularx}{0.8\linewidth}{cXcXc}
    & & \code{mapply(FUN, x, y)} \\
    \begin{picture}(4.5,2.3)
      \put(0,2.5){\makebox(0,0){\code{x}}}
      \multiput(1,2.0)(1,0){3}{\usebox{\cube}}
      \put(0,0.5){\makebox(0,0){\code{y}}}
      \multiput(1,0.0)(1,0){3}{\usebox{\dcube}}
    \end{picture}
    & \quad\raisebox{1.0\unitlength}{\faArrowRight} &
    \begin{picture}(3.5,4.3)(-1.0,0.0)
      \put(-4.0,1.0){\makebox(4.0,0)[l]{\code{FUN} $\mapsto$}}
      \multiput(0,-1.5)(0,2){3}{\usebox{\ccube}}
      \multiput(1.5,-1.5)(0,2){3}{\usebox{\dccube}}
    \end{picture}
    \rule[-1.5\unitlength]{0pt}{4.3\unitlength} % éloigner légende
    & \quad\raisebox{1.0\unitlength}{\faArrowRight} &
    \begin{picture}(3.5,1.8)
      %% résultats
      \multiput(0,1.0)(1,0){3}{\usebox{\cube}}
    \end{picture}
  \end{tabularx}
  \caption{Représentation de la fonction d'application \code{mapply}}
  \label{fig:donnees:application:mapply}
\end{figure}
...
Le \Index{tableau de données}%
\index{data frame|see{tableau de données}}tableau de données
(\emph{data frame}\footnote{%
...
Les facteurs sont peu utilisés en tant que structure de données en
programmation. En revanche, ils s'avèrent essentiels pour les calculs
par groupe de données à l'aide d'une fonction d'application.

\warningbox{Évitez d'effectuer des calculs avec les codes des
  catégories d'un facteur. Ça peut être très tentant lorsque les codes
  sont des valeurs numériques (\code{"0"} et \code{"1"}, par exemple).
  Vous devez d'abord récupérer les véritables valeurs
  numériques
  correspondant aux codes à l'aide de la recette recommandée dans la
  rubrique d'aide de \icode{factor}.}
...
La mécanique de \code{tapply} est illustrée à la
\autoref{fig:donnees:application:tapply}.
...
\begin{figure}
  ...
  \begin{tabular}{cc}
    \code{X} & \code{INDEX} \\
    \begin{picture}(10.5,1.3)
      \multiput(0,0)(1,0){10}{\usebox{\cube}}
    \end{picture}
    &
    \begin{picture}(10.5,1.3)
      \put(0,0){\usebox{\mcube}}
      \put(1,0){\usebox{\dcube}}
      \put(2,0){\usebox{\mcube}}
      \put(3,0){\usebox{\mcube}}
      \put(4,0){\usebox{\cube}}
      \put(5,0){\usebox{\dcube}}
      \put(6,0){\usebox{\cube}}
      \put(7,0){\usebox{\mcube}}
      \put(8,0){\usebox{\cube}}
      \put(9,0){\usebox{\mcube}}
    \end{picture}
  \end{tabular} \\[4mm]
  \begin{tabularx}{0.6\linewidth}{cXc}
    \code{tapply(X, INDEX, FUN)} \\
    \begin{picture}(10.5,5)(-4.0,-0.8)
      \thicklines
      \put(-4.0,1.0){\makebox(4.0,0)[l]{\code{FUN} $\mapsto$}}
      \multiput(0.0, 2.5)(1,0){3}{\usebox{\ccube}}
      \multiput(0.0, 0.5)(1,0){5}{\usebox{\mccube}}
      \multiput(0.0,-1.5)(1,0){2}{\usebox{\dccube}}
    \end{picture}
    \rule[-1\unitlength]{0pt}{5\unitlength} % éloigner légende
    & \raisebox{1.5\unitlength}{\faArrowRight} &
    \begin{picture}(3.5,1.3)(0,-0.8)
      \multiput(0,0.5)(1,0){3}{\usebox{\cube}}
    \end{picture}
  \end{tabularx}
  \caption{Représentation de la fonction d'application \code{tapply}}
  \label{fig:donnees:application:tapply}
\end{figure}
...
## Il arrive que les codes des catégories soient des valeurs
## numériques, surtout lorsque les données proviennent de
## systèmes externes (autres que R) qui ne reconnaissent pas
## les facteurs ou les données booléennes.
##
## Par exemple, les catégories «non membre» et «membre»
## représentées dans un système par des 0 et des 1 deviennent,
## dans R, un facteur avec les catégories "0" et "1".
(is_member <- factor(c(1, 0, 1, 1, 0, 0, 1, 0, 1, 1)))
levels(is_member)

## Les valeurs numériques sous-jacentes n'ont toutefois aucun
## lien avec les codes des catégories.
as.integer(is_member)

## La manière sécuritaire de récupérer les valeurs numériques
## des codes est fournie dans ?factor. Il est ensuite possible
## d'effectuer des calculs avec les valeurs numériques comme
## à l'accoutumée.
(x <- as.numeric(levels(is_member))[is_member])
sum(x)                     # nombre de membres
mean(x)                    # proportion de membres
...
comparaison, génération de suites, etc. L'infrastructure repose sur
...
\begin{figure}[t]
  \centering
  %% taille d'image choisie pour que toutes les boites signalétiques
  %% entrent sur la page suivante
  \begin{minipage}{0.46\linewidth}
    \setkeys{Gin}{width=\textwidth}
    \includegraphics{images/bug} \\
    \footnotesize\sffamily%
    Tiré de \href{https://xkcd.com/376/}{XKCD.com}
  \end{minipage}
\end{figure}
%
...
\tipbox{Les rubriques d'aide de \code{Date} et de
  \code{DateTimeClasses} contiennent une foule de détails additionnels
  sur la représentation et le traitement des dates dans R.}

%% La boite signalétique ci-dessus étant grande, je la place dans un
%% float pour éviter un saut de page disgracieux
\begin{figure}
  \notebox{Les exemples de la \autoref{sec:donnees:date} utilisent
    tous la date du 29 février 2000. Que s'est-il passé de spécial ce
    jour-là? Les années multiples de 100 ne sont pas bissextiles, sauf
    celles multiples de 400. C'est pourquoi 2000 était bel et bien une
    année bissextile. La prochaine du type aura donc lieu en{\dots}
    2400. Certains logiciels de l'époque étaient affligés d'un bogue
    qui faisait en sorte que le 29 février 2000 n'existait pas dans
    leur calendrier. C'est beaucoup par espièglerie que le \emph{R
      Core Team} a justement choisi cette journée pour lancer la
    \link{https://stat.ethz.ch/pipermail/r-announce/2000/000127.html}{version
      1.0.0 de R}.}
\end{figure}
...
\item[\Icode{nrow}] \index{matrice!nombre de lignes}nombre de
  lignes.
<<echo=TRUE>>=
nrow(x)
@
\item[\Icode{ncol}] \index{matrice!nombre de colonnes}nombre de
  colonnes.
<<echo=TRUE>>=
ncol(x)
@
...
\begin{exercice}
  ...
  \item \verb|as.POSIXlt(as.Date("2001-05-09"))$mday|
...
@
  ...
  \begin{sol}
    \begin{enumerate}
\item Extraction d'un élément unique à l'aide de d'un indice de ligne
  et d'un indice de colonne.
<<echo=TRUE,eval=FALSE>>=
x[4, 3]
@
\item Extraction d'une ligne complète à l'aide d'un indice de ligne.
<<echo=TRUE,eval=FALSE>>=
x[6, ]
@
\item Extraction de deux colonnes complètes à l'aide de deux indices
  de colonne.
<<echo=TRUE,eval=FALSE>>=
x[, c(1, 4)]
@
\item Extraction de lignes par critère, critère lui-même obtenu par un
  calcul sur les éléments de la première colonne.
<<echo=TRUE,eval=FALSE>>=
x[x[, 1] > 50, ]
@
    \end{enumerate}
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
\item Sommes par ligne.
<<echo=TRUE, eval=FALSE>>=
rowSums(m)
@
\item Moyennes par colonne.
<<echo=TRUE, eval=FALSE>>=
colMeans(m)
@
\item Indiçage de la matrice d'origine et calcul du maximum.
<<echo=TRUE, eval=FALSE>>=
max(m[1:3, 1:3])
@
\item Extraction par critère, celui-ci étant obtenu en calculant les
  moyennes par ligne.
<<echo=TRUE, eval=FALSE>>=
m[rowMeans(m) > 7, ]
@
...
x <- list(1:5, data = matrix(1:6, 2, 3), numeric(3),
          test = logical(4))
@
    \item La fonction \icode{names} permet d'extraire les étiquettes
      d'un objet.
<<echo=TRUE,eval=FALSE>>=
names(x)
@
    \item Extraire le quatrième élément de la liste avant d'utiliser
      \icode{mode} et \icode{length} pour obtenir son mode et sa
      longueur.
<<echo=TRUE,eval=FALSE>>=
mode(x$test)
length(x$test)
@
    \item Extraire le deuxième élément de la liste avant d'utiliser
      \icode{dim} pour obtenir ses dimensions.
<<echo=TRUE,eval=FALSE>>=
dim(x$data)
@
    \item Le truc ici consiste à indicer deux fois: d'abord avec des
      crochets doubles pour extraire le second élément de la liste,
      puis avec des crochets simples (puisque le résultat de la
      première extraction est un vecteur) pour extraire les deuxième
      et troisième éléments.
<<echo=TRUE,eval=FALSE>>=
x[[2]][c(2, 3)]
@
    \item Il faut utiliser des crochets doubles.
<<echo=TRUE,eval=FALSE>>=
x[[3]] <- 3:8
@
    \end{enumerate}
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \begin{enumerate}
\item Il s'agit de calculer les moyennes pondérées par lignes.
<<echo=TRUE,eval=FALSE>>=
...
@
\item Il s'agit de calculer les moyennes pondérées par colonnes.
<<echo=TRUE,eval=FALSE>>=
...
@
\item Ici, le résultat est la moyenne pondérée de toutes les données.
<<echo=TRUE,eval=FALSE>>=
sum(Xij * wij)/sum(wij)
@
\item Il s'agit de calculer les moyennes pondérées par carottes
  horizontales.
<<echo=TRUE,eval=FALSE>>=
apply(Xijk * wijk, c(1, 2), sum) /
    apply(wijk, c(1, 2), sum)
@
\item Il s'agit de calculer les moyennes pondérées par tranches
  horizontales.
<<echo=TRUE,eval=FALSE>>=
...
@
\item Il s'agit à nouveau de la moyenne pondérée de toutes les
  données.
<<echo=TRUE,eval=FALSE>>=
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
\item Générer séparément les suites de $0$ jusqu'à $1, 2, \dots, 10$, puis
  les placer bout à bout.
<<echo=TRUE>>=
unlist(lapply(0:10, seq, from = 0))
@
\item Générer séparément les suites de $1, 2, \dots, 10$ $0$ jusqu'à $10$,
  puis les placer bout à bout.
<<echo=TRUE>>=
unlist(lapply(1:10, seq, from = 10))
@
\item Générer séparément les suites de $10, 9, \dots, 1$ $0$ jusqu'à $1$,
  puis les placer bout à bout.
<<echo=TRUE>>=
...
@
  ...
  \item Nommer les éléments de la liste créée en a) \code{sample1},
    \code{sample2}, \dots, \code{sample5} à l'aide de la fonction
    \icode{paste} présentée à la \autoref{sec:bases:internes:texte}.
  ...
  \begin{sol}
    \begin{enumerate}
\item La fonction \code{rpareto} n'est pas vectorielle pour son argument
  \code{n}. Il faut avoir recours à une fonction d'application pour
  simuler plusieurs échantillons de taille différentes. La fonction à
  utiliser est \icode{lapply} puisqu'elle  permet d'itérer
  sur un vecteur de tailles d'échantillons et qu'elle retourne son
  résultat dans une liste, tel que demandé.
<<echo=TRUE>>=
x <- lapply(seq(100, 300, by = 50), rpareto,
            shape = 2, scale = 5000)
@
\item On attribue des étiquettes à la liste avec \icode{names}.
<<echo=TRUE>>=
names(x) <- paste("sample", 1:5, sep = "")
@
\item Il faut encore utiliser un fonction d'application. Pour
  retourner le résultat directement sous forme de vecteur, c'est
  \icode{sapply} qu'il nous faut.
<<echo=TRUE,eval=FALSE>>=
sapply(x, mean)
@
\item Définissons d'abord une fonction pour calculer la fonction de
  répartition de la loi de Pareto. Celle-ci est vectorielle pour son
  argument \code{q}.
<<echo=TRUE>>=
ppareto <- function(q, shape, scale)
    1 - (scale/(q + scale))^shape
@
  Nous pouvons ensuite effectuer le calcul demandé, mais nous avons
  besoin d'une fonction anonyme pour aussi trier les résultats.
<<echo=TRUE,eval=FALSE>>=
lapply(x, function(x) sort(ppareto(x, 2, 5000)))
@
\item Pour utiliser un opérateur arithmétique comme fonction dans une
  fonction d'application, il faut le placer entre guillemets.
<<echo=TRUE,eval=FALSE>>=
lapply(x, "+", 1000)
@
    \end{enumerate}
