...
sélectionne des «tranches» du tableau, alors que l'indiçage sur deux
dimensions simultanément sélectionne des «carottes»\footnote{%
  Un terme que j'emprunte à la géologie, où une carotte est un
  échantillon de roche cylindrique extrait d'un sol par forage.} %
ou des tubes. Ces idées se généralisent aisément à des tableaux à plus
de trois dimensions. Les figures~\ref{fig:donnees:indicage:matrice} et
\ref{fig:donnees:indicage:tableau} proposent respectivement des
représentations schématiques des opérations d'indiçage d'une matrice et
...
est $2$, on applique plutôt \code{FUN} à des «carottes»\footnote{%
  Un terme que j'emprunte à la géologie, où une carotte est un
  échantillon de roche cylindrique extrait d'un sol par forage.} %
tirées de \code{X} (des vecteurs).
