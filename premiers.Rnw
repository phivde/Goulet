...
Regardons cela de plus près. Aux fins d'illustrations, définissons
tout d'abiord une petite fonction \code{foo} qui calcule la quantité
\begin{equation*}
...
Cette section présente quelques fonctions internes souvent utilisés
pour programmer en R et pour manipuler des données. Elle reviendra à
intervalles réguliers dans le document au fur à mesure que nous
