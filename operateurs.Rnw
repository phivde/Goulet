...
\begin{objectifs}
\item Connaître les règles de l'arithmétique vectorielle
...
\item Savoir utiliser la fonction \code{if} pour l’exécution
  conditionnelle de commandes R.
\item Distinguer la construction \code{if() ... else} de la fonction
  \code{ifelse}.
\item Savoir faire des boucles en R.
\item Savoir choisir entre les opérateurs \code{for}, \code{while} et
...
Le tableau~\ref{tab:operateurs:operateurs} présente les opérateurs
...
\begin{table}
  ...
  \begin{tabular}{lp{7cm}}
    ...
    \verb|:|\index{:@\verb=:=}  & génération de suites \\
    ...
    \Fonction{|}\, \Fonction{||}    & «ou» logique \\
    \fonction{->}\, \verb|->>|\index{->>@\verb=->>=|textbf} & assignation \\
    \fonction{<-}\, \verb|<<-|\index{<<-@\verb=<<-=|textbf} & assignation \\
    \bottomrule
  \end{tabular}
...
Les opérateurs de puissance (\fonction{\^}) et d'assignation à gauche
(\fonction{<-}, \verb|<<-|) sont évalués de droite à gauche; tous les
autres de gauche à droite. Ainsi, \verb|2 ^ 2 ^ 3| est \verb|2 ^ 8|,
et non \verb|4 ^ 3|, alors que \verb|1 - 1 - 1| vaut \verb|-1|, et non
\verb|1|.
...
Les opérateurs du tableau~\ref{tab:operateurs:operateurs} constituent
...
Le langage R compte un très grand nombre (quelques milliers!) de
fonctions internes. Cette section en présente quelques-unes seulement,
les fonctions de base les plus souvent utilisées pour programmer en R
...
section~\ref{operateurs:exemples} fournit des exemples additionnels,
...
%%% plus compacts.
\fvset{listparameters={\setlength{\topsep}{-0.5ex}}}
\renewenvironment{Schunk}{\setlength{\topskip}{-\parskip}}{}
...
sort(c(4, -1, 2, 6))
@
\item[\Fonction{order}] ordre d'extraction des éléments d'un vecteur
  pour les placer en ordre croissant ou décroissant
<<echo=TRUE>>=
order(c(4, -1, 2, 6))
@
...
\item[\Fonction{solve}] avec un seul argument (une matrice carrée):
  ...
  (une matrice carrée et un vecteur): solution du système d'équations
  linéaires $\mat{A} \mat{x} = \mat{b}$
<<echo=TRUE>>=
solve(x)
solve(x, c(1, 2))
@
...
Le produit extérieur n'est pas la fonction la plus intuitive à
utiliser, mais s'avère extrêmement utile pour faire plusieurs
opérations en un seul appel de fonction tout en évitant les boucles.
La syntaxe de la fonction \Fonction{outer} est
\begin{quote}
  \code{outer(X, Y, FUN)}
\end{quote}
Le résultat est l'application la fonction \code{FUN} (\fonction{prod}
...
disponibles en R. Se reporter à la section~\ref{operateurs:exemples}
...
\fonction{sapply} et \fonction{mapply} (section \ref{avance:apply})
...
des packages déjà en mémoire avec\Indexfonction{search}
<<echo=TRUE>>=
search()
@ %
et le contenu de toute la bibliothèque avec la fonction
\Fonction{library} (résultat non montré ici).
...
compte aujourd'hui plusieurs centaines d'extensions et le nombre ne
cesse de croître.

Le système R rend simple de télécharger et d'installer de nouveaux
packages avec la fonction \Fonction{install.packages}.
L'annexe~\ref{packages} explique plus en détails comment gérer sa
