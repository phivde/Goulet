...
rank(c(4, -1, 2, 6))
@
\item[\code{\ul{order}}] \Indexfonction{order}
  ordre\index{ordre}\marginpar{\video} d'extraction des
...
La \ul{fonction \texttt{outer}}\marginpar{\video}\Indexfonction{outer}
...
évitant les boucles. La syntaxe \fonction{outer} est
\begin{quote}
  \code{outer(X, Y, FUN)}
\end{quote}
