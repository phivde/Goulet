...
fonctions permettant de calculer les coefficients d'une régression
simple ou multiple. Les outils disponibles vont toutefois bien au-delà
de ce calcul relativement simple. Ainsi, par l'entremise de quelques
...
Ce chapitre présente les principales fonctions --- dont la liste se
...
de S-Plus ou \textsf{R}, ainsi que \citet{MASS} pour de plus amples
détails.

À noter que l'on souligne les menues différences qui existent entre
S-Plus et \textsf{R}, mais que les exemples ont été exécutés en
\textsf{R}.
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    ...
    \item[2] Dans le package \texttt{MASS}\index{package!MASS@\texttt{MASS}}.
    \end{tablenotes}
  \end{threeparttable}
...
La modélisation statistique en S --- par exemple, l'analyse de
...
\textsf{R} des variables que l'on entend inclure dans celle-ci et
leurs relations entre elles. La convention utilisée dans le langage S
est celle dite des «formules»\Index{formule}. Le tableau
\ref{tab:formules} présente quelques exemples de formulation de
modèles linéaires simples en S.
...
S-Plus et \textsf{R} diffèrent quant au calcul des intervalles de
confiance et de prévision. Dans \Splus S-Plus, les intervalles de
confiance en chaque point seront calculés par \fonction{predict} avec
l'option \code{ci.fit = T}\indexargument{ci.fit} alors que les
intervalles de prévision le sont avec \texttt{pi.fit =
  T}\indexargument{pi.fit}. Il est par conséquent possible de calculer
en un seul appel à \fonction{predict} les deux types d'intervalles. Le
niveau de confiance est spécifié avec l'option \argument{conf.level}
($0,95$ par défaut).
...
\verb!interval = "confidence"!\indexargument{interval} %
alors que pour les intervalles de prévision on utilise %
\verb!interval = "prediction"!. %
Le niveau de confiance est déterminé avec l'option \argument{level}.
Le résultat est une matrice de trois colonnes dont la première
contient les prévisions et les deux autres les bornes inférieures
(\code{lwr}) et supérieures (\code{upr}) des intervalles de confiance.
...
\R (\textsf{R} seulement).  Consulter les rubriques d'aide et les
exemples pour de plus amples détails.
