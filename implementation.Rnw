...
d'environnement et de portée lexicale de R. Maitriser ces
...
La notion demeure pour l'instant plutôt abstraite. Elle devient plus
concrète dans les contextes de création et d'appel de fonctions, où
les environnements jouent un rôle important. Nous nous concentrons
dans cette section sur le cas de la création des fonctions; le rôle
des environnements dans les appels de fonctions est étudié à la
section suivante.
...
arguments formels de la fonction et le corps et la fonction. Le
troisième élément est l'environnement de la fonction. L'environnement
englobant de celui-ci est l'environnement actif au moment de la
définition de la fonction (à moins qu'il ne soit changé) et le
dictionnaire «capture» toutes les paires symbole-valeur de ce même
environnement actif.

Autrement dit, dès que nous définissons une fonction, R lui attribue
un environnement. Lorsque la fonction est crée dans l'espace de
travail, son environnement englobant est l'\emph{environnement
  global}\Index{environnement!global}, représenté par le symbole
\Icode{.GlobalEnv}, et son dictionnaire contient tous les objets se
trouvant dans l'espace de travail. (Les expressions ci-dessous sont
exécutées dans un espace de travail vide au préalable.)
<<echo=FALSE>>=
rm(list = ls())
@
<<echo=TRUE>>=
...
Quelques fonctions permettent de manipuler l'environnement d'une
fonction. Nous en avons déjà utilisé trois ci-dessus.
\begin{ttscript}{environment}
\item[\Icode{environment}] extraction ou modification de
  l'environnement d'une fonction
\item[\icode{ls}] affichage de la liste des éléments du
  dictionnaire de l'environnement
\item[\Icode{get}] extraction de la valeur associée à un symbole
  dans le dictionnaire de l'environnement
\item[\Icode{assign}] affectation d'une nouvelle paire
  symbole-valeur dans le dictionnaire de l'environnement
...
\item Lorsque R atteint l'environnement
  global\index{environnement!global} \icode{.GlobalEnv}, la recherche
...
fonctions crées à l'intérieur d'autres fonctions puisque nous pouvons
faire l'économie de passer tous les arguments d'une fonction à
l'autre. On trouvera un exemple complet dans le code informatique de la
...
fonctions appelée \emph{évaluation paresseuse}\index{évaluation
  paresseuse} (\emph{lazy evaluation}). Ce que cela signifie, c'est
que toute expression passée en argument à une fonction ne sera évaluée
qu'au moment où la valeur de l'argument sera utilisé dans la fonction,
et pas avant. Ainsi, une expression en argument pourrait ne jamais
être évaluée.

La fonction \code{sumsq} ci-dessous calcule la somme des écarts au
carré entre les éléments d'un vecteur \code{y} et une valeur pivot
\code{about}, par défaut la moyenne des éléments de \code{y}.
<<echo=TRUE>>=
...
\begin{exercice}
  La fonction \icode{ecdf} de R prend en argument un vecteur de
  données et retourne une fonction pour calculer la fonction de
  répartition empirique de ces données:
  \begin{equation*}
...
Fn(0:3)
@
  Dans l'exemple ci-dessus, on remarque que lorsque l'on appelle la
  fonction \code{Fn} pour calculer $F_n(x)$ en diverses valeurs de
  $x$, il n'est pas nécessaire de fournir le vecteur des données.
  Expliquer pourquoi. \emph{Astuce}: fouiller dans l'environnement de
  la fonction \code{Fn}.
  \begin{sol}
    On a
<<echo=TRUE>>=
...
\begin{exercice}
  On évalue les expressions suivantes dans un espace de travail vide.
<<echo=TRUE>>=
y <- 123
f <- function(x)
{
...
@
  Déterminer le résultat de l'expression ci-dessous.
<<echo=TRUE, eval=FALSE>>=
f(10)
@
  \begin{sol}
