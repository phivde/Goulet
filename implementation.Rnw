...
qu'au moment où la valeur de l'argument sera utilisée dans la fonction,
et pas avant. Ainsi, une expression en argument pourrait ne jamais
être évaluée. Ou encore, la valeur par défaut d'un argument peut
dépendre de la valeur d'un autre argument puisque celui-ci sera connu
...
\begin{exercice}
  ...
  données $(x_1, \dots, x_n)$ et retourne une fonction pour calculer
  la fonction de répartition empirique de ces données:
  \begin{equation*}
