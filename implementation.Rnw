...
Quelques fonctions permettent de manipuler les environnement de
...
@
  \begin{sol}
    Le résultat de \code{f(1)} est $3$. Nous avons ici une fonction
    tout à fait standard qui retourne la somme d'un nombre en argument
    (\code{x}) et d'un nombre défini à l'intérieur de la fonction
    (\code{a}).
  \end{sol}
\end{exercice}
