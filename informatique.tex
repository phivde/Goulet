...
Le terme \emph{Unix} --- ou UNIX qui, en majuscules, est une marque
déposée de \link{http://www.opengroup.org/unix}{Open Group} ---
...
\item Tout utilisateur dispose d'un espace disque réservé identifié
  comme le \emph{répertoire personnel} ou le \emph{dossier de départ}
...
Le système de fichiers d'un ordinateur contrôle l'inscription,
l'organisation et la récupération des données sur une unité de
stockage, souvent un disque dur.

Loin de nous l'idée d'entrer dans les détails techniques des systèmes
de fichiers. Nous souhaitons seulement attirer l'attention des
lecteurs sur quelques points susceptibles de faciliter  la logique de
classement des données dans un ordinateur

En premier lieu, les fichiers sont regroupés dans des répertoires qui
eux-mêmes contiennent des fichiers ou des sous-répertoires. Ce schéma
...
\osxbox{Les clés USB, disques durs externes ou disques réseau sont
  montés dans le répertoire \code{/Volumes} sous macOS. Le répertoire
  n'existe pas lorsque aucun disque externe n'est connecté.}
...
Le chemin d'accès (\emph{path}) d'un fichier ou d'un répertoire décrit
la position de la ressource dans le système de fichier.
Un chemin d'accès peut-être absolu ou relatif.
\begin{description}
\item[Chemin absolu] La position d'un fichier est décrite à partir de
  la racine, de telle sorte que le chemin d'accès demeure valide
  depuis n'importe quel point dans le système de fichier.
...
\end{Sinput}
\end{Schunk}
\item[Chemin relatif] La position d'un fichier est donnée à partir
  d'un endroit précis dans le système de fichier autre que la racine.
  Le chemin dépend donc du répertoire courant. On a recours au nom
  fictif «\verb=..=» pour identifier le répertoire parent (un niveau
  supérieur dans l'arbre des fichiers).
...
d'expliquer comment modifier la variable d'environnement. Toutefois,
la réponse se trouve à une requête près dans un moteur de recherche.


\section{Ligne de commande}
\label{sec:informatique:cli}

Une interface en ligne de commande (\emph{command line interface},
CLI), ou tout simplement \emph{ligne de commande}, est un mode
d'interaction avec un programme informatique dans lequel l'utilisateur
dicte les commandes et reçoit les réponses de l'ordinateur en mode
texte. Le programme qui gère cette interface est un \emph{interpréteur
  de commandes}, parfois aussi appelé (à tort) \emph{terminal} ou
\emph{shell}. Lorsque l'interpréteur est prêt à recevoir une commande,
il l'indique par une \emph{invite de commande} (\emph{command
  prompt}).
...
Sous Windows, l'interpréteur de commandes standard est \code{cmd.exe}.
On le trouve dans le groupe de programmes \code{Accessoires} sous le
nom \code{Invite de commandes}. L'invite de commande par défaut est
\verb=C:\>=. Le répertoire courant au lancement de l'interpréteur est
donc la racine du système de fichiers.

Les programmeurs qui souhaitent bénéficier de certains outils Unix
sous Windows peuvent installer les couches logicielles MinGW et MSYS
--- nous recommandons à cet égard la distribution
\link{http://www.msys2.org}{MSYS2}. Le programme \code{MSYS} fournit
un interpréteur de commandes Bash \citep{bash} comme en standard
sur les systèmes Unix.

\windowsbox{On identifie la racine \code{C:\textbackslash} du
  système de fichier d'un disque par \code{/c/} dans Bash sous
  Windows.}
...
commandes est Bash. L'invite de commande par défaut est \verb=$= et le
répertoire courant au lancement de l'interpréteur est le répertoire
personnel \,\verb=~=\, de l'utilisateur.

Nous considérons essentiel pour des programmeurs de connaître les
rudiments de la ligne de commande. Cela dit, comme nous ne pouvons
nous étendre sur le sujet, nous limiterons notre présentation aux
commandes suivantes de déplacement dans le système de fichiers et
d'affichage des fichiers d'un répertoire.

\begin{ttscript}{pwd}
\item[\code{cd}] Change le répertoire courant pour celui
  donné en argument. Cet argument peut être un chemin d'accès absolu
  ou relatif (\autoref{sec:informatique:fs:path}). Le nom fictif
  «\verb=..=» identifie le parent du répertoire courant.
...
\item[\code{pwd}] (Bash seulement) Affiche le chemin
  d'accès absolu du répertoire courant.
\item[\code{dir}] (\code{cmd.exe} seulement) Affiche les
  fichiers du répertoire donné en argument, ou les fichiers du
  répertoire courant sans aucun argument.
\item[\code{ls}] (Bash seulement) Affiche les fichiers du
  répertoire donné en argument, ou les fichiers du répertoire courant
  sans aucun argument.
\end{ttscript}
...
\section*{Chapitre \ref*{informatique}}
\addcontentsline{toc}{section}{Chapitre \protect\ref*{informatique}}
...
\begin{exercice}[nosol]
  ...
  ayant servi sources d'inspiration à Swift et à Go.
\end{exercice}
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
    \item $/\, \times\, A\, +\, B\; C\; D$ en notation préfixée;
      $A\; B\; C\, +\, \times\, D\; /$ en notation suffixée.
    \item $A \times (B + C / D)$
    \end{enumerate}
  \end{sol}
\end{exercice}
