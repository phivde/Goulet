...
programmation standard. De ces rencontres naîtra \index{Algol}Algol
...
\index{MinGW}MinGW et \index{MSYS}MSYS --- nous recommandons à cet
...
Dans cet environnement, on identifie la racine \code{C:\textbackslash}
du système de fichier d'un disque par \code{/c/}.
...
Les programmeurs doivent connaître les rudiments de la ligne de
...
\begin{ttscript}{pwd}
\item[\code{cd}] \indexfonction{cd} Change le répertoire courant pour
...
\item[\code{pwd}] \indexfonction{pwd} (\index{Bash}Bash seulement) Affiche le
  chemin d'accès absolu du répertoire courant.
\item[\code{dir}] \indexfonction{dir}
  (\index{cmd@\code{cmd.exe}}\code{cmd.exe} seulement) Affiche les
  fichiers du répertoire donné en argument, ou les fichiers du
  répertoire courant sans aucun argument.
\item[\code{ls}] \indexfonction{ls} (\index{Bash}Bash seulement) Affiche les
  fichiers du répertoire donné en argument, ou les fichiers du
  répertoire courant sans aucun argument.
\end{ttscript}
...
\begin{exercice}[nosol]
  ...
  ayant servi sources d'inspiration à \index{Swift}Swift et à
  \index{Go}Go.
\end{exercice}
...
\begin{exercice}[nosol]
  ...
  d'exploitation de votre ordinateur est Windows, installer d'abord
