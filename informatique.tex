...
\index{Unix}Unix. Cela ne laisse donc en fait que deux grandes classes
...
natif, via un port depuis \index{Unix}Unix, ou encore par
...
droits accordés à \index{administrateur systeme@administrateur
  système}l'administrateur système (communément appelé
...
est réservé, identifié comme le \index{repertoire personnel@répertoire
...
\osxbox{Vous savez désormais pourquoi le \index{repertoire
    ...
    directory}, est représenté par une maison dans le \index{Finder}Finder et que le
...
uniquement par l'\index{administrateur systeme@administrateur
  système}administrateur système) et ceux des utilisateurs. Ceux-ci ne
peuvent créer et modifier des fichiers que dans leur \index{repertoire
  personnel@répertoire personnel}répertoire personnel. Ce répertoire
est situé dans \code{/Users} sous macOS et habituellement dans
\code{/home} sous Linux.
...
\osxbox{Sous macOS, les clés USB, les disques durs externes et les
  disques réseau sont montés dans le répertoire \code{/Volumes}. Ce
  répertoire n'existe pas lorsqu'aucun disque externe n'est connecté.}
...
et \verb=$PATH= sous \index{Unix}Unix. Il est hors de la portée de cet
...
\begin{exercice}[nosol]
  ...
  de la racine avec l'\index{Explorateur Windows}Explorateur Windows
  ou avec le \index{Finder}Finder.
\end{exercice}

\begin{exercice}[nosol]
  (Utilisateurs de Mac\marginpar{\hfill\faApple} seulement.) Dans les
  versions modernes de macOS, le \index{Finder}Finder n'affiche plus
  par défaut le dossier personnel dans la barre latérale. C'est
  pourtant pratique de pouvoir y accéder directement. Effectuer les
  opérations suivantes pour afficher le raccourci vers le répertoire
  personnel.
  \begin{enumerate}[1.]
