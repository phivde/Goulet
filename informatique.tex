...
\index{semantique@sémantique}sémantique et de \index{syntaxe}syntaxe.
...
l'interlocuteur\footnote{%
  La formulation correcte est «I am thirsty», soit littéralement «Je
  suis soif», en français.}.
...
\index{compilé (langage)}\emph{compilé}. Le compilateur produit un
...
fichiers par leurs extensions \code{.exe} ou \code{.dll}. Les noyaux
...
\index{interprété (langage)}langage interprété ne peut être distribué
...
\index{notation!préfixée}notation préfixée (voir
...
\index{C++@\Cpp}\Cpp, \index{C#@C\#}C\# et \index{Java}Java reprennent
...
\notebox{Dans l'ouvrage classique de \cite{KandR:1978}, le premier
  ...
  fonctionnement ou la syntaxe d'un langage avec ce même exemple.}
...
\index{repertoire personnel@répertoire personnel}\emph{répertoire
  personnel} ou le %
\index{dossier de départ|see{répertoire personnel}}\emph{dossier de
...
\macosbox{Vous savez désormais pourquoi le %
...
\windowsbox{Le répertoire personnel n'est pas affiché par défaut dans
  l'\index{Explorateur Windows}Explo\-ra\-teur Windows. Pour y accéder,
...
\macosbox{Sous \index{macOS}macOS, les clés USB, les disques durs
...
\begin{exercice}[nosol]
  ...
  l'\index{Explorateur Windows}Explorateur Windows ou avec le
  \index{Finder}Finder.
\end{exercice}

\begin{exercice}[nosol]
  (macOS seulement) Dans les versions modernes de \index{macOS}macOS,
  le \index{Finder}Finder n'affiche plus par défaut le dossier
  ...
  \item Ouvrir une fenêtre du Finder et accéder aux options de
    configuration par le menu standard \code{Finder|Préférences}
    (\code{\cmdkey\,,}).
