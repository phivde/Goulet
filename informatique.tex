...
ce langage qu'il est «compilé». Le compilateur produit un fichier
...
«interprété». Un programme écrit dans un langage interprété ne
...
\item[Orienté objet] Un programme est conçu comme un ensemble de blocs
  ...
  «méthode» applique un traitement différent à un objet
  selon sa «classe». Ce paradigme est particulièrement utilisé
...
\item Toujours à la ligne de commande, il est possible d'appuyer sur
  ...
  commandes.
\end{itemize}
La \autoref{sec:informatique:fs} fournit des détails additionnels sur
le système de fichiers de Unix.
...
forme d'arbre dont la «racine» est le répertoire contenant
...
Si le système comporte plus d'un disque, l'utilisateur doit
constamment savoir sur quel disque retrouver ou enregistrer ses
données.
...
c'est-à-dire qu'il ne fait aucune distinction entre \code{Program
  Files}, \code{program files} et \code{PROGRAM FILES}.

Dans les chemins d'accès (voir ci-dessous), on sépare les noms de
répertoires par la barre oblique inversée \textbackslash\,.

\subsection{Particularités du système de fichiers de Unix}
\label{sec:informatique:fs:unix}

La racine du système de fichiers est toujours \code{/} sous Unix.
Les disques, physiques ou réseau, sont accessibles par divers «points
de montage» dans le système de fichiers. Pour illustrer, supposons
qu'un ordinateur dispose de deux disques durs; le premier est réservé
au système d'exploitation et aux applications partagées, alors que le
second renferme les fichiers personnels des utilisateurs. Dans un tel
scénario, le premier disque sera monté sur le répertoire \code{/} et
le second, sur le répertoire \code{/home}. Le système de fichier
dirigera automatiquement les requêtes vers le bon disque.
L'utilisateur n'a jamais à se préoccuper de l'organisation physique
des disques.

\osxbox{Sous macOS, les clés USB, disques durs externes ou disques
  réseau sont montés dans le répertoire \code{/Volumes}. Si aucun
  disque externe n'est connecté, le répertoire n'existe pas.}

Le système de fichiers sous Unix est généralement sensible à la casse.
On sépare les noms de répertoires par la barre oblique \code{/} dans
les chemins d'accès.

Les fichiers dont le nom débute un point «\verb=.=» --- souvent des fichiers
de configuration, tel que mentionné à la
...
donc pas dans les listes de fichiers.
...
\item[Chemin absolu] La position d'un fichier est décrite à partir de
  la racine, de telle sorte que le chemin d'accès demeure valide où
  que l'on se trouve dans le système de fichier.
...
\item[Chemin relatif] La position d'un fichier est donnée à partir
  d'un point dans le système de fichier autre que la racine. Le chemin
  dépend donc de l'endroit où l'on se trouve dans le système de
  fichier. On a recours à deux points consécutifs «\verb=..=» pour
  signifier une remontée d'un niveau dans l'arbre des fichiers.

  Par exemple, pour une application dont le répertoire de travail
  courant est \code{Documents/Cours} de l'utilisatrice Marianne dans
  le système de fichier Unix de la \autoref{fig:informatique:fs}, le
  chemin d'accès relatif vers le fichier \code{freestyle.txt} est:
\begin{Schunk}
\begin{Sinput}
...
répertoires dans lesquels le système d'exploitation va chercher
lorsqu'une application est appelée. Cette liste est conservée dans une
variable d'environnement nommée \verb=%PATH%= sous Windows
et \verb=$PATH= sous Unix. Il est hors de la portée de ce document
d'expliquer comment modifier la variable d'environnement, mais la
