...
L'étude des algoirthmes classiques de tri et de recherche fera l'objet
...
\importantbox{Le répertoire personnel n'est pas affiché par défaut
  dans l'Explorateur Windows. Pour y accéder, partir de la racine du
  système de fichiers dans «Ce PC» ou «Ordinateur».}
...
\begin{exercice}[nosol]
  Les répertoires \code{Documents} et \code{Desktop} (\code{Bureau}
  dans la version française) existent par défaut dans le
  \index{repertoire personnel@répertoire personnel}répertoire
  personnel des utilisateurs dans macOS ainsi que dans Windows.
