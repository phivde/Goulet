...
\begin{objectifs}
\item Savoir générer des nombres aléatoires uniformes avec la fonction
  \code{runif}.
\item Savoir générer des nombres aléatoires non uniformes provenant de
  lois de probabilité discrètes et continues.
\item Savoir générer des nombres aléatoires provenant d'une
  distribution discrète quelconque.
\item Savoir tirer profit de la nature vectorielle des fonctions de
  simulation de R.
\end{objectifs}
...
\section{Fonctions de simulation de variables aléatoires non
  uniformes}
...
\Writetofile{reponses}{\protect\section*{Chapitre \protect\ref{rng}}}
