...
d'abstraction que nous avons appelée l'\emph{arithmétique vectorielle}
(\autoref{sec:premiers:vecteurs:arithmetique}). Pour les cas où
l'arithmétique vectorielle ne saurait s'appliquer,
...
La fonction \Icode{apply} s'utilise autant avec les matrices qu'avec
les tableaux. Sa syntaxe complète est la suivante:
\begin{Schunk}
\begin{Verbatim}
apply(X, MARGIN, FUN, ...)
\end{Verbatim}
...
La fonction \Icode{outer} calcule le \index{produit!extérieur}produit
extérieur entre deux vecteurs, c'est à dire le résultat de
\code{FUN(X[i], Y[j])} pour toutes les valeurs des indices \code{i} et
\code{j}. La syntaxe de la fonction est la suivante:
\begin{Schunk}
\begin{Verbatim}
outer(X, Y, FUN, ...)
\end{Verbatim}
