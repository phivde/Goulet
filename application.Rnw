...
\begin{exercice}
  Nous pouvons généraliser la définition de moyenne pondérée de
  l'\autoref{exercice:application:moyennes} aux matrices et aux
  tableaux de données.
...
\end{Schunk}
  ...
  \item Nous vous donnons l'exemple suivant d'utilisation de la
