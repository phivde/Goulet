...
\tipbox{Truc mnémotechnique: la ou les dimensions figurant dans
  ...
  \fonction{apply}. En d'autres termes, les dimensions du résultat de
...
\Icode{lapply}. Sa syntaxe est similaire à celle de \icode{apply},
l'argument \code{MARGIN} en moins:
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
lapply(X, \code{FUN}, ...)
\end{Verbatim}
...
La fonction \fonction{lapply} applique une fonction \code{FUN} à
...
Impossible, par conséquent, de
passer directement la liste à la fonction \icode{sort} en espérant
qu'elle effectuera le tri sur chaque élément de la liste. L'analyste
se résoud à écrire une fonction itérative de tri de chaque élément
d'une liste.

Une liste \code{x} contient les âges des employés de trois entreprises.
<<echo=FALSE>>=
set.seed(1)
x <- lapply(c(6, 12, 7), sample, x = 20:65, replace = TRUE)
@
<<echo=TRUE>>=
x
@

Un analyste souhaite trier en ordre croissant chacun des vecteurs de
la liste. Or, il n'y a pas d'arithmétique vectorielle qui tienne pour
les listes. Impossible, par conséquent, de passer directement la liste
à la fonction \icode{sort} en espérant qu'elle effectuera le tri sur
chaque élément de la liste. L'analyste se résoud à écrire une fonction
itérative de tri de chaque élément d'une liste.
<<echo=TRUE>>=
lsort <- function(x)
{
