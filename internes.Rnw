...
exhaustive, surtout étant donné l'évolution rapide du langage. Un des
meilleurs endroits pour découvrir de nouvelles fonctions demeure la
section \texttt{See Also} des rubriques d'aide, qui offre des
hyperliens vers des fonctions apparentées au sujet de la rubrique.
...
nous consacrons une section de ce chapitre à la mise en place d'une
bibliothèque personnelle et à l'installation de paquetages.

\section{Opérateurs}
\label{sec:internes:operateurs}

Nous avons déjà présenté au \autoref{tab:bases:operateurs} à la
...
source, nous n'illustrons ici que les opérateurs de modulo et de
division entière.
...
Pour chaque fonction présentée dans cette section, nous fournissons un
ou deux exemples d'utilisation. Ces exemples sont loin de couvrir
toutes les utilisations possibles d'une fonction. La
\autoref{sec:internes:exemples} fournit des exemples additionnels. Nous
recommandons de consulter également les rubriques d'aide des fonctions
...
@
\item[\Icode{rep.int}] répétition de vecteurs uniquement (plus rapide
  que \code{rep})
<<echo=TRUE>>=
rep.int(2, 10)
@
...
ajouter des paquetages au système de base. Nous recommandons fortement
...
\tipbox{Consultez la rubriques d'aide de \icode{Startup} pour les
  ...
  configuration et celles de \icode{library} et de \icode{.libPaths}
...
\autoref{sec:internes:paquetages:utilisation}. R recherche le paquetage
