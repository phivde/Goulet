...
\item[\Icode{sum}] \index{somme}somme des éléments d'un vecteur
<<echo=TRUE>>=
sum(x)
@
\item[\Icode{prod}] \index{produit}produit des éléments d'un vecteur
<<echo=TRUE>>=
prod(x)
@
...
var(x)
@
\item[\Icode{min}] \index{minimum!valeur}minimum d'un vecteur
<<echo=TRUE>>=
min(x)
@
\item[\Icode{max}] \index{maximum!valeur}maximum d'un vecteur
<<echo=TRUE>>=
max(x)
@
...
cumprod(x)
@
\item[\Icode{cummin}] \index{minimum!cumulatif}minimum
<<echo=TRUE>>=
cummin(x)
@
