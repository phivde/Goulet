...
\item[\ess{C-c C-c}] évaluer dans le processus R la région
...
\begin{ttscript}{C-x C-w}
  \raggedright
\item[\ess{C-c C-z}] déplacer le curseur vers le processus R
...
\item[\ess{C-c C-z}] déplacer le curseur vers le fichier de script
...
\item Composer le code. Lors de cette étape, on se déplacera souvent
  ...
  \begin{quote}
    ...
    \item[\ess{C-RET}] pour exécuter une ligne du fichier de script ou la
      région sélectionnée;
    \item[\ess{C-c C-c}] pour exécuter un paragraphe du fichier de
      script;
    \item[\ess{C-c C-z}] pour se déplacer entre le fichier de script et
