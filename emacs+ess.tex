%%% Copyright (C) 2012-2022 Vincent Goulet
%%%
...
Utilisateur invétéré de GNU~Emacs, je prépare des distributions pour
\link{https://vigou3.github.io/emacs-modified-windows/}{Windows} et
pour \link{https://vigou3.github.io/emacs-modified-macos/}{macOS} qui
intègrent le mode ESS.
...
\begin{enumerate}
\item tout au haut de la fenêtre (ou de l'écran sous macOS) se trouve
  l'habituelle barre des menus dont le contenu change selon le mode dans
...
\begin{figure}
  ...
  \caption[Fenêtre de GNU~Emacs sous macOS]{Fenêtre de GNU~Emacs et
    ...
    La barre des menus se trouve au haut de l'écran dans macOS et à
    l'intérieur de la fenêtre de Emacs sous Windows et Linux.}
...
\begin{figure}[t]
  ...
  \begin{macosblock}
    ...
    touche \code{Meta} à \code{Commande} (\cmdkey). Cela bloque alors
...
Emacs compte une pléthore de commandes. Je me contenterai de
...
\macosbox{Plusieurs des raccourcis-clavier de Emacs composés avec la
  touche \code{Contrôle} (\ctlkey) sont valides sous
  \index{macOS}macOS. Par exemple, \ctlkey\,\code{A} et
  \ctlkey\,\code{E} déplacent le curseur au début et à la fin de la
  ligne dans les champs texte.}

\subsection{Annuler, rechercher, remplacer}
\label{sec:emacs+ess:commandes:annuler}

\begin{ttscript}{C-x C-w}
\item[\code{C-\_}] annuler (pratiquement illimité); aussi
  \code{C-x u} (\code{undo})\index{Emacs!annuler}.
\end{ttscript}
