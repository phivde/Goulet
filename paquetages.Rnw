...
Avant toute chose, vous devez prévoir un emplacement dans votre
système de fichiers pour accueillir la bibliothèque personnelle. Sous
macOS, je recommande d'utiliser \code{\string~/Library/R/library}.
Pour les autres systèmes d'exploitation, \code{\string~/R/library}
devrait convenir. Ces emplacements n'existent probablement pas dans
votre système de fichiers. Vous devez donc créer le répertoire
\code{R} et, à l'intérieur de celui-ci, le répertoire \code{library}.
(Dans ce qui précède, «\code{\string~}» représente le dossier de
départ tel que mentionné à la \autoref{sec:informatique:os:unix}.)

\osxbox{Le dossier \code{\string~/Library} est caché dans le Finder.
  Pour y accéder, cliquez sur le menu \code{Aller} en appuyant
  simultanément sur la touche Option (\optkey), puis sélectionnez
  \code{Bibliothèque}.}

Au démarrage, R lit le fichier de configuration
\code{\string~/.Renviron}, s'il existe. C'est dans ce fichier que vous
indiquez à R l'existence d'une bibliothèque personnelle de même que
l'endroit où elle se trouve dans le système de fichiers. À l'aide d'un
éditeur de texte, vous devez donc ouvrir\footnote{Pour afficher les
  fichiers débutant par un point dans la boite de dialogue d'ouverture
  de fichier d'une application comme RStudio sous macOS, appuyez sur
  \shiftkey\cmdkey\code{.} (\shiftkey\cmdkey~«point»).} %
ou créer\footnote{Pour créer le fichier dans RStudio, sélectionnez
  \code{File|New File|Text File}.} %
le fichier \code{\string~/.Renviron} et y ajouter la ligne ci-dessous
en adaptant, au besoin, le chemin d'accès pour celui du répertoire
créé précédemment. Utilisez la barre oblique avant («\code{/}») pour
...
\gotorbox{Le fichier de script \code{\scriptfilename} reproduit à la
  \autoref{sec:donnees:exemples} propose, aux lignes
  \reflines{paquetages:library}, une procédure alternative --- et sans
  doute plus simple --- pour créer une bibliothèque personnelle
...
##-!- dir.create("~/Library/R/library", recursive = TRUE)
...
##-!-            con = "~/.Renviron")  `\labelline{paquetages:library:fin}`
