...
C'est le principal rôle de la fonction \Icode{library}.
\begin{Schunk}
...
\item Inscrire l'emplacement de la bibliothèque dans le fichier de
  ...
  \item \textbf{macOS}
    \begin{Schunk}
\begin{Verbatim}
$ echo 'R_LIBS_USER="~/Library/R/library"' >> ~/.Renviron
\end{Verbatim}
    \end{Schunk}
  \item \textbf{autres systèmes d'exploitation}
    \begin{Schunk}
...
\end{Verbatim}
    \end{Schunk}
  \end{trivlist}
  \warningbox{Les commandes ci-dessus utilisent des guillemets simples
    «\code{'}» et doubles «\code{"}». Veillez à respecter leur nature
    et leur position.}
\item Redémarrer R et vérifier que le chemin vers la bibliothèque
  personnelle apparait dans les résultats de la commande
...
## Ensuite, la commande 'cat' permet d'écrire directement dans un
## fichier. Assurez-vous que le chemin d'accès correspond à celui
## utilisé ci-dessus.
##
## Comme dans les instructions du chapitre, les commandes ci-dessous
## ajoutent du contenu à un fichier si celui-ci existe déjà. Évitez
## donc d'évaluer les expressions plusieurs fois.
##
## Si vous êtes sous macOS, utilisez la commande suivante.
