...
langages de programmation interprétés comme R consiste à exécuter
...
Remarquez comment j'ai évité d'évaluer les lignes 1--2 et 20--21 de la
\autoref{fig:debogage:rgamma_ar}. J'ai également placé le nombre
aléatoire issu de l'expression \code{runif(1)} dans la clause
\code{if} dans un objet \code{u} afin de mieux pouvoir valider le
test. Au final, la technique permet de détecter le problème de
compteur mal initialisé.
...
\tipbox{Une fois la séance de débogage terminée, supprimez de l'espace
  de travail les objets que vous avez créés au cours de celle-ci et
  exécutez de nouveau votre fonction. Cela vous prémunira contre une
  fonction qui se serait valide seulement lorsque certains objets
  existent dans l'espace de travail grâce à la magie de la portée
  lexicale de R.}
...
- traceback
- browser; exemple de ligne de commande
- liens vers procédures dans RStudio et avec ESS
