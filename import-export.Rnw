...
sections \ref{sec:import-export:scan} et
\ref{sec:import-export:read.table} pour exporter ses données vers
d'autres systèmes. Les exemples de la présente section permettent
d'ailleurs de recréer les fichiers des figures
\ref{fig:import-export:scan} et \autoref{fig:import-export:read.table}
...
\gotorbox{Des exemples interactifs vous permettrons de mieux
  ...
  \autoref{sec:import-export:exemples}.}
