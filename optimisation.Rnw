...
<<echo=FALSE>>=
### Échantillon aléatoire d'une Gamma(5, 2) pour avoir les mêmes
### chiffres dans S-Plus et R.
x <- c(2.2557923, 2.6291918, 2.1579953, 5.2925777, 0.8625360,
       0.6744605, 1.5091443, 1.0829637, 2.5340812, 1.9135480)
@
...
commune avec le package \texttt{MASS}\index{package!MASS@\texttt{MASS}}.

Le package \texttt{MASS} \citep{MASS} contient plusieurs fonctions
aussi utiles que de grande qualité.  Les auteurs de ces fonctions
contribuent activement au développement de \textsf{R} et de S-Plus et,
comme il est mentionné au chapitre~\ref{presentation}, leurs livres
sur le langage S \citep{Sprogramming,MASS} constituent des références
de choix.

Le package \texttt{MASS} est distribué avec S-Plus (depuis au moins la
version 6.1) comme avec \textsf{R}.  On peut aussi le télécharger
...
Ce qui suit est une description des fonctions d'optimisation
disponibles dans S-Plus et \textsf{R}. Un exemple illustre la plupart
d'entre elles.
...
\end{ex}
\begin{sol}
  La loi bêta étant définie dans $[0, 1]$, on sait que l'extremum se
  trouve dans cet intervalle. On utilise l'argument \code{...} de
  \texttt{optimize} pour passer les paramètres de la loi bêta à
  \texttt{dbeta()}.
<<echo=TRUE>>=
...
somme\index{minimum!d'une somme}. C'est une des principales fonctions
...
l'usage de formules\index{formule} et de \emph{data frames}\index{data
  frame} (voir les sections \ref{regression:formules} et
\ref{bases:dataframes}, respectivement).
\begin{ex}
...
Minimisation d'une fonction non linéaire avec des bornes inférieure
et/ou supérieure pour les paramètres. (Disponible dans \textsf{R}
...
Outil d'optimisation tout usage, la fonction
\Fonction{optim}\index{fonction!optimisation} est souvent utilisée par
d'autres fonctions.  Elle permet, selon l'algorithme choisi, de fixer
des seuils minimum et/ou maximum aux paramètres à optimiser.  Dans
S-Plus, il faut charger la section
\texttt{MASS}\index{package!MASS@\texttt{MASS}} de la bibliothèque.
\begin{ex}
  Répéter l'exemple \ref{ex:optimisation:ms} à l'aide de
  \code{optim}.
\end{ex}
...
\begin{rems}
  \begin{enumerate}
  \item L'option \R \code{log = TRUE} de la fonction \fonction{dgamma}
    (et de toutes les autres fonctions de densité) permet de calculer
    plus précisément le logarithme de la densité.  Cette option n'est
    disponible que dans \textsf{R}.
  \item L'estimation par le maximum de
    vraisemblance\index{vraisemblance} est de beaucoup simplifiée par
    l'utilisation de la fonction \fonction{fitdistr} du package
