...
\begin{enumerate}
\item Dans la catégorie des options de configuration générales,
  désactiver l'option \code{Restore .RData into workspace at startup}
  et régler l'option \code{Save workspace to .RData on exit} à
  \code{Never}, tel qu'illustré à la
  \autoref{fig:rstudio:rstudio-startup-and-exit}. Avec ces réglages,
  RStudio démarre toujours avec une séance de travail vierge et ne
  sauvegarde pas l'espace de travail de R à la fermeture de
  l'application.
