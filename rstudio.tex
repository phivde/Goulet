...
\item[\code{Alt+-} $\bullet$ \code{\optkey\,-}] insérer le symbole
  d'affectation \verb*| <- | (consulter l'encadré de la
  \autopageref{fig:rstudio:affectation} si cela ne fonctionne pas sur
  un Mac avec un clavier canadien-français).
...
\begin{figure}[t]
  ...
  \begin{titled-frame}{Symbole d'affectation et
    ...
    \emph{Le problème ci-dessous semble réglé depuis la version 1.3 de
      RStudio. Je laisse l'information ici au cas où elle pourrait
      toujours s'avérer utile.}

    Le très pratique raccourci-clavier \code{\optkey\,-} servant à
    insérer le symbole d'affectation dans RStudio pourrait ne pas
    fonctionner correctement sur les Mac munis d'un clavier
    canadien-français, et ce, à la console R, dans les fichiers de
    script ou les deux.

    La combinaison de touches \code{\optkey\,-} insère normalement le
    symbole «\textbar». Il s'agit en fait d'un doublon puisque ce
    symbole est principalement associé à la touche \code{\optkey\,/}.

    Il est possible d'associer un autre raccourci-clavier au symbole
    d'affectation. Je recommande d'utiliser la combinaison
    \code{\optkey\,=}. D'une part, c'est un choix mnémonique
    raisonnable (davantage même que \code{\optkey\,-}) et, d'autre
    part, le caractère lié normalement à cette touche («\code{¬}»)
    n'est d'aucune utilité dans R.
    ...
    \item Dans le champ de recherche en haut, taper les lettres de
      «assignment» jusqu'à ce que la commande \code{Insert Assignment
        Operator} apparaisse.
    \item Cliquer sur la case du raccourci-clavier (qui devrait être
      \code{Alt+-} par défaut) pour l'activer. Appuyer sur
      \code{\optkey\,=} pour utiliser cette combinaison de touches
      comme raccourci.
...
\item Toujours dans la catégorie \code{Code}, mais dans la
  sous-catégorie \code{Saving}, cocher l'option \code{Strip trailing
    horizontal whitespace when saving} et régler l'option
  \code{Default text encoding} à \code{UTF-8}, tel qu'illustré à la
