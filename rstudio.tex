...
d'autre part, macOS, différent légèrement. Nous fournissons ci-dessous
...
\item[\code{Alt+-} $\bullet$ \code{\optkey\,-}] insérer le
  symbole d'assignation \verb*| <- | (pour les Mac munis d'un clavier
  canadien français, consulter l'encadré de la
  \autopageref{fig:rstudio:assignation})
...
\begin{figure}[t]
  \begin{emphbox}{\mdseries Symbole d'assignation et
    ...
    clavier associé au symbole d'assignation.
    ...
    \item Une nouvelle fenêtre liste toutes les commandes disponibles
      et les raccoucis clavier associés. Dans le champ de recherche en
    ...
    \item Cliquer sur la case du raccouci clavier (qui devrait être
      ...
      Pour une raison quelconque, c'est «Alt+\textbackslash» qui
      apparait alors comme raccourci.
...
Nous recommandons de régler l'option \code{Save workspace to .RData on
  exit} à \code{Never} dans les options de configuration générales,
