...
Où placer les \index{tests unitaires}tests unitaires? Pour les projets
simples, vous pouvez les insérer dans le fichier de script contenant
la définition de votre fonction. Cette stratégie convient toutefois
mal aux grands projets ou aux tests longs à exécuter. Une meilleure
pratique consiste à placer les tests dans des fichiers de script
...
\index{tests unitaires}tests unitaires, par exemple \pkg{tinytest}
\citep{Rpackage:tinytest}, \pkg{testthat} \citep{Rpackage:testthat} ou
\pkg{RUnit} \citep{Rpackage:RUnit}, pour n'en nommer que trois. Pour
ma part, j'estime que les fonctions ci-dessous de la distribution R de
base s'avèrent généralement suffisantes pour la mise en place d'une
