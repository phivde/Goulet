...
\end{Verbatim}
  ...
  sans la documentation appropriée. Et même là, la documentation
  pourrait ne plus être à jour ou se trouver loin de l'endroit où les
  constantes apparaissent dans le code.

  Pour clarifier la \index{semantique@sémantique}sémantique des
  nombres magiques et éviter du même coup les périls de la répétition,
  il vous suffit d'affecter les constantes à des variables --- portant
  des noms significatifs! --- puis d'utiliser ces variables dans les
  opérations arithmétiques. Par convention, vous devriez écrire les
  noms de ces variables en majuscules.
