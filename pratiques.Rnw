%%% Copyright (C) 2019-2023 Vincent Goulet
%%%
...
\item Interpréter les résultats de tests unitaires.
\end{objectifs}

De manière générale, le développement et la maintenance de code
informatique reposent sur la contribution de plusieurs personnes. En
...
\tipbox{Je me suis efforcé de respecter les bonnes pratiques
  présentées dans ce chapitre dans les fichiers de script fournis avec
  l'ouvrage. Prêtez donc attention à la manière dont le code qui vous
  est fourni est construit et présenté.}
...
style en programmation; \citet{Hunt:pragmatic:1999} offrent quasiment
un traité sur le sujet; \citet{Martin:CleanCode:2009} est la bible des
ingénieurs logiciels\footnote{%
  Les disciples appellent d'ailleurs affectueusement l'auteur
  \emph{Uncle Bob}.}. %
Je ne saurais être aussi exhaustif que ces auteurs établis. Néanmoins,
je vous incite à porter une attention particulière aux quelques points
de style livrés en vrac ci-dessous.

\begin{figure}[t]
  \centering
  \begin{minipage}{0.9\linewidth}
    \setkeys{Gin}{width=\textwidth}
    \includegraphics{images/code_quality} \\
    \footnotesize\sffamily%
    Tiré de \href{https://xkcd.com/1513/}{XKCD.com}
  \end{minipage}
\end{figure}
...
\end{Verbatim}
  ...
  usuels qu'il devient contreproductif de leur préférer des versions plus
  ...
  divers styles s'affrontent, notamment:
\begin{Verbatim}
variable.name
variable_name
variableName
VariableName
\end{Verbatim}
  Assurez-vous simplement de suivre le standard en vigueur dans votre
  équipe de travail, et, par-dessus tout, soyez constant. Ma
...
\end{Verbatim}
  \end{Schunk}
  %
\item Évitez de vous répéter. Évitez de vous répéter. Si vous utilisez
  une expression ou un groupe d'expressions plus d'une fois dans un
  programme et que vous devez un jour y changer quelque chose, il vous
  faudra penser à effectuer le changement partout. Tôt ou tard, vous
  allez oublier un cas et introduire un bogue; c'est une règle de
  l'univers, ne pensez pas y échapper. Comment éviter les périls de la
  répétition?
  \begin{enumerate}
  \item Révisez votre algorithme pour éliminer la répétition. Voici un
    exemple que je rencontre parfois dans des travaux d'étudiants: une
    expression présente à la fois dans la conséquence (\code{if}) et
    dans l'alternative (\code{else}) d'une structure conditionnelle.
    Il suffit de sortir l'expression en question de la structure pour
    éviter la répétition.
  \item Placez plutôt le code dans une fonction auxiliaire. Vous
    pourrez ensuite utiliser celle-ci autant de fois que vous voudrez
    en sachant que toute modification que vous y apporterez se
    répercutera automatiquement partout.
  \item Ne prenez pas de raccourcis --- ils sont connus pour faire de
    longs délais.\footnote{%
      «\emph{Les raccourcis font de longs délais}, argumenta Pippin.»
      --- \emph{Le Seigneur des anneaux, La Communauté de l'anneau},
      chapitre~4.} %
    Une répétition insérée volontairement pour espérer sauver du temps
    risque fort de vous causer des soucis plus tard. Prenez le temps
    de l'éviter ou de l'éliminer.
  \end{enumerate}
  %
\item Connaitre sur le bout des doigts la priorité des opérateurs du
  ...
  \end{enumerate}
  %
\item Évitez les \emph{nombres magiques}. Un \Index{nombre
    magique}nombre magique est une constante numérique non nommée ou
  mal documentée dans le code source d'un programme
  \citep{Wikipedia:Nombre_magique}. Les nombres magiques rendent les
  programmes difficiles à lire, à interpréter et à déboguer. Voici une
  expression qui en est truffée:
  \begin{Schunk}
\begin{Verbatim}
cost <- 4.79 + 3 * floor(x/900)
\end{Verbatim}
  \end{Schunk}
  Que représentent au juste les trois constantes? Difficile de savoir
  sans la documentation appropriée. Dès qu'une constante dans une
  ligne de code nécessite de documenter sa signification, vous avez
  affaire à un nombre magique. Cependant, la documentation n'est pas
  une panacée: elle pourrait ne plus être à jour ou se trouver loin de
  l'endroit où les constantes apparaissent dans le code.

  Afin de déjouer à la fois les périls des nombres magiques et de la
  répétition, affectez les constantes à des variables, puis employez
  ces variables dans les opérations arithmétiques. Utilisez des noms
  qui expliquent le rôle --- ou la
  \index{semantique@sémantique}sémantique --- des nombres magiques,
  pas leur valeur numérique. Par convention, vous devriez écrire les
  noms des variables contenant des nombres magiques en majuscules.

  Le code ci-dessous est fonctionnellement équivalent à l'expression
  précédente, mais vous pouvez sans doute maintenant décoder sa
  signification même sans documentation:
  \begin{Schunk}
\begin{Verbatim}
BASE <- 4.79
EXTRA <- 3
...
\end{Verbatim}
  \end{Schunk}
  Les nombres $-1$, $0$ et $1$ ne sont généralement pas considérés
  comme des nombres magiques, tout comme $2$ (souvent utilisé pour
  déterminer si un nombre est pair) et $100$ (beaucoup utilisé dans la
  conversion des pourcentages). Selon le contexte, il peut convenir
  d'omettre d'autres valeurs.
  %
...
\end{Verbatim}
  ...
  Ici encore, évitez de pousser la logique trop loin. Si une boucle
...
\videobox{\link{https://youtu.be/3uD67Z5Rd88}{Le guide de style R du
    \emph{geek} de placard}}{%
  La vidéo «\link{https://youtu.be/3uD67Z5Rd88}{Le guide de style R du
    \emph{geek} de placard}» propose, sur un ton humoristique, trois
  autres éléments de style à surveiller en programmation R.}
...
\item de part et d'autre de tous les opérateurs\footnote{%
    Sauf peut-être la division: je préfère \code{(x + y)/z} à \code{(x
      + y) / z}.};
\item après les virgules (mais jamais avant);
...
d'exemples, l'éditeur GNU~Emacs\index{Emacs} reconnait et supporte au
...
\end{Verbatim}
\end{Schunk}
Cette pratique s'avère plus souvent qu'autrement contreproductive: non
seulement vous force-t-elle à passer du temps à rédiger des
commentaires sans véritable utilité, mais elle surcharge également le
code, le rendant de ce fait plus difficile à lire.
...
\item Définissez ce que fait chaque fonction, la nature de ses
  arguments et la valeur retournée. Fournissez aussi des exemples
  d'utilisation utiles et pertinents. Si une fonction R fait partie
  d'un paquetage, vous devrez nécessairement placer ces informations
  dans l'obligatoire rubrique d'aide de la fonction. Autrement, placez
  ces informations en commentaires avant la définition de la fonction.
...
\item Ne documentez pas du mauvais code, réécrivez-le. Si les
  commentaires sont beaucoup plus longs que le code auquel ils se
...
documentation contenant au minimum:
\begin{enumerate}
...
\item Un ou plusieurs exemples d'utilisation de la fonction, selon le
  niveau de complexité de celle-ci.
\end{enumerate}
Il s'agit de la structure générale des rubriques d'aide de R. La
\autoref{fig:pratiques:doc} fournit un exemple pour une fonction
simple.

\tipbox{Utilisez le fichier \code{gabarit-documentation-fonction.R}
  livré avec cet ouvrage pour insérer la structure de base de la
  documentation d'une fonction dans vos fichiers de script.}
...
Dans R, le symbole carré «\code{\#}» --- ou numéro --- marque le début
d'un commentaire, et ce, peu importe où le symbole se trouve sur la
ligne. Il est possible de combiner les symboles «\code{\#}» pour
former une hiérarchie dans les commentaires ou pour délimiter
différentes sections d'un fichier de script. J'ai utilisé dans cet
ouvrage la
...
L'éditeur \index{RStudio}RStudio, pour sa part, utilise par défaut les
...
les commentaires de premier niveau débutent par un seul carré,
«\code{\#}»; les commentaires de deuxième niveau débutent par deux
carrés, «\code{\#\#}», etc.

\gotorbox{Le fichier de script \code{\scriptfilename} reproduit à la
  \autoref{sec:pratiques:exemples} contient, aux lignes
  \reflines{pratiques:style}, un long exemple d'amélioration du style,
  de la présentation et de la documentation d'une fonction.}
...
sqrt <- function(x)
{
    ...
    while (abs(y^2 - x) >= ERROR_MAX)
        y <- improve(y, x)

    ## Retourner la valeur tel que y^2 - x < 0.001.
    y
}                          #-*- `\labelline{pratiques:style:fin}`
@

<<script, results=hide>>=
###
### STYLE, PRÉSENTATION, COMMENTAIRES ET DOCUMENTATION `\labelline{pratiques:style}`
###
...
##
## Il reste tout de même quatre choses à améliorer.
##
## 1. Nous pouvons traiter séparément le cas trivial où
##    l'argument est égal à 0. Il n'y a aucun calcul à faire
##    dans ce cas et la fonction peut directement retourner la
##    réponse 0.
## 2. L'expression logique dans la clause 'while' qui utilise
##    une formulation «n'est pas plus petite que 0.001» est
##    inutilement compliquée. De plus, elle repose sur la
##    priorité des opérations: la négation logique a-t-elle
...
## 3. Ce 0.001, justement, est un nombre magique dans notre
##    fonction. Affectons plutôt la valeur à une variable qui
##    clarifiera la sémantique du nombre.
## 4. Il y a ce 'return' à la dernière ligne de la fonction.
##    Ça, c'est un gros non en R.
##
## Améliorons le style.
sqrt <- function(x)
{
...
\label{sec:pratiques:tests}

Lorsque vous rédigez un programme, vous savez ce que celui-ci doit
accomplir.\footnote{%
  Si ce n'est pas le cas, retournez soit à la table à dessin, soit
  consulter la personne qui vous a confié le mandat de
  programmation!} %
Il tombe sous le sens qu'une fois la programmation complétée, vous
devriez tester que le programme effectue le traitement adéquat, non?
Et pourtant\dots\ Tout bogue dans un logiciel signale un test non ou
mal exécuté.

Cette section explique brièvement comment mettre en place une
infrastructure de tests pour du code R. Comme
\citet{Hunt:pragmatic:1999} le soulignent, effectuer des tests est
davantage une question culturelle que technique, en ce sens que le
plus difficile n'est pas tant d'apprendre à tester le code dans un
langage donné que de \emph{prendre l'habitude} de tester son code.
Efforcez-vous donc d'intégrer les tests dès maintenant dans votre
routine de programmation.

Compte tenu de la portée de cet ouvrage, nous allons limiter la
discussion aux \index{tests unitaires}\emph{tests unitaires}
(\emph{unit tests}). Ceux-ci permettent de vérifier le bon
fonctionnement d'une partie d'un grand programme. Dans notre contexte,
un test unitaire porte sur une fonction R.

Au moment d'écrire une fonction, vous devriez avoir en main --- ou au
moins en tête --- sa
\index{specification@spécification}\emph{spécification}. Celle-ci
contient normalement:
\begin{enumerate}
\item La \index{signature}signature de la fonction;
\item La liste des arguments de la fonction;
\item La valeur retournée par la fonction;
\item Une description détaillée de ce que doit réaliser la fonction, y
  compris les cas limites et la gestion des erreurs, le cas échéant.
\end{enumerate}
Vous aurez reconnu plusieurs des éléments de la documentation d'une
fonction mentionnés à la \autoref{sec:pratiques:commentaires}. En
effet, une large part de la documentation s'écrit toute seule à partir
de la spécification.

Les \index{tests unitaires}tests unitaires confrontent la réalisation
d'une fonction à sa \index{specification@spécification}spécification.
Les bons tests ne valident pas uniquement les cas «normaux», mais
également les cas limites et les erreurs, lorsque la fonction se doit
de les traiter. Si la fonction est vectorielle, assurez-vous aussi de
la tester tant avec un vecteur de longueur $1$ qu'avec un vecteur de
longueur supérieure à $1$.

\tipbox{Durant le codage d'une fonction, vous effectuez sans doute des
  petits tests pour valider votre fonction. Conservez ces bouts de
  code! Ils formeront la base de vos tests unitaires.}

Prenons l'exemple d'une fonction simple qui calcule la racine carrée
d'un seul nombre. Vous devriez alors écrire des tests unitaires pour
les cas suivants:
\begin{enumerate}
\item La fonction retourne une erreur (ou \code{NaN}) si l'argument
  est strictement négatif;
\item La fonction retourne un véritable $0$ (et non une approximation)
  pour le cas limite où l'argument est nul;
\item le carré de la valeur retournée par la fonction est
  approximativement égal à l'argument lorsque celui-ci est strictement
  supérieur à $0$.
\end{enumerate}

Concrètement, les tests consistent en des expressions qui retournent
une erreur uniquement lorsque la réalisation d'une fonction n'est pas
conforme à la spécification. Si l'évaluation de l'ensemble des
expressions se déroule sans erreur, vous savez que tous les tests sont
réussis.

Reste à savoir quelles fonctions utiliser pour rédiger les
\index{tests unitaires}tests unitaires. Plusieurs
\index{paquetage}paquetages externes\footnote{%
  L'utilisation de paquetages fait l'objet du
  \autoref{chap:paquetages}.} %
proposent des outils pour enjoliver leur écriture ou leurs résultats,
par exemple \pkg{tinytest} \citep{Rpackage:tinytest}, \pkg{testthat}
\citep{Rpackage:testthat} ou \pkg{RUnit} \citep{Rpackage:RUnit}, pour
n'en nommer que trois. Pour ma part, j'estime que les fonctions
ci-dessous de la distribution R de base s'avèrent généralement
suffisantes pour la mise en place d'une infrastructure de tests.

\begin{ttscript}{assertWarning}
\item[\Icode{source}] évaluation séquentielle d'un fichier de script.
<<echo=TRUE, eval=FALSE>>=
source("tests.R")
@
\item[\Icode{stopifnot}] erreur dès qu'une expression n'est pas vraie.
\begin{Schunk}
\begin{Sinput}
> stopifnot(1 == 1, 1 < 2)
> stopifnot(3 < 2)
...
\end{Schunk}
%
\item[\Icode{assertError}] validation qu'une expression retourne une
  erreur.
\begin{Schunk}
\begin{Sinput}
...
\end{Schunk}
%
\item[\Icode{assertWarning}] validation qu'une expression retourne un
  avertissement.
\begin{Schunk}
\begin{Sinput}
...
\end{Schunk}
%
\item[\Icode{all.equal}] égalité entre deux nombres à une (petite)
  différence près.
<<echo=TRUE>>=
tan(pi/4) == 1
1 - tan(pi/4)
...
@
%
\item[\Icode{identical}] égalité exacte et complète entre deux objets.
<<echo=TRUE>>=
identical("abc", "abc")
...
\warningbox{Lorsqu'il s'agit de comparer des valeurs numériques
  résultant de calculs, vous devez absolument éviter d'utiliser
  l'opérateur «\icode{==}». Il est hors de la portée de cet ouvrage de
  discuter d'arithmétique en virgule flottante, mais consultez déjà
  l'\link{https://tinyurl.com/y2c46aug}{entrée 7.31} (la plus célèbre
  de toutes!) de la foire aux questions de R \citep{R-FAQ}.}

\tipbox{Deux objets vous apparaissent identiques et \icode{identical}
  persiste à dire que ce n'est pas le cas? La fonction \icode{str},
  qui révèle la structure interne des objets, pourrait vous aider à
  identifier les différences que R y voit.}

Terminons avec une question toute pratique: où placer les \index{tests
  unitaires}tests unitaires? Pour les projets simples, vous pouvez les
insérer dans le fichier de script contenant la définition de votre
fonction. Cette stratégie convient toutefois mal aux grands projets ou
aux tests longs à exécuter. Une meilleure pratique consiste à placer
les tests dans des fichiers de script dédiés. Si un jour vous
développez votre propre paquetage R, vous pourrez déposer les fichiers
dans un répertoire spécial justement nommé \code{tests}.

Un fichier de tests dédié doit pouvoir être évalué intégralement dans
une session R vierge. Par conséquent, assurez-vous d'inclure des
appels à \icode{source} (pour du code libre) ou à \icode{library}
(pour du code dans un paquetage) pour faire en sorte que les fonctions
testées existent dans l'espace de travail de la session R.

\gotorbox{Difficile de bien saisir comment concevoir un fichier de
  tests sans un exemple. Il s'en trouve un complet pour une fonction
  simple aux lignes \reflines{pratiques:tests} du fichier de script
  \code{\scriptfilename} reproduit à la section suivante.}

<<tests-sqrt>>=
###
### Tests unitaires de la fonction 'sqrt'
###

## Définir la fonction dans l'espace de travail.
source("sqrt.R")

## La fonction 'sqrt' calcule la racine carrée d'un nombre par
## la méthode des approximations successives de Newton.
##
## Tests unitaires:
##
## 1. Erreur pour un argument négatif.
## 2. Véritable 0 pour un argument nul.
## 3. Carré de la réponse égal à l'argument autrement; il faut
##    ajuster la tolérance en fonction de celle utilisée dans
##    la fonction.
...
<<script, results=hide>>=
###
### TESTS UNITAIRES  `\labelline{pratiques:tests}`
###
...
## ligne. Il s'agit de fournir à l'argument 'exprs' (qu'il
## faut nommer explicitement) un groupe d'expressions entre
## accolades «{ }» et séparées les unes des autres par un
## retour à la ligne.
stopifnot(exprs = {
    1 < 2
    2 == 2
...
## Nous pouvons aussi regrouper les deux derniers tests en un
## seul appel à 'stopifnot'.
stopifnot(exprs = {
    identical(0, sqrt(0))
    all.equal(3, sqrt(3)^2,
              tolerance = 0.001)
...
## tests dans des fichiers de script dédiés et intégralement
## évaluables dans une session R vierge.
##
## Le fichier de script 'tests-sqrt.R' distribué avec
## l'ouvrage reprend les tests ci-dessus. Le fichier a aussi
## recours à 'source' pour faire en sorte que la fonction
## 'sqrt' existe dans l'espace de travail de la session R.
##
## Vous pouvez ainsi ensuite évaluer les tests simplement
## en évaluant séquentiellement le fichier de tests.
...
rm("sqrt")                 #-*- `\labelline{pratiques:tests:fin}`
@
...
\begin{exercice}
  Rédiger des tests unitaires pour la fonction de
  l'\autoref{ex:pratiques:irr} à partir de la lecture du code de la
  fonction et des informations additionnelles suivantes:
  \begin{enumerate}[1.]
  \item La fonction prend en argument un vecteur de nombres réels
    positifs, négatifs ou nuls;
  \item Elle vérifie le nombre de changements de signe dans le vecteur
    après avoir éliminé les valeurs nulles;
  \item La réponse correcte correspondant au vecteur
    $(-10, -7, 12, 0, 11)$ est:
    \begin{center}
      $\nombre{0,128564783114}$;
    \end{center}
  \item La réponse correcte correspondant au vecteur
    $(-10, 12, -7, 0, 11)$ est:
    \begin{center}
      $\nombre{0,225982251102}$.
    \end{center}
  \end{enumerate}
  \begin{sol}
    Nous devons effectuer les vérifications suivantes:
    \begin{enumerate}[1.]
    \item erreur si tous les éléments du vecteur en argument sont
      positifs;
    \item erreur si tous les éléments du vecteur en argument sont
      négatifs;
    \item avertissement s'il y a plus d'un changement de signe;
    \item réponses adéquates pour les deux cas fournis dans l'énoncé.
    \end{enumerate}
...
stopifnot(exprs = {
    ...
    all.equal(0.225982251102, f(c(-10, 12, -7, 0, 11)))
})
@
  \begin{Schunk}
\begin{Verbatim}
