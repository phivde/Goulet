...
Les bons tests ne valident pas uniquement les cas triviaux, mais
...
Par exemple, pour une fonction simple qui calcule la racine carrée
d'un nombre, vous devriez écrire des tests unitaires pour les cas
suivants:
\begin{enumerate}
...
\item le carré de la valeur retournée par la fonction est
  ...
  supérieur à $0$;
\item si la fonction est vectorielle, elle effectue le traitement
  approprié pour chaque élément d'un vecteur de valeurs positives,
  négatives ou nulles.
\end{enumerate}
...
Où placer ces \index{tests unitaires}tests unitaires? Pour les petits
projets très simples, vous pouvez les insérer dans le fichier de
script contenant la définition de votre ou de vos fonctions. Cette
stratégie a toutefois toutes les chances de devenir laborieuse,
surtout si les tests sont longs à exécuter. Une meilleure pratique
consiste à placer les tests dans un ou plusieurs fichiers de script
dédiés. Si un jour vous développez votre propre paquetage R, vous
pourrez déposer les fichiers dans un répertoire spécial justement
nommé \code{tests}.

Plusieurs \index{paquetage}paquetages externes\footnote{%
  L'utilisation de paquetages fait l'objet du
  \autoref{chap:paquetages}.} %
proposent des outils pour enjoliver l'écriture ou les résultats des
tests unitaires, par exemple \pkg{tinytest} \citep{Rpackage:tinytest},
\pkg{testthat} \citep{Rpackage:testthat} ou \pkg{RUnit}
\citep{Rpackage:RUnit}, pour n'en nommer que trois. Pour ma part,
j'estime que les fonctions ci-dessous de la distribution R de base
s'avèrent généralement suffisantes pour la mise en place d'une
infrastructure de \index{tests unitaires}tests unitaires.
...
@
%
\item[\Icode{identical}] égalité \emph{exacte} entre deux objets, y
  compris leurs classes, leurs attributs, etc.
<<echo=TRUE>>=
identical("abc", "abc")
...
\tipbox{Deux objets vous apparaissent identiques et \icode{identical}
  ...
  vous révèlera la structure interne des objets, ce qui pourrait vous
  aider à identifier les différences que R y voit.}
...
\gotorbox{Difficile de bien saisir comment concevoir un fichier de
  ...
  \autoref{sec:pratiques:exemples}.}
