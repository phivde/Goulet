...
ingénieurs logiciels\footnote{%
  L'auteur est affectueusement appelé \emph{Uncle Bob} dans la
  discipline.}. %
...
@
  ...
  sans la documentation appropriée.

  Pour éviter le problème des nombres magiques, il suffit d'affecter
  les constantes à des variables --- portant des noms significatifs
  --- puis d'utiliser ces variables dans les opérations arithmétiques.
  Par convention, les noms de ces variables s'écrivent en majuscules.

  Le code suivant est fonctionnellement équivalent à l'expression
  ci-dessus, mais vous pouvez sans doute décoder sa signification même
  sans documentation:
<<echo=TRUE, eval=FALSE>>=
BASE <- 4.79
EXTRA <- 3
HALFHOUR <- 30 * 60
cost <- BASE + EXTRA * floor(x/HALFHOUR)
@
  La \index{semantique@sémantique}sémantique des nombres $4,79$, $3$
  et $900$ est maintenant claire: en supposant que \code{x} représente
  la durée d'utilisation d'un service en secondes, il s'agit, dans
  l'ordre, du coût de base du service, du montant additionnel par
  demi-heure et du facteur de conversion de la durée en heures.

  Les nombres $-1$, $0$ et $1$ ne sont généralement pas considérés
  comme des nombres magiques. Selon le contexte, il peut en aller de
  même pour $2$ et $100$ dans des expressions arithmétiques.
  %
