...
faire des simulations avec R à l'aide d'un exemple simple de nature
statistique.
...
simulation en R consiste à utiliser une boucle \fonction{for}. Avec
...
La figure \ref{fig:planification:boucle} présente un exemple de code
...
\ref{fig:planification:boucle} facilement à l'aide d'une seule
expression, il suffit de placer l'ensemble du code dans une fonction.
La fonction \code{simul1} de la figure \ref{fig:planification:simul1}
reprend le code de la figure \ref{fig:planification:boucle}, sans les
...
On le sait, les boucles sont inefficaces en R. Il est en général plus
efficace de déléguer les boucles aux fonctions \fonction{lapply} et
\fonction{sapply}, dont la syntaxe est
\begin{center}
  \code{lapply(x, FUN, ...)} \quad et \quad \code{sapply(x, FUN, ...)}.
\end{center}
Celles-ci appliquent la fonction \code{FUN} à tous les éléments de
la liste ou du vecteur \code{x} et retournent les résultats sous
...
simulations. La figure \ref{fig:planification:fun1} présente une première
version d'une telle fonction. On remarquera que l'argument \code{i} ne
joue aucun rôle dans la fonction. Voici un exemple d'utilisation pour
un petit nombre (4) de simulations:
...
\end{lstlisting}
  \end{framed}
  \caption{Définition de la fonction \code{fun1}}
  \label{fig:planification:fun1}
\end{figure}
...
\code{simul2}, dont le code se trouve à la figure
\ref{fig:planification:simul2}:
...
\end{lstlisting}
  \end{framed}
  \caption{Définition de la fonction \code{simul2}}
  \label{fig:planification:simul2}
\end{figure}
...
fonction R \fonction{replicate} permet toutefois de passer outre cette
contrainte. En effet, cette fonction exécute un nombre donné de fois
une expression quelconque. Les fonctions \code{fun2} et \code{simul3}
des figures \ref{fig:planification:fun2} et \ref{fig:planification:simul3},
...
\end{lstlisting}
  \end{framed}
  \caption{Définition de la fonction \code{fun2}}
  \label{fig:planification:fun2}
\end{figure}
...
\end{lstlisting}
  \end{framed}
  \caption{Définition de la fonction \code{simul3}}
  \label{fig:planification:simul3}
\end{figure}


\section{Comparaison des temps de calcul}
\label{planification:temps}

A-t-on gagné quoi que ce soit en termes de temps de calcul d'une
approche à l'autre? La fonction \fonction{system.time} permet de
mesurer le temps requis pour l'exécution d'une expression. Le premier
résultat de \fonction{system.time} est le temps CPU utilisé et le
troisième, le temps total écoulé. Sous Windows, les quatrième et
cinquième résultats sont \code{NA}.
...
de la section~\ref{planification:replicate} avec la fonction
\code{replicate} en R et que le code des fonctions
\code{fun2} et \code{simul3} est sauvegardé dans des fichiers
\code{fun2.R} et \code{simul3.R}, respectivement. Si l'on crée un
autre fichier, \code{go.R}, ne contenant que des expressions
...
exemple, le fichier \code{go.R} contiendrait les lignes suivantes:
\begin{verbatim}
source("fun2.R")
source("simul3.R")
simul3(10000, 100, 0)
...
Les utilisateurs plus avancés pourront vouloir exécuter R en lot
(\emph{batch}) pour accélérer le traitement d'une simulation. Dans ce
mode, aucune interface graphique n'est démarrée et tous les résultats
sont redirigés vers un fichier pour consultation ultérieure. Pour les
simulations demandant un long temps de calcul, c'est très pratique.

On exécute R en lot depuis la ligne de commande (Invite de commande
sous Windows, Terminal sous OS~X ou Linux). Une fois placé dans le
