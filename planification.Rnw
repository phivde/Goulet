...
figure~\ref{fig:planification:boucle} facilement à l'aide d'une seule
...
\fonction{sapply} (section~\ref{avance:lapply}). On rappelle que la
...
de liste (\fonction{lapply}) ou, lorsque c'est possible, de vecteur ou
de matrice (\fonction{sapply}). Il est important de noter que les
valeurs successives de \code{x} seront passées comme \emph{premier}
argument à la fonction \code{FUN}. Le cas échéant, les autres
arguments de \code{FUN} sont spécifiés dans le champ~`\argument{...}'.
...
simulations. La figure~\ref{fig:planification:lapply} présente une
...
figure~\ref{fig:planification:lapply}:
<<echo=TRUE>>=
simul2(10000, 100, 0)
@
...
fonction \fonction{replicate} (section \ref{avance:replicate}) permet
toutefois de passer outre cette contrainte. En effet, cette fonction
exécute un nombre donné de fois une expression quelconque.

Les fonctions \code{fun2} et \code{simul3} de la
figure~\ref{fig:planification:replicate} sont des versions légèrement
modifiées de \code{fun1} et \code{simul2} pour utilisation avec
\fonction{replicate}. On a alors
<<echo=TRUE>>=
simul3(10000, 100, 0)
@
...
section~\ref{planification:replicate} avec la fonction
...
\code{go.R}, ne contenant que des expressions \fonction{source} pour
