...
\item Remplacer le nom d'une commande par un autre lorsque la commande
  ...
  \cmd{\cmd}\oarg{option}\marg{arg}. Pour remplacer toutes les
  ...
  \oarg{option} ainsi que de l'argument \marg{arg} qui change d'une
...
livrés avec les interpréteurs de commande Bash de \index{MSYS2}MSYS2
ou de \index{Git~Bash}Git~Bash (\autoref{sec:informatique:cli}).
...
de transfert de données et de redirection de \index{Unix}Unix. Les
programmes Unix en ligne de commande recoivent leurs données d'une
\Index{entree standard@entrée standard}\emph{entrée standard}
...
\importantbox{L'espace horizontal étant compté dans ce document, il
  ...
  «\textbackslash» de Bash à la fin des lignes. Vous pouvez ainsi copier les
