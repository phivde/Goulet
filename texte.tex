...
du programme \icode{cat} vers \icode{awk}, alors que la seconde, plus
...
la dernière version du document. L'idée consiste à afficher les lignes
du fichier à partir de la première section marquée par '\#\#\#' après
'\#\# Historique' et d'arrêter lorsqu'une autre section marquée par
'\#\#\#' est atteinte. Tout ceci requiert deux valeurs de la
...
\tipbox{Les opérations de composition du présent document, de
  publication dans GitHub et de mise à jour de la page web sont
  entièrement automatisées à l'aide de l'outil
  \link{https://fr.wikipedia.org/wiki/Make}{\code{make}} standard sur
  les systèmes Unix. Vous pouvez retrouver une variante de la commande
  \code{awk} ci-dessus dans la règle \code{create-release} du fichier
  de configuration \code{Makefile} qui se trouve dans le
