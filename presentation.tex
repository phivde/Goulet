\chapter{Présentation du langage S}
\label{presentation}
...
Le S est un langage pour «programmer avec des données» développé chez
Bell Laboratories (anciennement propriété de AT\&T, maintenant de
Lucent Technologies).
...
De plus, \citet{Sprogramming,MASS} constituent des références sur le
langage S devenues au cours des dernières années des standards
\emph{de facto}.
...
\item À la question 6.2 de la foire aux questions (FAQ) de \textsf{R},
  ...
  Emacs?»\index{Emacs}, la réponse est: «Oui, définitivement.» Nous
...
\item L'annexe \ref{ess} présente les plus importantes commandes à
  connaître pour utiliser Emacs et le mode ESS.
\end{itemize}
...
\end{verbatim}
  ...
  dans un \emph{buffer} nommé \texttt{*R*}.
...
\item Le code est virtuel et les objets sont réels. C'est l'approche
  qu'encouragent les interfaces graphiques, mais aussi la moins
...
\begin{exercice}
  ...
  davantage orienté vers les application statistiques que vers la
