...
\label{sec:environnement:contexte}
...
<<script, results=hide>>=
###
### MISE EN CONTEXTE  `\labelline{environnement:contexte}`
###
...
\gotorbox{Étudiez les lignes \reflines{environnement:contexte} du
...
Le code informatique de la \autoref{sec:environnement:exemples}
...
\gotorbox{Avant d'aller plus loin, passez en revue les exemples des
  lignes \reflines{environnement:environnement} du fichier de script
...
\autoref{sec:environnement:contexte}.
<<echo=TRUE>>=
y <- 123
f <- function(x)
{
...
dans le code informatique de la \autoref{sec:environnement:exemples}.
...
\gotorbox{Étudiez les lignes \reflines{environnement:portee}
...
\gotorbox{Étudiez les lignes \reflines{environnement:lazy}
  du fichier de script \code{\scriptfilename} reproduit à la
  \autoref{sec:environnement:exemples}.}
