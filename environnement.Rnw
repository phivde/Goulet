...
\begin{objectifs}
\item Décrire les concepts d'environnement d'une fonction et
...
travail, son environnement est l'\emph{environnement
...
global. De plus, le dictionnaire contient la paire (\code{x}, $10$),
...
crée un nouvel environnement d'évaluation $\rho_2$ pour la fonction
\code{g}. L'environnement englobant est $\rho_1$ et, comme cette
