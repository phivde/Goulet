...
\begin{prob-enonce}
  ...
  dividendes de 6 titres financiers.

  Pour les fins de cet exercice, nous nous intéresserons qu'aux
  mouvements boursiers, et ce, pour un seul titre --- nous ignorerons
  donc le versement de dividendes.

  La valeur du titre est établie à partir de la valeur au tour de jeu
  précédente et du résultat du lancer de deux dés.
  \begin{enumerate}
  ...
  On fournit les résultats de 10 lancers des dés:
...
@
  ...
  successives du titre. La fonction prend en arguments un \emph{data
    frame} \code{x} contenant les résultats des lancers des dés, comme
  ci-dessus, ainsi que \code{start}, la valeur de départ du titre
  ($100$ par défaut). La fonction retourne un vecteur contenant les
  valeurs  successives du titre.

  Par exemple, avec les données ci-dessus, on obtiendrait:

<<label=calculervaleurs, echo=FALSE, eval=TRUE>>=
...
\begin{prob-solution}
  ...
  que nous proposons ci-dessous repose sur deux idées principales:
  \begin{enumerate}
...
valeurs <- function(x, start = 100)
{
    ## Création d'un vecteur pour les résultats. La valeur de
    ## départ est placée au début du vecteur pour faire la
    ## boucle. Elle sera supprimée à la fin.
    res <- c(start, numeric(nrow(x)))

    ## Conversion des valeurs de la première colonne des
    ## données de ("hausse", "nul", "baisse") en (1, 0, -1).
    d <- (x[, 1] == "hausse") - (x[, 1] == "baisse")
