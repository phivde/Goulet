...
commentaires contenant au minimum: la
\index{signature}\emph{signature} de la fonction (son nom suivi, entre
parenthèses, de tous les arguments avec leur valeur par défaut, le cas
échéant); une courte description de ce que fait la fonction; la liste
des arguments, de leur signification et des valeurs admissibles; la
valeur retournée par la fonction. Il s'agit de la structure générale
des rubriques d'aide de R. La \autoref{fig:fonctions:doc} fournit un
exemple de documentation pour une fonction simple.
...
\gotorbox{Le fichier de script \code{\scriptfilename} reproduit à la
  ...
  cas de Syndrôme de la plaque à biscuits{\texttrademark}.}
