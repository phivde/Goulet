...
plusieurs personnes trient sans que ne soit nécessaire; ou bien que
...
\item tri par échange, où deux cartes qui se sont pas dans le bon
...
\item tri par une méthode \emph{ad hoc} qui fonctionne pour une main
  de cinq cartes, mais qui en saurait se généraliser à un problème de
  plus grande taille;
...
\begin{exercice}
  ...
  Les fichier \code{100metres.dat} distribué avec cet ouvrage et
  reproduit à la \autoref{fig:tri:100metres} contient la date et le
  temps des 31 meilleurs résultats enregistrés au 100~mètres homme
  entre 1964 et 2005. Les dates se trouvent dans la première colonne
  du fichier et les temps, dans la seconde. La commande \icode{scan}
  utilisée avec l'argument \code{what = list("", 1)} permet d'importer
  ces données dans R. Extraire de ce vecteur les records du monde
  ...
  \begin{figure}[t]
    ...
    \begin{minipage}{0.5\linewidth}
      \lstinputlisting{100metres.dat}
    \end{minipage}
    \caption{Données du fichier \code{100metres.dat}}
    \label{fig:tri:100metres}
  \end{figure}
  \begin{sol}
