...
\item tri paresseux, qui consiste à ne pas faire l'expérience
  recommandée.
\end{enumerate}
La \autoref{fig:tri:cartes} illustre les quatre premières méthodes de
tri ci-dessus.
...
\begin{figure}
  ...
  \caption{Illustrations de quatre méthodes de tri pour une main de
    cinq cartes. Les lignes verticales délimitent les cartes triées de
    celles qui ne le sont pas. Les symboles «{\smaller\faAngleLeft}»
    et «{\smaller\faAngleRight}» indiquent le sens et le nombre de
    déplacements à effectuer. Le symbole «{\smaller\faAngleUp}»
    identifie la plus petite carte non triée.}
