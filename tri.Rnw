...
\label{sec:fonctions:boucles}

Comme nous l'avons déjà vu au \autoref{chap:algorithmes}, il existe
deux grandes manières de répéter des calculs en programmation: par
\index{iteration@itération}itération ou par
\index{recursion@récursion}récursion. La récursion est très simple à
mettre en œuvre dans un langage fonctionnel comme R puisqu'il suffit
de faire en sorte qu'une fonction s'invoque elle-même, idéalement,
dans R, avec la fonction \icode{Recall}
...
rm("sqrt")                              #-*- `\labelline{tri:boucles:fin}`
@
