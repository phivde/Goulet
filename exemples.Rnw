...
étant dû dans un an. Trouver le montant du prêt pour chacune des
...
@
\item Avec différents paiements annuels et des taux d'intérêt
  différents, il faut employer la formule \eqref{eq:vp}. Le produit
  des taux d'intérêt est obtenu avec la fonction \fonction{cumprod}.
<<1(c)>>=
...
@
  ...
  \item Ici, la valeur de $\alpha$ demeure fixe, mais on doit calculer,
    en une seule expression, la valeur de la fonction de répartition en
    plusieurs points. C'est un travail pour la fonction
    \fonction{outer}.
<<3(b)>>=
x <- 2:10
...
\begin{sol}
  ...
  créer une fonction à partir de ce code.
<<4>>=
i <- 0.05
repeat
{
