...
\verb|x[i]|\index{[@\code{[}}, ou remplacer des éléments avec la
construction \verb|x[i] <- y|\Index{[<-@\code{[<-}}.
...
c'est tout à fait optionnel. En effet, l'appel à \icode{function}
retourne déjà une fonction et si aucun nom n'est attribué à cet objet,
on obtient ce qui est appelé une
\index{fonction!anonyme}\emph{fonction anonyme}. Les fonctions
...
Une fonction peut n'avoir aucun \index{fonction!argument
  formel}argument formel ou plusieurs. Il n'y a pas de limite pratique
au nombre d'arguments formels que peut avoir une fonction. Les
arguments peuvent être des symboles, des constructions de la
