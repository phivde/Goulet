...
\item chaines de caractères: \code{"a"}, \code{"abc"};
\item valeurs booléennes: \Objet{TRUE}, \Objet{FALSE};
\item donnée manquante: \Objet{NA};
\item valeurs infinies et indéterminée: \Objet{Inf}, \code{-Inf},
  \Objet{NaN};
\item valeur «néant»: \Objet{NULL};
...
utiles pour la programmation mathématique et la manipulation de
données. Nous reviendrons sur les caractéristiques de certains à la
...
\end{itemize}
Les opérations arithmétiques sont applicables aux nombres réels, voire
...
\item Une expression est immédiatement évaluée et le résultat est
  affiché sous l'invite de commande. Les symboles des données
  fondamentales représentent ces données, comme on pourrait s'y
  attendre.
<<echo=TRUE>>=
42
3 + 2i
...
\item Lors d'une affectation, une expression est évaluée, mais le
  résultat est stocké dans un \emph{objet} (ou \emph{variable}) et
  rien n'est affiché à l'écran. Le symbole d'affectation est
  \Fonction{<-}, c'est-à-dire les deux caractères \verb|<| et
  \verb|-| placés obligatoirement l'un à la suite de l'autre.
<<echo=TRUE>>=
a <- 5
a
b <- a
...
\item Évitez d'utiliser l'opérateur \,\fonction{=}\, pour affecter une
  ...
  commandent d'utiliser l'opérateur \code{<-} pour l'affectation, point.
\end{itemize}
...
\item Par conséquent, si le regroupement se termine par une
  affectation, aucune valeur n'est retournée ni affichée à l'écran.
<<echo=TRUE>>=
{
    a <- 2 + 3
    b <- a
...
\item Les règles ci-dessus joueront un rôle important dans la
  composition de fonctions; voir le \autoref{sec:bases:fonctions}.
