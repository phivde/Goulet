...
un objet ne peut avoir qu'un seul mode. Le \autoref{tab:bases:modes} contient la liste des principaux modes
...
les plus fréquemment rencontrés se trouve au \autoref{tab:attributs}. Pour chaque attribut, il existe une fonction du
