%%% Copyright (C) 2005-2023 Vincent Goulet
%%%
...
\begin{table}
  ...
  \begin{tabular}{lp{8cm}}
    ...
    \code{\string^}\index{\string^@\code{\string^}} & puissance \\
...
## 'Comparaison' au besoin (qui vous dira de ne pas faire
...
NA == 2                    # 'NA' n'est égale à rien...
NA == NA                   # ... pas même elle-même!
is.na(NA)                  # test si une valeur est 'NA'
is.na(2)                   # '2' n'est pas manquante
...
correspondent à l'opérateur et aux opérandes de la combinaison. Les
...
\begin{figure}
  ...
  \end{picture}
  \caption{Représentation en arbre de l'évaluation de l'expression
    composée \code{(2 + ((2 + (4 * 6)) * (3 + 5)))/2}}
...
et rien n'est affiché à l'écran. Tel que mentionné précédemment,
l'opérateur d'affectation est «\icode{<-}», c'est-à-dire les deux
caractères «\code{<}» et «\code{-}» placés obligatoirement l'un à la
suite de l'autre, et l'opération d'affectation est évaluée de droite à
gauche. Ainsi, toute expression est évaluée avant que son résultat ne
soit placé dans l'objet.
<<echo=TRUE>>=
a <- 5
a
b <- a
...
parenthèses pour ainsi créer une nouvelle expression.\footnote{%
  En fait, l'opérateur d'affectation «\code{<-}» retourne un résultat
  --- la valeur affectée ---, mais de manière \emph{invisible}. Une
  affectation placée entre parenthèses devient un appel à l'opérateur
  «\code{(}» qui, lui, ne fait que retourner son argument.}
<<echo=TRUE>>=
(circ <- 2 * pi * 10)
@

L'opérateur d'affectation inversé «\icode{->}» existe aussi, mais il
...
généralement transparente, mais au fur et à mesure que votre apprentissage
du langage progressera, le rôle de l'environnement deviendra plus important.
Nous étudierons le concept au \autoref{chap:environnement}.
...
il faut entourer les commandes d'accolades «\code{\{~\}}». Le résultat
...
valeurs, souvent des coordonnées d'un plan cartésien. Dans R, la
...
\autoref{fig:bases:vecteur} représente schématiquement un
\index{vecteur!simple}vecteur simple dans R.
...
Nous allons nous restreindre pour le moment aux
\index{vecteur!simple}vecteurs simples
(\index{vecteur!atomique}atomiques) dans lesquels tous les éléments
sont du même type ou, plus précisément, du même
\index{mode}\emph{mode}\footnote{%
...
\index{vecteur!concaténation}\index{concaténation}concaténation
\Icode{c}.
<<echo=TRUE>>=
(x <- c(2, 5.1, 42))
@
...
d'un vecteur; on dit qu'on leur attribue une
\Index{etiquette@étiquette}\emph{étiquette}. Le résultat est un
\Index{vecteur!étiqueté}\emph{vecteur étiqueté}, ou
\Index{vecteur!nommé}\emph{vecteur nommé}. Il y a deux grandes façons
de procéder: en spécificant les noms dès la création du vecteur, ou à
posteriori en ajoutant l'attribut au vecteur à l'aide de la fonction
\Icode{names}. Celle-ci permet aussi d'extraire les
\index{etiquette@étiquette}étiquettes du vecteur.
<<echo=TRUE>>=
(x <- c(a = 1, b = 2, c = 5))
names(x)
@
...
Si vous tentez de créer un \index{vecteur!simple}vecteur simple avec
des données de modes différents, R effectuera une %
\index{conversion forcée}conversion forcée (\emph{coercion}) vers un
mode unique. Celui-ci est choisi pour minimiser la perte d'information
sur les objets originaux.
<<echo=TRUE>>=
c(2, TRUE, FALSE)
c(2, FALSE, "a")
@
...
courts sont \Index{recyclage}\emph{recyclés} autant de fois que
nécessaire pour correspondre au plus long vecteur. Cette règle est
...
Autrement, le plus court vecteur est \index{recyclage}recyclé un
nombre fractionnaire de fois, mais comme ce résultat est rarement
celui souhaité et qu'il provient généralement d'une erreur de
programmation, un avertissement est affiché.
\begin{Schunk}
\begin{Sinput}
> 1:10 + c(2, 4, 6)
\end{Sinput}
...
\cautionbox{La règle de \index{recyclage}recyclage des vecteurs fait
  en sorte qu'il y a très peu d'erreurs de longueur dans R. Qu'une
  expression soit valide ne signifie donc pas qu'elle effectue le bon
  calcul!}

Il tombe sous le sens que les opérateurs arithmétiques sont conçus
pour des arguments numériques et les opérateurs logiques, pour des
arguments booléens. Si un argument n'est pas du bon mode, R effectuera
la \index{conversion forcée}conversion forcée vers le mode approprié.
En particulier, les valeurs booléennes \code{TRUE} et \code{FALSE} se
verront converties en $1$ et $0$, respectivement, dans les opérations
arithmétiques. À l'inverse, dans les opérations logiques, $0$ est
converti en \code{FALSE} et \emph{tout} autre nombre est converti en
\code{TRUE}.
<<echo=TRUE>>=
2 + c(TRUE, FALSE)
c(0, 5, -1) & TRUE
...
beaucoup utilisée dans R, aussi est-il important d'en maitriser toutes
les subtilités. L'opération sert principalement à deux choses:
extraire des éléments d'un objet avec la construction
\code{x[i]}\indexcode{[}, ou remplacer des éléments avec la
construction \code{x[i]~<-~y}\Indexcode{[<-}.
...
x[x > 0]
@
\item \textbf{Extraction par étiquette} avec un vecteur de chaines de
  caractères. Les éléments dont \index{etiquette@étiquette}l'étiquette
  correspond à l'une des chaines sont extraits du vecteur. Cette
  méthode d'indiçage a comme principal avantage de permettre
  l'extraction d'éléments d'un vecteur indépendamment de leur position
  dans celui-ci.
<<echo=TRUE>>=
x[c("B", "D")]
@
...
x[2.6]                     # fractions tronquées vers 0 `\labelline{bases:vecteurs:fin}`
@
...
\item \meta{arguments} est la liste des arguments formels de la
  ...
  Une fonction peut n'avoir aucun %
  \index{fonction!argument formel}argument formel ou plusieurs. Il n'y
  a pas de limite pratique au nombre d'arguments formels que peut
  avoir une fonction. Les arguments peuvent être des symboles, des
  constructions de la forme \code{symbole = défaut}, ou encore
  l'argument formel spécial «\icode{...}».
...
Sous peine de causer bien des interrogations, évitez que la dernière
expression d'une fonction soit une affectation. En effet, pour les
raisons mentionnées à la \autoref{sec:bases:commandes:affectation}, la
fonction ne retournerait alors aucun résultat \emph{visible}
susceptible d'être affiché à l'écran.

Lorsque le résultat de l'évaluation d'une fonction n'est pas affecté à
une variable, R affiche par défaut le résultat à l'écran.

\warningbox{N'utilisez \emph{jamais} la fonction \icode{print} à
  l'intérieur d'une fonction pour afficher ses résultats à l'écran.
  Cette fonction sert principalement pour le débogage
  (\autoref{sec:debogage:syntaxe}) ou pour des méthodes spécialisées
  d'affichage des résultats --- un sujet qui n'est pas abordé dans le
  présent ouvrage.}

Les fonctions suivantes permettent de retourner un résultat, un
...
\begin{enumerate}
\item Spécifier les valeurs des arguments dans le bon ordre.
...
## Suite à l'appel de fonction ci-dessus, R a automatiquement
## affiché le résultat à l'écran, et ce, parce que le résultat
## n'est pas affecté à une variable. L'expression suivante
## effectue le même calcul, mais en affectant le résultat dans
## un objet. Comme à l'habitude, aucun résultat ne s'affiche
## alors à l'écran.
y <- square(10)
...
\item \meta{condition} est une expression dont le résultat est une
  ...
  (\autoref{sec:bases:internes:tests}) se révèlent utiles dans les
  clauses \code{if} pour réduire les vecteurs booléens à une valeur
  unique. Les fonctions \icode{isTRUE} et \icode{isFALSE}, quant à
  elles, permettent de s'assurer que la condition est un vecteur
  booléen de longueur $1$ différent de \icode{NA}.
\item \meta{conséquence} est une expression, ou un groupe
  d'expressions regroupées entre accolades «\code{\{~\}}», qui sont
...
Les constructions de type «Selon que» se conçoivent en R avec la
...
valeur numérique $k$, alors l'\meta{action $k$} est exécutée. Si le
...
## condition en argument n'est pas une valeur unique. Il n'y a
## pas de scénario dans lequel ce pourrait être souhaitable de
## procéder ainsi. R signale donc une erreur.
...
## Ou pire encore en utilisant une logique tordue où l'on
...
Consultez les rubriques d'aide des fonctions pour connaitre toutes
leurs options.
...
seq(1, 9, by = 2)
@
%% Utilisation de \char 95\relax pour générer un « _ » pour que le
%% terme soit regroupé avec les entrées générées par listings dans
%% l'index.
\item[\Icode{seq\char 95\relax len}] \index{suite}suite de
  nombres de la longueur donnée en argument à partir de $1$ (plus
  rapide que \code{seq}).
<<echo=TRUE>>=
seq_len(10)
@
\item[\Icode{seq\char 95\relax along}] suite de nombres de
  la longueur du vecteur en argument à partir de $1$ (plus rapide que
  \code{seq}).
<<echo=TRUE>>=
seq_along(c(-1, 0, 2))
@
...
rep.int(2, 10)
@
\item[\Icode{rep\char 95\relax len}] répétition de vecteurs jusqu'à une
  certaine longueur (plus rapide que \code{rep}).
<<echo=TRUE>>=
rep_len(1:3, 10)
@
\end{ttscript}

\cautionbox{Les fonctions \icode{seq\char 95\relax len} et
  \icode{seq\char 95\relax along} sont plus robustes que \icode{seq}
  ou que l'opérateur «\code{:}» pour la programmation. Lorsque leur
  argument est, dans l'ordre, $0$ et un vecteur de longueur nulle, les
  fonctions retournent un vecteur de longueur nulle. En revanche, le
  résultat de \code{seq(0)} ou de \code{1:0} est le vecteur \code{c(1,
    0)}.}
...
mean(x)
@
\item[\Icode{var}] \index{variance}variance %
  (et \index{ecart type@écart type}écart type avec \Icode{sd}).
<<echo=TRUE>>=
var(x)
@
...
\subsection{Manipulation de chaines de caractères}
\label{sec:bases:internes:texte}

\begin{ttscript}{strsplit}
\item[\Icode{paste}] \index{chaine de caractères!concaténation}
  \index{concaténation}concaténation de vecteurs après les avoir
  convertis en chaines de caractères, séparés par défaut par des
  espaces
<<echo=TRUE>>=
paste(c("Camille", "Marianne", "Alexandre"),
      "porte le numéro", c(8, 7, 2))
@
\item[\Icode{paste0}] comme \icode{paste}, mais sans séparateur entre
  les éléments
<<echo=TRUE>>=
paste0(1:5, c("er", rep("e", 4)))
@
\item[\Icode{nchar}] \index{chaine de caractères!nombre de caractères}
  nombre de caractères dans une chaine de caractères
<<echo=TRUE>>=
nchar(c("foo", "foobar"))
@
\item[\Icode{substr}] \index{chaine de caractères!sous-chaine}
  sous-chaine (partie) d'une chaine de caractères
<<echo=TRUE>>=
substr("abcdef", 2, 4)
@
\item[\Icode{strsplit}] \index{chaine de caractères!séparation}
  séparation d'une chaine de caractères en parties sur un caractère
  quelconque
<<echo=TRUE>>=
...
seq_len(0)                 # plus simple, plus robuste
...
## 'seq_len(length(x))'. De plus, elle est plus robuste que
## 'seq' lorsque la longueur du vecteur est 0.
seq(1, along = x)            # suite de la longueur de x
seq_len(length(x))           # idem, mais deux fonctions
seq_along(x)                 # plus rapide, plus simple
y <- numeric(0)              # vecteur de longueur nulle
seq(length(y))               # pas ce qui souhaité!
1:length(y)                  # ça non plus!
seq_along(y)                 # plus simple, plus robuste
...
all(c(TRUE, FALSE, TRUE))   # pas toutes les valeurs TRUE
...
rev(cumprod(y))            # produit cumulatif (de g. à d.)
rev(cumprod(rev(y)))       # produit cumulatif (de d. à g.)
...
strsplit("2001-05", "-")   # séparation en sous-chaines  `\labelline{bases:internes:fin}`
@
...
@
     \item Attention à la priorité des opérations! En vertu des
       ...
       (\code{<}), négation logique (\code{!}) , «ou» logique
       (\code{|}).
<<echo=FALSE>>=
...
@
    \begin{enumerate}
\item Indiçage avec un seul entier positif pour extraire une donnée.
<<echo=TRUE,eval=FALSE>>=
x[2]
@
\item Indiçage avec un vecteur d'entiers positifs pour extraire
  plusieurs données.
<<echo=TRUE,eval=FALSE>>=
x[1:5]
@
\item Indiçage avec un vecteur booléen pour extraire par critère.
<<echo=TRUE,eval=FALSE>>=
x[x > 14]
@
\item Indiçage avec un vecteur d'entiers négatifs pour laisser tomber
  des données.
<<echo=TRUE,eval=FALSE>>=
x[-c(6, 7, 12)]
@
...
\begin{exercice}
  ...
  \item $n/(\sum_{i = 1}^n x_i^{-1})$ (moyenne harmonique de
    $\mat{x}$)
  ...
  \begin{sol}
    \begin{enumerate}
    \item La somme de deux vecteurs --- qui s'effectue élément par
      élément --- fait partie intégrante du langage R.
<<echo=TRUE,eval=FALSE>>=
3 * (x + y)
@
    \item La fonction \icode{sum} additionne tous les éléments d'un
      vecteur.
<<echo=TRUE,eval=FALSE>>=
sum(x)/length(x)
@
    \item La fonction \icode{prod} multiplie entre eux tous les
      éléments d'un vecteur.
<<echo=TRUE,eval=FALSE>>=
prod(x)^(1/length(x))
@
    \item Le calcul de la moyenne harmonique repose sur la fonction
      \icode{sum}.
<<echo=TRUE,eval=FALSE>>=
length(x)/sum(1/x)
@
    \item Le produit scalaire est la somme des produits élément par
      élément de deux vecteurs.
<<echo=TRUE,eval=FALSE>>=
sum(x * y)
@
    \item La norme $1$ est la somme des différences en valeurs
      absolues élément par élément de deux vecteurs.
<<echo=TRUE,eval=FALSE>>=
sum(abs(x - y))
@
    \item La norme «infini» est la plus grande des différences en
      valeurs absolues élément par élément de deux vecteurs.
<<echo=TRUE,eval=FALSE>>=
max(abs(x - y))
@
    \end{enumerate}
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
    \item Les fonctions utilisent la structure \code{if ... else ...}
      qui n'est pas vectorielle.
    \item Dans les mises en œuvre des algorithmes
      ...
      Si nous ne forcions pas la sortie de la fonction avec
...
@
  ...
  \begin{sol}
    ...
    juste en vertu des règles de %
    \index{portee lexicale@portée lexicale}\emph{portée lexicale} de
    R. Celles-ci font l'objet de la
    \autoref{sec:environnement:portee}. Dans certains langages de
    programmation, les objets seraient indéfinis et les appels de
    fonction résulteraient tous en une erreur.
    \begin{enumerate}
    \item Dans l'appel \code{f(1, 1)}, les valeurs de \code{x} et
      \code{y} sont égales à $1$ dans la fonction $f$ et l'argument
      \code{a} de la fonction \code{g} est égal à $2$.
<<echo=FALSE>>=
f(1, 1)
@
    \item Dans l'appel \code{f(f(1, 1), 1)}, l'argument \code{x} de la
      fonction \code{f} externe est égal au résultat en a), soit
      $\Sexpr{f(1, 1)}$, et l'argument \code{y} est égal à $1$. Le
      reste des calculs s'effectue comme en a).
<<echo=FALSE>>=
f(f(1, 1), 1)
@
    \item Cette fois, l'argument \code{x} de la
      fonction \code{f} externe est égal à $1$ et l'argument \code{y}
      est égal  au résultat en b).
<<echo=FALSE>>=
f(1, f(f(1, 1), 1))
@
    \end{enumerate}
...
\end{verbatim}
  ...
  \begin{sol}
    \begin{enumerate}
\item Vecteur des nombres $0$ et $6$, répété trois fois.
<<echo=TRUE, eval=FALSE>>=
rep(c(0, 6), 3)
@
\item Suite de $1$ à $10$ par bonds de $3$.
<<echo=TRUE, eval=FALSE>>=
seq(1, 10, by = 3)
@
\item Suite $1, 2, 3$ répétée quatre fois.
<<echo=TRUE, eval=FALSE>>=
rep(1:3, 4)
@
\item Suite $1, 2, 3$ dont les éléments sont répétés une, deux et
  trois fois, dans l'ordre.
<<echo=TRUE, eval=FALSE>>=
rep(1:3, 1:3)
@
\item Suite $1, 2, 3$ dont les éléments sont répétés trois, deux et
  une fois, dans l'ordre.
<<echo=TRUE, eval=FALSE>>=
rep(1:3, 3:1)
@
\item Suite de trois nombres équidistants entre $1$ à $10$, inclusivement.
<<echo=TRUE, eval=FALSE>>=
seq(1, 10, length = 3)
@
\item Suite $1, 2, 3$ dont les éléments sont chacun répétés quatre fois.
<<echo=TRUE, eval=FALSE>>=
rep(1:3, rep(4,3))
@
    \end{enumerate}
...
\begin{exercice}
  ...
  {\NoAutoSpacing\verb|:|}\index{:@\NoAutoSpacing\verb=:=} et de la fonction
...
\end{verbatim}
  ...
  \begin{sol}
    \begin{enumerate}
\item Générer les entiers de $11$ à $20$ et diviser ensuite par $10$.
<<echo=TRUE, eval=FALSE>>=
11:20/10
@
\item Générer la suite $0, 2, 4, \dots, 18$ en multipliant par $2$ la
  suite $0, 1, 2, \dots, 9$, puis ajouter $1$.
<<echo=TRUE, eval=FALSE>>=
2 * 0:9 + 1
@
\item Répéter la suite $-2, -1, 0, 1, 2$ deux fois.
<<echo=TRUE, eval=FALSE>>=
rep(-2:2, 2)
@
\item Répéter deux fois chaque élément de la suite $-2, -1, 0, 1, 2$.
<<echo=TRUE, eval=FALSE>>=
rep(-2:2, each = 2)
@
\item Générer la suite $1, 2, \dots, 10$ et multiplier par $10$.
<<echo=TRUE, eval=FALSE>>=
10 * 1:10
@
    \end{enumerate}
...
\begin{exercice}
  ...
  \begin{sol}
    Il s'agit de calculer le produit cumulatif des nombres de $1$ à
    $10$.
<<echo=TRUE, eval=FALSE>>=
cumprod(1:10)
@
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
\item Extraction par position des cinq premiers éléments du vecteur.
<<echo=TRUE, eval=FALSE>>=
x[1:5]
head(x, 5)
@
\item La fonction \icode{max} retourne la plus grande valeur d'un
  vecteur.
<<echo=TRUE, eval=FALSE>>=
max(x)
@
\item Il s'agit d'extraire les cinq premiers éléments du vecteur et de
  calculer la moyenne des valeurs obtenues.
<<echo=TRUE, eval=FALSE>>=
mean(x[1:5])
mean(head(x, 5))
@
\item Il s'agit d'extraire les cinq derniers éléments du vecteur et de
  calculer la moyenne des valeurs obtenues. Deux solutions: une qui
  implique de connaitre exactement la longueur du vecteur et l'autre,
  préférable, qui demeure valable peu importe la longueur du vecteur.
<<echo=TRUE, eval=FALSE>>=
mean(x[16:20])
mean(tail(x, 5))
@
