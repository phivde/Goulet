...
\autoref{fig:bases:vecteur} représente schématiquement un vecteur dans
R.
...
dans lesquels tous les éléments sont du même type ou, plus
...
forcée vers un mode unique. Celui-ci est choisi pour minimiser la
perte d'information sur les objets originaux.
<<echo=TRUE>>=
c(2, TRUE, FALSE)
c(2, FALSE, "a")
@

Vous pouvez aussi forcer la conversion d'un vecteur d'un mode vers un
autre avec les fonctions \Icode{as.numeric}, \Icode{as.logical} et
\Icode{as.character}.
<<echo=TRUE>>=
as.logical(1)
as.numeric(FALSE)
...
avoir recours à des procédures itératives ou récursives. En soi, il
s'agit d'une couche d'abstraction directement intégrée au langage.
Pour prétendre savoir programmer en R, vous devez absolument tirer
profit de l'arithmétique vectorielle.
...
\autoref{tab:bases:operateurs} ainsi que l'arithmétique vectorielle.
Le langage R serait toutefois très restreint s'il ne nous donnait pas
aussi accès à des fonctions\footnote{%
...
quelques unes dans les exemples, comme \icode{exp} ou \icode{cos}, et vous
en découvrirez de nouvelles à la \autoref{sec:bases:internes}. Chose
plus importante, R nous permet aussi de définir nos propres fonctions.
C'est là l'une des techniques d'abstraction les plus puissantes du
langage puisqu'elle permet de faire référence par un seul nom à toute
...
@
  ...
  meilleur exemple à la \autoref{sec:bases:exemples} que ce que nous
...
(\code{-} et \code{\string^}). Elle repose également sur l'arithmétique
...
étudiées à la \autoref{sec:algorithmes:if-else}.
...
\item \meta{condition} est une expression dont le résultat est une
  ...
  fréquente consiste à construire un test de telle sorte que
...
valeur numérique $k$, alors \meta{action$_k$} est exécutée. Si le
...
\begin{exercice}
  ...
  une fonction \code{mrl} pour calculer l'espérance résiduelle
...
@
  ...
  \begin{sol}
    ...
    \item le reste de la division étant nécessairement $0$ ou $1$,
      ...
      tirant profit de la conversion automatique.
    \end{enumerate}
<<echo=FALSE>>=
is.odd <- function(x)
{
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    Tel qu'expliqué à la \autoref{sec:fonctions:recursion}, il est
