...
L'expression ci-dessus peut se lire ainsi: «\code{square} est une
fonction qui prend en argument une valeur \code{x} et qui la multiplie
par elle-même.»
...
\begin{itemize}
\item Lorsqu'un symbole seul est utilisé, il indique le nom de
  l'argument. L'objet de ce nom sera disponible à l'intérieur de la
  fonction.
<<echo=TRUE>>=
f <- function(x) 2 * x + 1
f(2)
@
\item La construction \code{symbole = défaut} indique la valeur par
  défaut de l'argument \code{symbole}. En d'autres termes, c'est la
  valeur de l'argument si aucune valeur n'est spécifiée dans l'appel
  de la fonction.
<<echo=TRUE>>=
...
f()
@
\item L'argument spécial `\code{...}' peut contenir un nombre
  ...
  `\code{...}' pour passer des arguments tels quels à une autre
  fonction qui, elle, saura quoi faire de ces arguments.
\end{itemize}
...
obtient ce qui est appelé une \emph{fonction anonyme}. Les fonctions
anonymes sont particulièrement utiles avec les fonctions d'application
qui seront étudiées au \autoref{chap:application}.
...
En premier lieu, l'interpréteur R reconnait un appel de fonction au
fait que le nom de l'objet est suivi de parenthèses \code{(~)}.

Ensuite, l'ordre des arguments dans la définition de la fonction est
important. Nous avons deux choix au moment d'appeler une fonction:
spécifier les valeurs des arguments dans le bon ordre, ou nommer
explicitement les arguments avec une construction de la forme
\code{symbole = expression}, auquel cas l'ordre n'importe plus. Il est
beaucoup plus prudent et \emph{fortement recommandé} de spécifier les
...
Il n'est pas nécesaire de spécifier une valeur pour un argument
...
\autoref{sec:bases:commandes:regroupements} s'appliquent ici au corps
de la fonction: une fonction tout simplement le résultat de la
...
\lstinputlisting[firstline=3]{\scriptfilename}
