...
\cautionbox{La fonction \icode{seq\char 95\relax len} est plus
  robuste que \icode{seq} ou que l'opérateur «\code{:}» pour la
  programmation. L'expression \code{seq\_len(\meta{n})} génère une
  suite de longueur \meta{n}. C'est généralement équivalent à
  \code{seq(\meta{n})} ou \code{1:\meta{n}}, sauf lorsque \meta{n}
  vaut $0$. En effet, \code{seq\_len(0)} génère un vecteur de
  longueur nulle, alors que le résultat de \code{seq(0)} et de
  \code{1:0} est malencontreusement le vecteur \code{c(1, 0)}.}
