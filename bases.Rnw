...
des données de modes différents, R effectuera une \index{conversion
  forcée}conversion forcée (\emph{coercion}) vers un mode unique.
Celui-ci est choisi pour minimiser la perte d'information sur les
objets originaux.
<<echo=TRUE>>=
c(2, TRUE, FALSE)
c(2, FALSE, "a")
