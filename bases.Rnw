...
\item \meta{nom} est le nom de la fonction (les règles pour les noms
  ...
  retourne déjà une fonction et si aucun nom n'est attribué à cet
  objet, on obtient ce qui est appelé une
  \index{fonction!anonyme}\emph{fonction anonyme}. Les fonctions
  anonymes sont particulièrement utiles avec les fonctions
