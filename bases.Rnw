...
\begin{astuce}
  ...
  caractère de soulignement «\code{\_}». Cet emploi n'est plus permis,
  mais la pratique subsiste dans le mode ESS de Emacs. Ainsi, taper le
  caractère «\code{\_}» hors d'une chaîne de caractères dans Emacs
  génère automatiquement \verb*| <- |. Si l'on souhaite véritablement
  obtenir le caractère de soulignement, il suffit d'appuyer deux fois
  successives sur «\_».
\end{astuce}
...
\item Une autre --- la meilleure, en fait --- façon d'indicer un seul
  élément d'une liste est par le biais de l'opérateur \Fonction{\$},
  avec une construction \code{x\$etiquette}:
<<echo=TRUE>>=
x$size
@ %$
...
L'indiçage sert principalement à deux choses:
\begin{inparaenum}[1)]
\item extraire des éléments d'un objet avec la construction
  \code{x[i]};
\item les remplacer avec la construction \verb|x[i] <- y|.
\end{inparaenum}
\begin{itemize}
