...
\begin{objectifs}
\item Connaître la syntaxe et la sémantique du langage R.
\item Comprendre la notion d’objet et connaître les principaux types
  de données dans R.
\item Comprendre et savoir tirer profit de l’arithmétique vectorielle
  de R.
\item Comprendre la différence entre les divers modes d’objets R (en
  particulier \code{numeric}, \code{character} et \code{logical}) et
  la conversion automatique de l’un à l’autre.
\item Comprendre la différence entre un vecteur, une matrice, un
  tableau, une liste et un \emph{data frame} et savoir créer ces
  divers types d’objets.
\item Savoir extraire des données d’un objet ou y affecter de
  nouvelles valeurs à l’aide des diverses méthodes d’indiçage.
\end{objectifs}

Avant de pouvoir utiliser un langage de programmation, il faut en
connaître la syntaxe et la sémantique, du moins dans leurs grandes
lignes. C'est dans cet esprit que ce chapitre introduit des notions de
base du langage R telles que l'expression, l'affectation et l'objet.
Le concept de vecteur se trouvant au c{\oe}ur du langage, le chapitre
...
\begin{astuce}
  Dans les anciennes versions de S et R, l'on pouvait affecter avec le
  caractère de soulignement «\code{\_}». C'est emploi n'est plus
  permis, mais la pratique subsiste dans le mode ESS de Emacs. Ainsi,
  taper le caractère «\code{\_}» hors d'une chaîne de caractères dans
  Emacs génère automatiquement \verb*| <- |. Si l'on souhaite
  véritablement obtenir le caractère de soulignement, appuyer deux
  fois successives sur «\_».
\end{astuce}
...
\begin{itemize}
\item On considère généralement comme une mauvaise pratique d'employer
...
\item Les règles ci-dessus joueront un rôle important dans la
  composition de fonctions; voir le chapitre \ref{fonctions}.
...
un objet ne peut avoir qu'un seul mode. Le tableau
\ref{tab:bases:modes} contient la liste des principaux modes
disponibles en R. À chacun de ces modes correspond une fonction du
même nom servant à créer un objet de ce mode.
...
\item En revanche, les objets de mode \code{"list"} ou
  ...
  ou plusieurs autres listes; voir la section~\ref{bases:listes} pour
  plus de détails.
...
length(v2)
@
\item On obtient le nombre de caractères dans un chaîne avec la
  fonction \Fonction{nchar}:
<<echo=TRUE>>=
nchar(v1)
nchar(v2)
@
...
type $\frac{0}{0}$ ou $\inf - \inf$. R dispose d'objets spéciaux pour
...
chapitre~\ref{operateurs}, le vecteur est l'unité de base dans les
calculs.
...
généralement, les tableaux à plusieurs dimensions (\emph{arrays}).

Les matrices et tableaux ne sont rien d'autre que des vecteurs dotés
d'un attribut \attribut{dim}. Ces objets sont donc stockés, et peuvent
être manipulés, exactement comme des vecteurs simples.
...
\item La généralisation d'une matrice à plus de deux dimensions est un
  tableau. Le nombre de dimensions du tableau est toujours égal à la
  longueur de l'attribut \attribut{dim}. La classe implicite d'un
  tableau est \indexclasse{array}\code{"array"}.
...
\begin{important}
  ...
  Pour les matrices, cela revient à remplir par colonne. Cette
  convention, héritée du Fortran, est quelque peu contre-intuitive.

  La fonction \code{matrix} a un argument \argument{byrow} qui permet
  d'inverser l'ordre de remplissage, mais il vaut mieux s'habituer à
  la convention de R que d'essayer constamment de la contourner.
\end{important}
...
\end{itemize}
La figure~\ref{fig:bases:tableaux} fournit une représentation schématique
...
matrices et tableaux se fait avec \fonction{[~]}.
\begin{itemize}
\item On extrait un élément d'une matrice en précisant ses positions
...
m[3]
@
\item Lorsqu'une dimension est omise dans les crochets, tous les
  éléments de cette dimension sont extraits:
<<echo=TRUE>>=
m[2, ]
@
\item Les idées sont les mêmes pour les tableaux.
\item Pour le reste, les règles d'indiçage de vecteurs exposées à la
  section~\ref{bases:indicage} s'appliquent à chaque position de
...
qualificatif \emph{récursif} pour ce type d'objet.
...
\item Nous recommandons de nommer les éléments d'une liste. En effet,
  les listes contiennent souvent des données de divers type et il peut
  s'avérer difficile d'identifier les éléments s'ils ne sont pas nommés.
  De plus, comme nous le verrons ci-dessous, il est très simple
...
x[[1]]
@
\item Évidemment, on ne peut extraire qu'un seul élément à la fois
  avec les crochets doubles \fonction{[[~]]}.
\item Petite subtilité peu employée: si l'indice utilisé dans
  \fonction{[[~]]} est un vecteur, il est utilisé récursivement pour
  indicer la liste. Ainsi, cela sélectionnera la composante de la liste
  correspondant au premier élément du vecteur, puis l'élément de la
  composante correspondant au second élément du vecteur, et ainsi de
  suite. Cryptique.
\item Une autre --- et, en fait, la meilleure --- façon d'indicer un
  seul élément d'une liste est par le biais de l'opérateur
  \Fonction{\$}, avec une construction de la forme
  \code{x\$etiquette}:
<<echo=TRUE>>=
x$size
@ %$
...
\emph{data frames}\Index{data frame} pour le stockage des données.
...
\item On crée un \emph{data frame} avec la fonction
  \Fonction{data.frame} ou encore avec \Fonction{as.data.frame}, pour
  convertir un autre type d'objet en \emph{data frame}.
...
L'indiçage sert principalement à deux choses: extraire des éléments
d'un objet avec la construction \code{x[i]} ou les remplacer avec la
construction \verb|x[i] <- y|.
\begin{itemize}
...
Il existe quatre façons d'indicer un vecteur dans le langage R. Dans
...
x[]
@ %
  Remarquer que cela est différent d'indicer avec un vecteur vide (de
  type \code{numeric(0)}); cette opération retourne un vecteur vide.
\end{enumerate}
...
@
  Écrire une expression R permettant d'obtenir les éléments demandés
  ci-dessous.
  \begin{enumerate}
