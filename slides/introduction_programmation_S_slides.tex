\documentclass{beamer}
  \usepackage[T1]{fontenc}
  \usepackage[latin1]{inputenc}
  \usepackage[francais]{babel}
  \usepackage{arev}
  \usepackage{icomma}
  \usepackage{vgmath,actu,amsmath,amsthm,url}
  \usepackage{textcomp,fourier-orns,mathabx}
  \usepackage[noae]{Sweave}
  \usepackage{booktabs,threeparttable,expdlist}
  \usepackage[normalem]{ulem}
  
  %%% Titre
  \title{Introduction à la programmation en S}
  \author{Vincent Goulet}
  \institute{École d'actuariat \\ Université Laval}
  \date{}

  %%% Paramètres de beamer
  \usetheme[hideallsubsections]{Berkeley}
  \usecolortheme{sidebartab}
  \usefonttheme{professionalfonts}
  \setbeamercovered{transparent}
  \AtBeginPart[]
  {
    \begin{frame}
      \partpage
    \end{frame}
  }
  \AtBeginSection[]
  {
    \begin{frame}
      \frametitle{Sommaire}
      \tableofcontents[sectionstyle=show/shaded,subsectionstyle=hide/hide]
    \end{frame}
  }
  \AtBeginSubsection[]
  {
    \begin{frame}
      \frametitle{Sommaire}
      \tableofcontents[sectionstyle=show/hide,subsectionstyle=show/shaded/hide]
    \end{frame}
  }

  %%% Sections non numérotées dans la table des matières
  \setbeamertemplate{section in toc}{%
    \leavevmode\leftskip=1.75ex\llap{%
      \usebeamercolor[bg]{subsection number projected}%
      \vrule width1.25ex height1.25ex\kern1ex}\inserttocsection\par}

  %%% Épaisseur de la ligne pour biffer du texte (ulem)
  \renewcommand{\ULthickness}{1pt}

  %%% Désactivation de certaines commandes
  \newcommand{\R}{}
  \newcommand{\Splus}{}
  \newcommand{\Index}[1]{}

  %%% Environnement pour les listes de commandes
  \newenvironment{ttscript}[1]{%
    \begin{list}{}{%
        \setlength{\labelsep}{1.5ex}
        \settowidth{\labelwidth}{\code{#1}}
        \setlength{\leftmargin}{\labelwidth}
        \addtolength{\leftmargin}{\labelsep}
        \setlength{\parsep}{0.5ex plus0.2ex minus0.2ex}
        \setlength{\itemsep}{0.3ex}
        \renewcommand{\makelabel}[1]{##1\hfill}}}
    {\end{list}}

  %%% Styles pour les noms de fonctions, code, etc.
  \newcommand{\code}[1]{\texttt{#1}}
  \newcommand{\attribut}[1]{\code{#1}}
  \newcommand{\Attribut}[1]{\code{#1}}
  \newcommand{\argument}[1]{\code{#1}}
  \newcommand{\Argument}[1]{\code{#1}}
  \newcommand{\classe}[1]{\code{#1}}
  \newcommand{\Classe}[1]{\code{#1}}
  \newcommand{\fonction}[1]{\code{#1}}
  \newcommand{\Fonction}[1]{\code{#1}}
  \newcommand{\Mode}[1]{\code{#1}}
  \newcommand{\objet}[1]{\code{#1}}
  \newcommand{\Objet}[1]{\code{#1}}
  \newcommand{\emacs}[1]{\code{#1}}
  \newcommand{\ess}[1]{\code{#1}}

%  \includeonlylecture{cours 1}
%  \includeonly{presentation_slides,bases_slides}
  
\begin{document}

%%%
%%% PAGE TITRE
%%%
\begin{frame}
  \titlepage
\end{frame}


%%%
%%% CHAPITRES
%%% 

%\renewcommand{\partname}{Chapitre}
%\renewcommand{\insertromanpartnumber}{\arabic{part}}

%% COURS 1: Présentation.
\lecture{Présentation}{cours 1}
\include{presentation_slides}

%% COURS 2: Bases
\lecture{Bases}{cours 2}        % hiver 2006: section Indiçage au lab 2
\include{bases_slides}

%% COURS 3: Opérateurs
\lecture{Opérateurs}{cours 3}   % hiver 2006: fait au lab 2
\include{operateurs_slides}

%% LAB 3: Exemples résolus.
\lecture{Exemples}{lab 3}       % hiver 2006: fait au cours 3
\include{exemples_slides}

%% COURS 4: Fonctions et Concepts avancés
\lecture{Fonctions}{cours 4}
\include{fonctions_slides}
\include{avance_slides}


%%%
%%% ANNEXES
%%%

%% Utiliser un autre compteur pour les annexe, autrement les sections
%% de l'annexe A sont mélangées avec celles du chapitre 1, etc.
\newcounter{annexe}
\setcounter{annexe}{\value{part}}
\addtocounter{annexe}{-6}
\renewcommand{\partname}{Annexe}
\renewcommand{\insertromanpartnumber}{\stepcounter{annexe}\Alph{annexe}}

%% LAB 1: présentation de Emacs et ESS.
\lecture{Emacs et ESS}{lab 1}
\include{ess_slides}

\end{document}

%%% Local Variables: 
%%% mode: latex
%%% TeX-master: t
%%% End: 
