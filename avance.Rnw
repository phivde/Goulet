...
\begin{objectifs}
\item Savoir passer des valeurs à une fonction via l’argument `\code{...}’.
\item Savoir effectuer des sommaires sur des tableaux à l’aide de la
  fonction \code{apply}.
\item Savoir réduire des listes avec les fonctions \code{lapply},
  \code{sapply} et \code{mapply}; connaître les différences entre ces
  fonctions.
\item Comprendre comment la classe d’un objet peut modifier le
...
\item La fonction \fonction{lapply} applique une fonction \code{FUN} à
  ...
  \begin{quote}
    \code{list(FUN(X[[1]], ...), FUN(X[[2]], ...), FUN(X[[3]], ...), ...)}
  \end{quote}
...
@ %
  ...
  la fonction via l'argument `\code{...}'. Par conséquent, les
...
Dans un grand nombre de cas, il est possible de remplacer les boucles
\fonction{for} par l'utilisation de \fonction{lapply} ou
...
replicate(10000, fun(...))
@
\item L'annexe \ref{planification} présente en détail différentes
  stratégies --- dont l'utilisation de \code{replicate} --- pour
  la réalisation d'études de simulation en R.
\end{itemize}
...
\begin{itemize}
\item Certaines fonctions, dites
...
\begin{exercice}
  \index{moyenne!pondérée} À l'exercice
  \ref{exemples}.\ref{exercice:exemples:moyennes}, on a calculé la
  moyenne pondérée d'un vecteur d'observations
  \begin{displaymath}
...
\end{verbatim}
  \begin{enumerate}
  \item Écrire une expression R permettant de simuler, en utilisant la
    fonction \code{rpareto} ci-dessus, cinq échantillons aléatoires de
    tailles 100, 150, 200, 250 et 300 d'une loi de Pareto avec $\alpha
    = 2$ et $\lambda = \nombre{5000}$. Les échantillons aléatoires
