...
La %
\capsule{http://youtu.be/apply}{fonction \texttt{apply}}%
\Indexfonction{apply} %
...
apply(x, 3, det)       # déterminants des matrices 3 x 3
apply(x, 1, sum)       # sommes des tranches horizontales
apply(x, c(1, 2), sum) # sommes des carottes horizontales
apply(x, c(1, 3), sum) # sommes des carottes transversales
apply(x, c(2, 3), sum) # sommes des carottes verticales
@
\end{itemize}
