...
exec <- function(command, args, echo = TRUE, max.lines = NULL, eval = TRUE, ...)
{
    ...
    output <- if (eval)
    {
        ...
        if (is.null(max.lines))
            ...
            res <- paste(paste(head(res, max.lines), collapse = "\n"),
                         "...\n", sep = "\n")
        if (echo)
            paste(cmd, res, sep = "\n")
        else
...
\index{Git~Bash}Git~Bash de \link{https://git-scm.com/downloads}{Git
  for Windows} et \index{MSYS}MSYS de
...
\citet{Goulet:laboratoire-cli:2021} offre une rapide introduction
...
\citet{Goulet:laboratoire-cli:2021} avec quelques simplifications,
...
(habituellement entre guillemets simples ou doubles), lit l'entrée
standard ou une liste de fichiers ligne par ligne, puis retourne sur
la sortie standard les lignes qui contiennent des correspondances avec
...
Contrairement à ce que l'on pourrait penser, \icode{sed} ne modifie
...
##-!- grep 'même' chanson.txt	   # vers contenant le mot «même»
##-!-
##-!- ## Dressons maintenant la liste de toutes les utilisations de
...
\tipbox{Évitez-vous bien des maux de tête en utilisant toujours
  \icode{grep} et \icode{sed} avec l'option \code{-E}.}
...
zéro ou une fois». En d'autres termes, «\code{?}» rend le caractère
qui précède optionnel dans la chaine recherchée. Par exemple, le motif
...
trouve répétée autant de fois que nécessaire. Ce sont les combinaisons
...
##-!- ## Il signifie «n'importe quel caractère», y compris l'espace.
##-!- ##
...
Les caractères spéciaux autres que le caret et le tiret perdent leurs
significations particulières à l'intérieur des classes de caractères.
Il est donc possible d'identifier les différentes marques de fin de
phrase avec \regex{[.?!]}.
...
loin.
%% les commandes sont trop longues et la fonction 'exec' ne permet pas
%% de les couper; j'inclus néanmoins les commandes dans des blocs de
%% code afin de valider qu'elles fonctionnent correctement
\begin{Schunk}
\begin{Verbatim}
...
\regex{Le\aspace(.*)\aspace(.*)\aspace(.*)}\,. Rappelez-vous: les opérateurs de
...
\item Il faut lire le motif de la seconde commande ainsi: «\chaine{l}
  ...
  \chaine{mouvement} ou de \chaine{fatigue}». Cela correspond bien au
...
##-!- sed -E 's/le mouvement|la fatigue/le pendule/' chanson.txt
##-!- sed -E 's/l[ea] (mouvement|fatigue)/le pendule/' chanson.txt
##-!- sed -E 's/le|la mouvement|fatigue/le pendule/' chanson.txt
##-!- sed -E 's/(le|la) (mouvement|fatigue)/le pendule/' chanson.txt
##-!-
...
premières colonnes et d'une nouvelle colonne contenant la somme de
celles-ci. Votre premier réflexe consistera sans doute à utiliser un
tableur pour effectuer les opérations suivantes:
\begin{enumerate}
\item Importer le fichier CSV;
...
\item Copier la formule dans toutes les lignes;
...
qui effectuera le traitement requis \emph{en une fraction de seconde}
et sans même avoir à lancer un lourd tableur:
...
\end{Verbatim}
\end{Schunk}
(De gauche à droite, la commande ci-dessus indique à \icode{awk}
d'utiliser la virgule comme séparateur de champs; d'afficher à la
sortie standard, séparés par des virgules, le contenu de la première colonne, le contenu de la deuxième colonne, puis la somme de ces deux colonnes; de
lire les données dans un fichier \code{data.csv}. Ensuite, les
résultats sont redirigés de la sortie standard vers un fichier
\code{data-new.csv}.)
...
de calculer la longueur --- en caractères --- du plus court vers de
...
\item[\code{\string~}] %
  ...
  l'\index{expression régulière}expression régulière du côté
  droit.\footnote{%
    Non, l'opérateur \icode[\code{awk}]{==} n'est pas valide entre une
    chaine de caractères et une expression régulière.} %
  La version négative «\code{!\string~}» existe aussi.
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
chaque fois que la règle est testée pour un nouvel enregistrement.
C'est d'ailleurs cette réévaluation des motifs à chaque enregistrement
qui rend simple avec \icode{awk} d'effectuer un traitement sur
plusieurs lignes du texte en entrée. Nous y reviendrons dans l'exemple
ci-dessous.

Si l'expression du motif fait appel à la valeur d'un champ ---
\code{\$1} par exemple ---, c'est celle de l'enregistrement courant
qui prévaut. Autrement, la valeur de l'expression dépend de l'état du
programme au moment de son évaluation.
...
le texte jusqu'à la seconde ligne blanche, exclusivement. C'est donc
un traitement sur plusieurs lignes.
\begin{Schunk}
\begin{Verbatim}
$ awk '$0 ~ /^$/ && state == 1 { exit }
       $0 ~ /^$/ { state = 1 }
...
d'où le nom, \code{state}, que j'ai choisi de lui donner --- est égale
à 1. Ici, vous devez savoir que tant qu'une valeur n'a pas été
explicitement assignée à une variable, la valeur de celle-ci est 0. Le
programme ne peut donc quitter avant qu'une valeur de 1 n'ait été
explicitement affectée à la variable d'état.

La seconde règle du programme affecte justement la valeur 1 à la
variable d'état lorsque \icode{awk} rencontre une ligne blanche. Dans
le déroulement du programme, ce sera à la première ligne blanche.

La troisième règle extrait simplement les enregistrements au fur et à
mesure qu'ils sont lus.

En combinant les règles dans cet ordre précis, le programme se termine
à la \emph{seconde} ligne blanche du fichier, donc après avoir extrait
les deux premiers couplets. Si l'ordre des deux premières règles était
inversé, la commande n'extrairait que le premier couplet. (Déterminez
pourquoi.)
...
\item l'expression \code{/\string^\$/} est équivalente à
  ...
  \index{\string~@\code{\string~} (\code{awk})}«\code{\string~}»;
\item la troisième règle peut être remplacée par un motif identité qui
  est toujours vrai («\code{1}») et aucune action, l'instruction
  \icode[\code{awk}]{print} étant implicite. C'est la règle «affiche
  le contenu de l'enregistrement courant» que l'on retrouve souvent à la fin
  des programmes \icode{awk}.
\end{itemize}
\begin{Schunk}
\begin{Verbatim}
$ awk '/^$/ && state == 1 { exit }
...
l'enregistrement correspond au motif. Autrement dit, la règle
«\code{/foo/ \{ \}}» ne fait rien lorsque l'enregistrement correspond
à \regex{foo}\,, alors que la règle «\code{/foo/}» affiche le contenu
de l'enregistrement.
...
manipulation de chaines de caractères. Consultez au besoin
\citet[chapitres~6, 7 et 9]{gawk:5.0}.
...
convertir le poids des voitures, actuellement en milliers de livres,
vers des kilogrammes. L'action de la première règle remplace le poids
du véhicule --- le cinquième champ --- par le poids en kilogrammes
($2,20462$ étant le facteur de conversion entre les deux unités).
Remarquez que cette action n'affiche rien, elle ne fait que modifier
un champ de l'enregistrement courant. C'est la seconde règle «affiche
  le contenu de l'enregistrement courant» qui affiche les résultats.
Sans celle-ci, le programme ne retournerait aucun résultat.
<<echo=FALSE>>=
exec("awk", c("'/^[^#a-z]/ { $5 = $5*1000/2.20462 } 1'", "carburant.dat"),
     max.lines = 15, env = "LC_NUMERIC=en_US.UTF-8")
@
...
Dans le premier exemple, ci-dessous, la règle modifie le séparateur de
champs pour la virgule. Dans le second, le recours à un motif
d'expression régulière permet de définir comme séparateur de champs
l'espace \emph{ou} le tiret.
\begin{Schunk}
\begin{Verbatim}
BEGIN { FS = "," }
\end{Verbatim}
...
retour à la ligne et l'espace.

Par exemple, pour séparer les champs par un point-virgule dans les
résultats, il suffit d'inclure la règle ci-dessous dans son programme
\icode{awk}.
\begin{Schunk}
\begin{Verbatim}
BEGIN { OFS = ";" }
...
\notebox{Les opérations de composition du présent ouvrage, de
  publication d'une nouvelle version et de mise à jour de la page web
  sont entièrement automatisées avec une combinaison des utilitaires
  de ce chapitre et de
  \link{https://fr.wikipedia.org/wiki/Make}{\code{make}}, un autre
...
rubrique d'aide (commune, sauf pour \icode{regmatches}) de n'importe
laquelle de celles-ci pour connaitre l'ensemble des fonctions
disponibles et les détails concernant leur fonctionnement. Vous
trouverez également dans la rubrique d'aide de nombreux exemples
...
\item[\Icode{regexpr}] comme \code{grep}, mais avec des informations
  additionnelles sur la position et la longueur du premier appariement
  dans les résultats. Pour identifier tous les appariements plutôt que
  seulement le premier, utiliser la fonction \Icode{gregexpr}.
<<echo=TRUE>>=
regexpr("ia$", state.name.10)
@
\item[\Icode{regmatches}] extrait le texte des appariements à partir
  des informations de \icode{regexpr} ou \icode{gregexpr}.
<<echo=TRUE>>=
...
l'entrer plutôt sous la forme \code{r"(.\{1,8\}\bs.txt)"}.
...
## ligne du fichier. Il faut spécifier le type de codage du
## fichier lorsque l'UTF-8 n'est pas le type par défaut de
## votre système d'exploitation (bref, sous Windows).
##
## Vous remarquerez que cela correspond au mode de
## fonctionnement de 'grep', 'sed' et 'awk': une ligne à la
## fois.
chanson <- readLines("chanson.txt",
                     encoding = "UTF-8") # importer le fichier
...
##
## C'est ce travail qu'effectue la fonction 'regmatches'.
...
\begin{exercice}[nosol]
  Voici l'exercice --- une série d'exercices, en fait --- la plus
  ludique du document: les mots croisés d'expressions régulières de
  \link{https://regexcrossword.com}{Regex Crossword}. Compléter les
  casse-têtes jusqu'au niveau «\emph{Experienced}»\dots\ ou au-delà si
  le cœur vous en dit!
\end{exercice}

\begin{exercice}
  Déterminer à laquelle ou auxquelles des chaines de caractères
  correspondent les motifs d'expression régulière étendue
  ci-dessous.\footnote{%
    Exercice adapté de
    \url{https://regex.sketchengine.eu/basic-exercises.html}.}
  \begin{enumerate}
    \settasks{style=enumerate,before-skip=2pt}
  \item \regex{a(ab)*a}
    \begin{horizenum}(3)
    \item \code{abababa}
    \item \code{aaba}
  ...
  \begin{sol}
    ...
    \item Le motif identifie la lettre minuscule «\code{a}», suivie de
      ...
      «\code{ab}», suivies de la lettre minuscule «\code{a}». Les
    ...
    \item Le motif identifie zéro, une ou plusieurs lettres minuscules
      ou majuscules (mais aucun autre symbole), suivies de tout
...
@
      ...
      certains systèmes qui utilisent non pas le point, mais la
      virgule comme séparateur décimal. C'est le cas, notamment, des
      Mac en configuration française. Dans ce cas, essayer plutôt:
      \begin{Schunk}
\begin{Verbatim}
