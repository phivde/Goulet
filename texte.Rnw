...
Par exemple, la première règle ci-dessous modifie le séparateur de
champs pour la virgule, alors que seconde, qui a recours à un motif
d'expression régulière, le modifie pour l'espace \emph{ou} le tiret.
\begin{Schunk}
\begin{Verbatim}
BEGIN { FS = "," }
\end{Verbatim}
