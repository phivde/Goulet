...
\begin{table}
  ...
  \begin{tabular}{*{7}{l}}
    ...
    \midrule
    \regex{ah\{2\}}   & \chaine{ah} & \chaine{\matches{ahh}} & \chaine{\matches{ahh}h} & \chaine{\matches{ahh}hh} & \chaine{\matches{ahh}hhh} \\ \addlinespace[6pt]
    \regex{ah\{2,4\}} & \chaine{ah} & \chaine{\matches{ahh}} & \chaine{\matches{ahhh}} & \chaine{\matches{ahhhh}} & \chaine{\matches{ahhhh}h} \\ \addlinespace[6pt]
    \regex{ah\{2,\}}  & \chaine{ah} & \chaine{\matches{ahh}} & \chaine{a\matches{ahhh}} & \chaine{\matches{ahhhh}} & \chaine{\matches{ahhhhh}} \\
    \bottomrule
  \end{tabular}
\end{table}
...
dans une chaine de caractères: le caret (ou accent circonflexe)
...
définissent ce que l'on appelle une \emph{classe de caractères}. Par
exemple, \regex{cha[iî]ne} correspond à la fois à \matches{chaine} et
à \matches{chaîne}.
...
\index{-@\code{-} (regex)}tiret devient un opérateur lorsqu'il est
employé à l'intérieur d'une classe \emph{ailleurs que comme premier
  caractère} de la classe. Comme il ne saurait définir une plage à cet
endroit, il y conserve son rôle de caractère littéral. Cela permet
ainsi d'inclure le tiret lui-même dans une classe de caractères:
\regex{-A-F} recherche le tiret et les lettres majuscules de A à F.

Il est parfois plus simple de définir une classe de caractères par la
négative afin de rechercher des correspondances avec tous les
caractères qui ne se trouvent \emph{pas} dans la classe. Les
opérateurs pour de telles classes sont «[\string^ ]». Le sens du caret
\index{\string^@\code{\string^} (regex)}«\code{\string^}» change donc
complètement à l'intérieur des classes de caractères. L'expression
ci-dessous permet d'identifier dans le fichier \code{chanson.txt} les
occurrences de la \chaine{ou} qui se sont pas suivies des lettres
\chaine{r} et \chaine{s}.
<<echo=FALSE>>=
...
contenterai de traiter brièvement des classes prédéfinies. Celles-ci
peuvent s'avérer particulièrement utiles pour travailler avec des
lettres accentuées ou des caractères invisibles. La liste des classes
prédéfinies varie beaucoup d'un système d'expressions régulières à un
autre. Je fournis, au \autoref{tab:texte:POSIXclasses}, uniquement les
classes prédéfinies de ce qui constitue généralement le dénominateur
commun, soit la norme \index{POSIX}POSIX.\footnote{%
  J'ai tenté de contourner une source possible de confusion dans la
  terminologie anglaise relative aux classes de caractères. La norme
  POSIX --- déjà abordée au \autoref{chap:donnees} lorsqu'il
  s'agissait de la représentation des dates --- nomme \emph{character
    class} une classe prédéfinie. Une classe de caractères telle que
  généralement entendue est plutôt nommée \emph{bracket expression}.}
...
\tipbox{Les crochets font partie intégrante du nom des classes
  prédéfinies. Comme nous utilisons celles-ci dans une classe de
  caractères, y faut dédoubler les crochets. Par exemple, le motif
