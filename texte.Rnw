...
\item Il faut lire le motif de la seconde commande ainsi: «\chaine{l}
  ...
  à la différence de la première variante, le motif correspondrait
  aussi à \chaine{\matches{la mouvement}} et à \chaine{\matches{le
      fatigue}}. C'est sans conséquence ici.
\item La troisième commande n'effectue pas le traitement attendu. En
  ...
  la chanson --- y compris dans \chaine{Frôle} --- par \chaine{le
    pendule}. Ce n'est certainement pas ce que nous voulons.
