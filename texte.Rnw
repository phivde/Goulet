...
\begin{figure}[t]
  ...
  \begin{minipage}{0.9\linewidth}
    ...
    Tiré de \href{http://xkcd.com/208/}{XKCD.com}
  \end{minipage}
\end{figure}
...
\windowsbox{Utilisateurs de Windows: pas de souci! Bien qu'émanant de
  ...
  d'outils Unix standards. Or, tel qu'expliqué à la
  \autoref{sec:cli:windows}, vous disposez de ces outils si vous avez
  installé \index{Git}\emph{Git for Windows} ou MSYS2\index{MSYS2}.}
...
\begin{exercice}
  ...
  \item Extraire les lignes du fichier satisfaisant les deux
    ...
    redirection \index{{"|}@\code{\textbar} (Bash)}%
    «\code{\textbar}» (\autoref{sec:cli:flux}) pour combiner les
    commandes des parties a) et b).
  \end{enumerate}
  \begin{sol}
    \begin{enumerate}
