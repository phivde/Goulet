...
Examinez la commande, les explications détaillées suivent.
\begin{Schunk}
\begin{Verbatim}
$ awk '/^[#a-z]/ || $2 == 4
       { print ($0 ~ /^#/) ? $0 : $1 \" \" $2 }' \
...
\begin{ttscript}{?:}
\item[\code{\string~}] \index{\string~@\code{\string~} (\code{awk})|bfhyperpage}
  retourne vrai lorsque la chaine de caractères du côté gauche de
  l'opérateur correspond à l'expression régulière du côté droit. La
  version négative \index{!\string~@\code{!\string~}
...
et les deux premiers champs séparés par une espace autrement.
