...
\def\scriptfilename{\currfilebase.sh}
\SweaveOpts{echo=FALSE}

<<echo=FALSE, results=hide>>=
source(".Sweaveprofile")   # configuration
source("Stangle-texte.R")  # lancement de Stangle
...
\gotorbox{Le fichier de script \code{\scriptfilename} reproduit à la
  \autoref{sec:texte:exemples} fournit des exemples additionnels
  d'utilisation de \code{grep}, \code{sed} et \code{awk}. Vous pouvez
  ouvrir le fichier de script dans votre éditeur de texte pour
  facilement copier les commandes à la ligne de commande.}

<<script>>=
##-!- ## Les exemples de ce fichier utilisent des fichiers
##-!- ## distribués avec le présent ouvrage. Vous pouvez exécuter
##-!- ## les commandes ci-dessous telles quelles à une ligne de
##-!- ## commande Unix.
##-!- ##
##-!- ## Débutons en dressant la liste de toutes les utilisations de
##-!- ## la fonction 'matrix' dans les fichiers d'exemples. C'est un
##-!- ## travail pour 'grep'.
##-!- grep 'matrix' *.R
##-!-
...
##-!- grep -l 'matrix' *.R
##-!-
##-!- ## Pour connaitre les fichiers d'exemples dans lesquels
##-!- ## apparaissent les objets 'letters' et 'LETTERS', nous devons
##-!- ## rechercher sans égard à la casse pour attraper les deux
##-!- ## cas. L'option '-i' de 'grep' permet d'ignorer la casse.
##-!- grep -i 'letters' *.R
##-!-
##-!- ## Allons-y maintenant d'une expression régulière plus
##-!- ## élaborée pour trouver toutes les utilisations de la
##-!- ## fonction 'g' dans les fichiers d'exemples, c'est-à-dire la
##-!- ## chaine 'g(' précédée d'au moins une espace ou en début de
##-!- ## ligne.
##-!- grep -E '( |^)g\\(' *.R
##-!-
...
##-!- ## Pour faire bonne mesure, plaçons le résultat dans un
##-!- ## nouveau fichier 'pratiques-new.R' à l'aide de l'opérateur
##-!- ## de redirection Unix '>'. C'est un travail pour 'sed'.
...
##-!- ## et 2005. Effectuons dans un premier temps l'extraction des
##-!- ## temps des records. C'est le second champ du fichier. Voilà
##-!- ## un travail tout désigné pour 'awk'.
...
##-!-   100metres.dat
##-!-
##-!- ## Le fichier 'NEWS' contient l'historique des versions
##-!- ## du document. La commande suivante dresse la liste des
##-!- ## numéros de versions.
##-!- grep -E '^# [[:digit:]]' NEWS
##-!-
##-!- ## Nous souhaitons extraire du fichier 'NEWS' les notes
##-!- ## de mise à jour de la plus récente version.
##-!- ##
##-!- ## Les notes relatives à une version donnée débutent toujours
##-!- ## à une ligne marqué par '# '.
##-!- ##
##-!- ## Le meilleur outil pour effectuer cette opération est 'awk'
##-!- ## avec une variable d'état pour identifier les lignes que
...
##-!- ## pourquoi en comparant avec l'exemple similaire dans le
##-!- ## chapitre.)
##-!- ##
...
\notebox{Les opérations de composition de «\thetitle», de publication
  ...
  automatisées à l'aide des utilitaires étudiés dans ce chapitre et de
  \link{https://fr.wikipedia.org/wiki/Make}{\code{make}}, un autre
  outil standard des systèmes Unix. Jetez un coup d'œil au fichier de
  configuration \code{Makefile} qui se trouve dans le
  \link{\reposurl}{code source du projet}.}
...
\lstinputlisting[language=sh,firstline=\scriptfirstline]{\scriptfilename}
