...
ci-dessous permet d'extraire les strophes qui ne contiennent pas la
lettre «e» (ce qui inclut les lignes blanches dans le fichier).
\begin{Schunk}
\begin{Verbatim}
...
particulièrement ésotérique. Nous nous attarderons donc à une seule
commande de \icode{sed}, celle qui permet de rechercher et de
remplacer à l'intérieur d'un texte. Sa syntaxe est la suivante:
\begin{Schunk}
...
La commande de remplacement est fournie en argument à \icode{sed}, qui
lit ensuite l'entrée standard ou une liste de fichiers ligne par
ligne, puis retourne sur la sortie standard le texte modifié (sans
...
occurrences de «promet» sur une ligne par «jure». (J'ai abrégé les
