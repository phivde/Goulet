...
\item \index{motif}\meta{motif} est une %
...
rechercher est l'\index{expression régulière}expression régulière \regex{\string^M}\,. La syntaxe
...
\index{motif@motif (\code{awk})}motif ou l'action dans une règle
...
suffit d'affecter un nouveau symbole ou une %
