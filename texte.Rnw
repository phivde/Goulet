...
une liste de fichiers ligne par ligne. La syntaxe est donc la suivante:
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
champ par défaut est l'espace, mais il est facile de le changer pour
...
programme entre guillemets simples\footnote{%
  ...
  recommande de toujours utiliser des guillemets simples.}. %
...
série de règles. Chaque règle spécifie un motif à rechercher dans
l'enregistrement courant et une action à exécuter lorsque le texte
correspond au motif.
...
Le prochain exemple est un petit peu plus élaboré. La commande
\icode{awk} permet de calculer la longueur --- en caractères --- du
plus cours vers de \code{chanson.txt}. Cela nécessite trois règles:
\begin{itemize}
...
naturellement en enregistrements et en champs. Le fichier
\code{carburant.dat} livré avec le présent ouvrage contient des
données de consommation de carburant pour divers types de
voitures.\footnote{%
...
Comme les résultats de ces deux exemples occupent
plusieurs lignes, la validation des commandes vous est laissée en
exercice.)
...
passer par un tableur ou par R. La commande suivante conserve les
commentaires et les noms de colonnes en entête du fichier, ainsi que
les enregistrements dont le second champ est égal à 4.
<<echo=FALSE>>=
exec("awk", c("'/^[#a-z]/ || $2 == 4'", "carburant.dat"), eval = FALSE)
@

En second lieu, la commande suivante ajoute une action afin de
conserver uniquement les deux premiers champs des données de l'exemple
précédent. La sélection doit évidemment s'effectuer uniquement sur les
titres de colonnes et sur les données\footnote{%
  J'ai choisi, pour ne pas compliquer indument l'exemple, de ne pas
  toucher aux commentaires.}. %
L'action fait appel à trois opérations de \icode{awk} que nous n'avons
pas rencontrées auparavant:
\begin{itemize}
\item \index{\string~@\verb=~= (\code{awk})|bfhyperpage}«\verb=~=» est
  un opérateur de relation qui retourne vrai lorsque la chaine de
  caractère du côté gauche de l'opérateur correspond à l'expression
  régulière du côté droit;
\item «\Icode[\code{awk}]{?:}» est l'opérateur ternaire\footnote{%
    On retrouve cet opérateur dans plusieurs langages de
    programmation. Dans R, ce qui s'en approche le plus est la
    fonction \icode{ifelse}.} %
  qui permet d'écrire une expression conditionnelle de la forme
  \begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
\meta{condition} ? \meta{expression\_si\_vrai} : \meta{expression\_si_faux}
\end{Verbatim}
  \end{Schunk}
\item la concaténation de chaines de caractères, qui n'a pas
  d'opérateur désigné; il suffit de placer les chaines de caractères
  les unes à la suite des autres.
\end{itemize}
En assemblant tous ces morceaux, vous devriez pouvoir décoder que
l'action dans la commande ci-dessous affiche le contenu complet de
l'enregistrement lorsque celui-ci début par le symbole «\code{\#}», et
les deux premiers champs séparés par une espace autrement.
\begin{Schunk}
\begin{Verbatim}
$ awk '/^[#a-z]/ || $2 == 4
       { print ($0 ~ /^#/) ? $0 : $1 \" \" $2 }' \
...
Je vous invite à consulter aussi les exemples des sections 1.3 à 1.5
de l'excellent ouvrage libre de \citet{gawk:5.0}.


\begin{itemize}
\item \index{motif}\meta{motif} est une combinaison d'expressions
  régulières placées entre barres obliques «\icode[\code{awk}]{/}» et
  d'expressions relationnelles. Si le motif est absent, l'action est
  exécutée pour chaque enregistrement.
\item \index{action@action (\code{awk}}\emph{action} est une série
  d'instructions à exécuter lorsque le motif correspond, est non nul
  ou n'est pas vide. Si l'action est absente, l'instruction
  \icode[\code{awk}]{print} est exécutée implicitement.
\end{itemize}
