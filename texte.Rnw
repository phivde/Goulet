...
\icode{awk}, mais pas les deux. Si le motif est absent, l'action est
exécutée pour chaque enregistrement. Si c'est l'action qui est
absente, l'instruction \icode[\code{awk}]{print} est exécutée
implicitement.

Nous pouvons donc aller encore plus loin dans la simplification de la
commande précédente en omettant l'action.
<<echo=FALSE>>=
...
des exemples additionnels, je vous recommande ceux des sections 1.3 à
1.5 de l'excellent
...
\item[\code{/\meta{motif}/}] correspond lorsque l'expression régulière
  correspond à l'enregistrement courant.
\item[\meta{expression}] correspond lorsque la valeur de l'expression
...
\item[\code{\meta{motif\_début}, \meta{motif\_fin}}] \mbox{}\\
  ...
  (attention: la virgule «\code{,}» fait partie intégrante de la
  l'instruction).
...
valeur à une variable dans \icode{awk}. À cette liste il faut aussi
