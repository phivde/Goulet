...
est à son tour formée d'un \index{motif}\emph{motif} et d'une
\index{action@action (\code{awk}}\emph{action}. Les actions sont
placées entre accolades «\icode[awk]{\{~\}}» et les règles sont
généralement séparées par des retours à la ligne, lorsqu'il y en a
plus d'une. La forme générale d'un programme AWK est donc la suivante:
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
\icode{grep}. Nous devons utiliser une expression régulière comme
motif. Tel qu'expliqué à la \autoref{sec:texte:awk:base}, à
l'intérieur d'une règle la variable \code{\$0} contient l'intégralité
...
Cependant, l'instruction \icode[awk]{print} étant implicite, nous
pouvons aller encore plus loin dans la simplification de notre
commande en omettant l'action.
<<echo=FALSE>>=
...
affiche le deuxième mot de chaque ligne du fichier \code{chanson.txt}.
