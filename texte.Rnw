...
\begin{exercice}
  ...
  \item Extraire les États dont le nom comporte au moins trois fois la
    lettre «s», qu'elles sont successives ou non.
  \item Extraire les États dont le nom comporte un double «s» parmi
    les quatre premiers caractères, abréger leur nom après ce double
    «s» et faire suivre d'un point. Par exemple: «Massachusetts»
    devient «Mass.».
  \end{enumerate}
  \begin{sol}
    \begin{enumerate}
