...
\item Mettre en italique tous les mots d'un texte qui se trouvent
  entre guillemets (mais pas les guillemets eux-mêmes).
\end{itemize}
...
L'utilitaire \Icode{grep} prend en argument une expression régulière,
lit l'entrée standard ou une liste de fichiers ligne par ligne, puis
retourne sur la sortie standard les lignes qui contiennent des
correspondances avec l'expression régulière. C'est très utile pour
...
fichier \code{chanson.txt}: la strophe de la chanson qui contient le
mot «temps»; les strophes qui débutent par «M»; la strophe qui
contient le mot «chat» (aucune\footnote{%
...
En combinant \icode{grep} avec l'utilitaire \Icode{wc} (\emph{word
  count}), nous pouvons obtenir le nombre de lignes, de mots et de
caractères des strophes dans lesquelles le mot «flambant» apparait
dans la chanson.
\begin{Schunk}
\begin{Verbatim}
...
L'option \code{-v} de \icode{grep} permet d'inverser les résultats,
c'est-à-dire d'extraire les lignes qui ne contiennent aucune
correspondance ave l'expression régulière. La commande ci-dessous
permet d'extraire les strophes qui ne contiennent pas la lettre «e»
(ce qui inclut les lignes blanches dans le fichier).
\begin{Schunk}
\begin{Verbatim}
...
\notebox{Lorsqu'il est question d'expressions régulières dans
  la littérature informatique, le langage de programmation
  ...
  charge par \icode{grep} et \icode{sed}. Nous ne traiterons pas
  davantage de \icode{Perl} dans cet ouvrage.}
