...
\citep{Wikipedia:Expression_reguliere}. L'expression est souvent
appelée un \index{motif}«motif» (\emph{pattern}).
...
chiffre {\fullcaps 1} et la lettre \textsf{V}. Le \index{motif}motif
suivant permet de décrire l'ensemble des numéros de plaques possibles
...
tuyau \index{{"|}@\code{\textbar} (tuyau Bash)} «\code{\textbar}», ou
encore redirigé vers un fichier avec l'opérateur «\code{>}». Ce flux
des données est illustré à la \autoref{fig:texte:flux-cli}.
...
\begin{figure}
  ...
  \begin{picture}(31.25,9)(-0.25,-1.5)
    ...
    \put(25,3.5){\makebox(0,0)[b]{\parbox{4.5\unitlength}{
          \centering\code{>}}}}
    ...
    \put(12,-1){\makebox(0,0){\code{\textbar} (tuyau)}}
  \end{picture}
...
typographiques --- communément appelée \index{motif}motif
(\emph{pattern}) --- qui décrit une chaine de texte. Elle est composée
de caractères littéraux (dont la «valeur» correspond au symbole
lui-même) et de caractères spéciaux qui font office
...
\item Une expression régulière peut être formée par
  ...
  le motif \regex{am} correspond à la lettre «a» immédiatement suivie
  de la lettre «m», soit les deux premières lettres du mot «amour».
...
du programme \icode{cat} vers \icode{awk} avec le tuyau de Bash
\index{{"|}@\code{\textbar} (tuyau Bash)} «\code{\textbar}», alors que
...
\begin{exercice}
  ...
  \item Extraire les lignes du fichier satisfaisant les deux
    ...
    tuyau de Bash \index{{"|}@\code{\textbar} (tuyau Bash)}%
