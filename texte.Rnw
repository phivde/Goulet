...
\begin{table}
  ...
  \caption{Comparaison des quatre variantes des quantificateurs
    bornés. Les caractères «\chaine{//}» représentent une chaine de
    caractères vide.}
  ...
  \begin{tabular}{*{7}{l}}
    ...
    \midrule
    \regex{a\{2\}}   & \chaine{//} & \chaine{a} & \chaine{\matches{aa}} & \chaine{aaa} & \chaine{aaaa} & \chaine{aaaaa} \\ \addlinespace[6pt]
    \regex{a\{2,4\}} & \chaine{//} & \chaine{a} & \chaine{\matches{aa}} & \chaine{\matches{aaa}} & \chaine{\matches{aaaa}} & \chaine{aaaaa} \\ \addlinespace[6pt]
    \regex{a\{2,\}}  & \chaine{//} & \chaine{a} & \chaine{\matches{aa}} & \chaine{\matches{aaa}} & \chaine{\matches{aaaa}} & \chaine{\matches{aaaaa}} \\ \addlinespace[6pt]
    \regex{a\{,4\}}  & \chaine{\matches{//}} & \chaine{\matches{a}} & \chaine{\matches{aa}} & \chaine{\matches{aaa}} & \chaine{\matches{aaaa}} & \chaine{aaaaa} \\
    \bottomrule
  \end{tabular}
\end{table}
...
«\chaine{.}». Pour rechercher un nom de fichier long d'exactement huit
