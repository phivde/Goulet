...
L'utilitaire standard \Icode{grep} prend en argument une expression
...
\link{http://www.catb.org/jargon/html/G/grep.html}{jargon informatique} %
en anglais (\texttt{to grep}).
