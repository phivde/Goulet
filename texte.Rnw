...
La présentation des expressions régulières exige souvent de devoir
clairement délimiter une expression et d'afficher certains caractères
...
\notebox{Lorsqu'il est question d'\index{expression
    régulière}expressions régulières dans la littérature informatique,
  le langage de programmation \index{Perl}Perl est habituellement
  mentionné. En effet, Perl a été créé par Larry Wall en 1987 pour
...
correspondance avec l'\index{$expression
  régulière$}expression régulière. Par exemple, la commande ci-dessous
permet d'extraire les vers qui ne contiennent pas la lettre «e» (ce
qui inclut les lignes blanches entre les strophes).
\begin{Schunk}
\begin{Verbatim}
...
\item Dans les expressions régulières étendues, les accolades \code{\{
    \}} et les parenthèses \code{( )} sont, respectivement, des
...
expression régulière étendue dans laquelle l'opérateur  «\code{|}»
permet de reconnaitre «chien» ou «chat».
\begin{Schunk}
\begin{Verbatim}
