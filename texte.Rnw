...
\notebox{Dans la littérature informatique, la présentation des
  ...
  \icode{sed}. Le présent ouvrage ne traite pas de \icode{Perl}.}
...
\item[\code{\string~}] %
  ...
  l'opérateur (\code{\$0} par défaut) correspond à l'expression
  régulière du côté droit. La version négative
  «!\string~» existe aussi.
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
à 1, donc à la \emph{seconde} ligne blanche. Tous les autres
enregistrements sont extraits avec \icode[\code{awk}]{print}. Si
l'ordre des deux premières règles était inversé, la commande
n'extrairait que le premier couplet. (Déterminez pourquoi.)
...
\item la troisième règle peut être remplacée par un motif identité
  («\code{1}») qui est toujours vrai et aucune action, l'instruction
...
Le fichier \code{100metres.dat} contient la liste des 31 meilleurs
...
Voilà un travail tout désigné pour \icode{awk}.
<<echo=FALSE>>=
exec("awk", c("'{ print $2 }'", "100metres.dat"), eval = FALSE)
@

Nous pouvons aussi aisément avec \icode{awk} inverser les deux
colonnes du fichier.
<<echo=FALSE>>=
exec("awk", c("'{ print $2, $1 }'", "100metres.dat"), eval = FALSE)
@

Les colonnes du fichier \code{100metres.dat} sont séparées par une
...
<<echo=FALSE>>=
exec("awk", c("'{ print $1 \",\" $2 }'", "100metres.dat"), eval = FALSE)
exec("sed", c("'s/ /,/'", "100metres.dat"), eval = FALSE)
@
...
Nous souhaitons extraire du fichier \code{NEWS} les notes de mise à
jour de la plus récente version. Les notes relatives à une version
donnée débutent toujours à une ligne marqué par «\code{\#\aspace}». Le
meilleur outil pour effectuer cette opération est \icode{awk}. Nous
avons recours à une variable d'état pour identifier les lignes que
vous souhaitons extraire. De la manière dont les règles sont écrites
dans la commande, leur ordre n'a pas d'importance. (Déterminez
pourquoi en comparant avec l'exemple similaire de la
\autoref{sec:texte:awk:motifs}.)
...
\tipbox{Le motif dans la dernière règle de la commande précédente est
  équivalent à «\code{state~==~1}» ou «\code{state~!=~0}» puisque
...
\notebox{Les opérations de composition de «\thetitle», de publication
  ...
  automatisées à l'aide de
  \link{https://fr.wikipedia.org/wiki/Make}{\code{make}}, un outil
  standard des systèmes Unix. Le fichier de configuration
  \code{Makefile} qui se trouve dans le \link{\reposurl}{code source
    du projet} contient une variante plus élaborée de la commande
  \code{awk} ci-dessus. Jetez-y un coup d'œil!}
...
<<echo=FALSE>>=
exec("sed", c("'s/\\./,/'", "100metres.dat > 100metres-dec.data"), eval = FALSE)
@
  \end{sol}
\end{exercice}
...
<<echo=FALSE>>=
exec("grep", c("' 9\\.'", "100metres.dat"), eval = FALSE)
@
...
@
      ou
<<echo=FALSE>>=
exec("grep", c("-e", "'-08-'", "100metres.dat | grep ' 9\\.'"), eval = FALSE)
@
