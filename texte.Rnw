...
##
## - fichiers de script bases.R et donnees.R
## - chanson.txt
## - 100metres.dat
## - carburant.dat
## - NEWS
...
\gotorbox{Le fichier de script \code{\scriptfilename} reproduit à la
  \autoref{sec:texte:exemples} contient des exemples d'utilisation de
...
\gotorbox{Avant de véritablement entrer dans le vif du sujet, prenez
  ...
  de script \code{\scriptfilename} reproduit à la
...
\tipbox{Les quantificateurs des expressions régulières sont par défaut
  ...
  \autoref{sec:texte:exemples} une solution de contournement.}
...
\gotorbox{Nous avons déjà couvert plusieurs éléments importants de la
  ...
  \code{\scriptfilename} reproduit à la \autoref{sec:texte:exemples}.}
...
##-!- ## expressions régulières étendues, c'est un symbole sprécial
...
\lstinputlisting[language=shenhanced,firstline=\scriptfirstline]{\scriptfilename}
