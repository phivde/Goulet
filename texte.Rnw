...
point-virgule «\code{;}». Chaque instruction d'un action indique une
chose à faire.
...
manipulation de chaines de caractères. Je ne souhaite pas aller plus
loin dans le cadre de cet ouvrage. Consultez \citet[chapitres~6, 7 et
9]{gawk:5.0}.
...
le texte jusqu'à la seconde ligne blanche.
\begin{Schunk}
\begin{Verbatim}
$ awk '$0 ~ /^$/ && state == 1 { exit }
       $0 ~ /^$/ { state = 1 }
...
d'«état» lorsque \icode{awk} rencontre la première ligne blanche. La
première règle, quant à elle, cause la sortie du programme lorsque
\code{awk} rencontre une autre ligne blanche et que la variable d'état
est égale à 1, donc à la seconde ligne blanche. Tous les autres
enregistrements sont extraits avec \icode[\code{awk}]{print}. Si l'ordre
des deux premières règles était inversé, la commande n'extrairait que
le premier couplet. (Déterminez pourquoi.)

Question de parfaire notre style de programmation \icode{awk},
apportons deux simplifications à la commande ci-dessus:
\begin{itemize}
\item l'expression \code{/\string^\$/} est équivalente à \code{\$0
    \string~ /\string^\$/} puisque \code{\$0} est la valeur par défaut
  de l'argument gauche de l'opérateur %
  \index{\string~@\code{\string~} (\code{awk})}\code{\string~};
\item l'instruction \icode[\code{awk}]{print} étant implicite
  lorsqu'une action est absente, la troisième règle peut être
  remplacée par un motif identité qui est toujours vrai.
\end{itemize}
\begin{Schunk}
\begin{Verbatim}
$ awk '$/^$/ && state == 1 { exit }
...
Poussons maintenant un peu plus loin avec un exemple basé sur les
données du fichier \code{carburant.dat} livré avec le présent ouvrage.
(Examinez les données dans votre éditeur de texte avant d'aller plus
loin.) Il s'agit de données de consommation de carburant pour divers
types de voitures\footnote{%
  Comme la présence marquée des antédiluviens moteurs huit cylindres
  l'indique, il s'agit d'un très vieux jeu de données!}. %
entièrement en unités de mesure impériales. La commande \icode{awk}
suivante effectue la conversion du poids des voitures, actuellement en
milliers en livres, vers des kilogrammes. Les explications détaillées
suivent.
<<echo=FALSE>>=
...
\begin{itemize}
\item Le programme \icode{awk} compte deux règles:
  \begin{Schunk}
\begin{Verbatim}
/^[^#a-z]/ { $5 = $5*1000/2.20462 }
\end{Verbatim}
  \end{Schunk}
  et
\begin{Schunk}
...
\end{Verbatim}
  \end{Schunk}
  La première règle est composée d'un motif et d'une action, alors que
  la seconde ne comporte qu'un motif trivial («\code{1}») et aucune
  action.
\item Le motif \regex{/\string^[\string^\#a-z]/} de la première règle
  est une expression régulière qui correspond aux lignes qui ne
  débutent \emph{pas} par «\code{\#}» ou une lettre minuscule. Le
  traitement est donc effectué sur toutes les lignes, sauf celles
  contenant des commentaires ou les titres des colonnes.
\item L'action de la première règle est \code{\$5 = \$5*1000/2.20462}.
  Cette expression remplace le poids du véhicule en milliers de livres
  --- le cinquième champ --- par le poids en kilogrammes ($2,20462$
  étant le facteur de conversion entre les deux unités). Remarquez que
  cette action n'affiche rien, elle ne fait que modifier un champ de
  l'enregistrement courant.
\item Dans la seconde règle, le motif «\code{1}» est toujours vrai et
  comme il n'y a pas d'action, c'est l'instruction implicite
  \icode[\code{awk}]{print} qui s'exécute. Résultat: cette règle ne
  fait qu'afficher toutes les lignes du fichier après avoir modifié le
  cinquième champ. Sans cette règle --- un idiome du langage
  \icode{awk} --- le programme ne retournerait aucun résultat.
\end{itemize}
...
- Motifs
  ...
  - BEGIN/END
- Actions
  - expressions (--> fonctions internes)
