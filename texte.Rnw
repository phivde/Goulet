...
Terminons cette section avec un exemple de remplacement de texte qui
...
déterminant qui le précède. Comparons quatre commandes \icode{sed}
pour effectuer le traitement. Avant de poursuivre la lecture, analysez
les différences entre les variantes.
%% les commandes sont trop longues et l'insfrastructure de la fonction
%% 'exec' ne permet pas de les couper; j'inclus les commandes
%% néanmoins afin de valider qu'elles fonctionnent correctement
...
Étudions maintenant les commandes de plus près.
\begin{itemize}
\item La première commande utilise la syntaxe la plus directe pour
  effectuer le traitement désiré. Le motif trouve des correspondances
...
\item Il faut lire le motif de la seconde commande ainsi: «\chaine{l}
  ...
  texte que nous voulons remplacer par \chaine{le pendule}. À la
  différence de la première variante, le motif correspondrait aussi à
  \chaine{\matches{la mouvement}} ou à \chaine{\matches{le fatigue}}.
...
\item La troisième commande n'effectue pas le traitement attendu. En
  ...
  correspond à \chaine{\matches{le}} ou à
  \chaine{\matches{la mouvement}} ou à \chaine{\matches{fatigue}}.
