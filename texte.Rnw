...
\index{AWK@AWK}AWK est conçu autour du concept de séparation du texte
...
entre accolades «\icode[awk]{\{~\}}» pour la séparer du motif. La
forme générale d'un programme \icode{awk} est donc la suivante:
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
rechercher est l'expression régulière \regex{\string^M}\,. La syntaxe de
\icode{awk} exige que les expressions régulières soient placées
entre barres obliques «\icode[awk]{/}». Nous avons déjà vu à la
...
Vous aurez compris que la commande \icode[awk]{print} sert à afficher
du texte en sortie. L'instruction \code{print} sans argument est
équivalente à \code{print \$0}. Nous pourrions donc simplifier
...
absente, l'instruction \icode[awk]{print} est exécutée implicitement.
...
relationnelle et la variable \icode[awk]{NR} qui contient le numéro de
l'enregistrement courant. Celle-ci est une des variables fournies
automatiquement par \icode{awk} durant l'exécution des règles.
<<echo=FALSE>>=
exec("awk", c("'NR <= 5 { print $2 }'", "chanson.txt"))
@

Une autre variable fournie automatiquement par \icode{awk} est
\icode[awk]{NF}, le nombre de champs dans l'enregistrement courant.
Nous pouvons utiliser cette variable pour extraire les vers de la
chanson qui comptent moins de quatre mots.
...
\item la troisième utilise le motif spécial \icode[awk]{END} qui
  permet d'exécuter des instructions après la lecture de tous les
...
Terminons avec un exemple basé sur des données divisées naturellement
en enregistrements et en champs. Le fichier \code{carburant.dat} livré
avec le présent ouvrage contient des données de consommation de
carburant pour divers types de voitures.\footnote{%
...
d'aller plus loin.) Nous pouvons facilement créer une nouvelle base de
données ne contenant que les modèles de voitures 4~cyclindres sans
passer par un tableur ou par R. La commande suivante converve les
commentaires et les noms de colonnes en entête du fichier, ainsi que
les enregistrements dont le second champ est égal à 4. (La validation
du résultat vous est laissée en exercice.)
<<echo=FALSE>>=
...
\item \index{motif}\meta{motif} est une combinaison d'expressions
  régulières placées entre barres obliques «\icode[awk]{/}» et
...
\item \index{action@action (\code{awk}}\emph{action} est une série
  ...
  \icode[awk]{print} est exécutée implicitement.
\end{itemize}
...
\item Dans la seconde règle, le motif «\code{1}» est toujours vrai et
  ...
  \icode[awk]{print} qui s'exécute. Résultat: cette règle ne fait
  qu'afficher toutes les lignes du fichier après avoir modifié le
