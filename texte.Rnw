...
##
exec <- function(command, args, echo = TRUE, max.lines = Inf, eval = TRUE, ...)
{
    ...
    if (eval)
    {
        ...
        if (echo)
            cat(paste(cmd, paste(head(res, max.lines), collapse = "\n"),
                      if (is.finite(max.lines)) "...\n", sep = "\n"))
        else
...
à traiter en enregistrements (\emph{records}), puis en champs
(\emph{fields}). Le séparateur d'enregistrement par défaut est le
retour à la ligne. Dans la suite, nous ne ferons plus de différence
entre une ligne du texte et un enregistrement.
...
Un programme (ou script) \icode{awk} est constitué d'une série de
\index{regle@règle (\code{awk})}règles. Une fois que \icode{awk} a
séparé un enregistrement en champs, il exécute les règles une par une.
Les règles ont accès à des variables qui contiennent l'enregistrement
complet et chacun des champs, ce qui facilite grandement les
opérations sur une ligne et sur ses composantes. La variable
\code{\$0} contient l'enregistrement complet, alors que les champs se
trouvent dans des variables nommées \code{\$1}, \code{\$2},
\code{\$3}, \dots.
...
programme entre guillemets simples\footnote{%
  ...
  recommande d'utiliser toujours des guillemets simples.}. %
...
\item \meta{programme} est un programme \icode{awk} formé de règles; voir
  ci-dessous.
\item \meta{fichiers} est le ou les fichiers à traiter; si aucun
  fichier n'est spécifié, \icode{awk} opère sur l'entrée standard.
\end{itemize}
...
série de \index{regle@règle (\code{awk})}\emph{règles}. Chaque règle
est à son tour formée d'un \index{motif}\emph{motif} à rechercher et
d'une \index{action@action (\code{awk}}\emph{action} à exécuter
lorsque le motif correspond, est non nul ou n'est pas vide, selon le
type de motif. Les actions sont placées entre accolades
«\icode[awk]{\{~\}}» et les règles sont généralement séparées par des
retours à la ligne, lorsqu'il y en a plus d'une. La forme générale
d'un programme AWK est donc la suivante:
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
programmation \icode{awk}. Le premier affiche les lignes du fichier
\code{chanson.txt} qui débutent par la lettre «M», comme le ferait
\icode{grep}. Nous voulons utiliser une expression régulière comme
motif. Pour l'indiquer à \icode{awk}, il suffit de placer celle-ci
entre barres obliques «\icode[awk]{/}». Tel qu'expliqué à la
\autoref{sec:texte:awk:base}, à l'intérieur d'une règle la variable
...
légèrement la commande ci-dessus.
<<echo=FALSE>>=
exec("awk", c("'/^M/ { print }'", "chanson.txt"))
@

Cependant, si une règle dans un programme ne comporte pas d'action, la
commande \icode[awk]{print} est implicite. Nous pouvons donc aller
encore plus loin dans la simplification de notre commande en
omettant l'action.
<<echo=FALSE>>=
...
Une règle peut ne pas comporter d'action. Que se passe-t-il si, à la
place, elle ne comporte pas de motif? Vous aurez sans doute deviné que
l'action est exécutée pour chacune des lignes en entrée. La commande
ci-dessous affiche donc le deuxième mot de chaque ligne du fichier
...
fichier, nous pouvons utiliser comme motif une expression \code{awk}
(plutôt qu'une expression régulière) et la variable \icode[awk]{NR}
qui contient le numéro de l'enregistrement courant. Celle-ci est une
des variables fournies automatiquement par \icode{awk} durant
...
À ce stade, vous disposez déjà d'une bonne idée d'ensemble du
fonctionnement de \icode{awk}. Les sections suivantes présentent
rapidement quelques notions additionnelles qui vous permettront
d'élargir l'éventail des traitements que vous pourrez réaliser avec
l'utilitaire.

- Appel de awk et script simple
  o pas de règle -> exécuté sur chaque ligne
  o print implicite
