...
\tipbox{Il existe une option pour remplacer directement dans le
  fichier d'origine, mais elle n'est pas standard dans toutes les
  versions de \icode{sed}. Je vous recommande de ne pas vous y fier.}
...
C'est là qu'entre en jeu le \index{caractere d'echappement@caractère
  d'échappement}caractère d'échappement %
«\Index{\string\@\code{\bs} (regex)}\bs». %
...
\emph{champs} (\emph{fields}). Le séparateur d'enregistrement par
défaut est le retour à la ligne. Dans la suite, nous ne ferons plus de
différence entre une ligne du texte et un enregistrement.

Lorsque \icode{awk} a terminé de lire une ligne du texte fourni en
entrée, il sépare automatiquement celle-ci en champs. Le séparateur de
champ par défaut est l'espace, mais, comme nous le verrons à la
\autoref{sec:texte:awk:separateur} il est facile de le changer pour
tout autre caractère\footnote{%
  C'est ce que «\code{-F ','}» permettait de faire dans l'exemple au
  début de la section.}.
...
\item Pour effectuer un traitement équivalent avec le fichier
  ...
  \icode{awk} d'utiliser la virgule «\code{,}» comme séparateur de
  champ.
...
constructions de motifs et d'actions dans les sections suivantes.
...
rechercher est l'expression régulière \regex{\string^M}\,. La syntaxe
de \icode{awk} exige que les expressions régulières soient placées
entre barres obliques «\icode[\code{awk}]{/}». Nous avons déjà vu à la
\autoref{sec:texte:awk:base} qu'à l'intérieur d'une règle, la variable
\code{\$0} contient l'intégralité de l'enregistrement courant.
<<echo=FALSE>>=
...
Cela dit, il est permis d'omettre le motif ou l'action dans une règle
...
données ne contenant que les modèles de voitures 4~cyclindres sans
...
colonnes et sur les données\footnote{%
  J'ai choisi, pour ne pas compliquer indument l'exemple, de ne pas
  toucher aux commentaires.}. %
Examinez d'abord la commande, les explications détaillées suivent.
\begin{Schunk}
\begin{Verbatim}
$ awk '/^[#a-z]/ || $2 == 4 {
           print ($0 ~ /^#/) ? $0 : $1 " " $2
...
\begin{ttscript}{?:}
\item[\code{\string~}] %
  \index{\string~@\code{\string~} (\code{awk})|bfhyperpage} retourne
  vrai lorsque la chaine de caractères du côté gauche de l'opérateur
  (\code{\$0} par défaut) correspond à l'expression régulière du côté
  droit. La version négative \index{!\string~@\code{!\string~}
    (\code{awk})|bfhyperpage}\code{!\string~} existe aussi.
\begin{Schunk}
\begin{Verbatim}[commandchars=\\\{\}]
...
\end{Verbatim}
  \end{Schunk}
\item[\code{?:}] \Index{?:@\code{?:} \code{awk}} retourne le résultat
  de l'expression en second ou en troisième argument selon que la
...
l'enregistrement lorsque celui-ci débute par le symbole «\code{\#}»,
et les deux premiers champs séparés par une espace (la chaine
«\code{"~"}») autrement.
...
Les sections suivantes ne font qu'àjouter des détails sur l'écriture
des règles et la configuration de l'outil. Pour des exemples
additionnels, consultez \citet[sections~1.3 à 1.5]{gawk:5.0}.

\subsection{Motifs}
\label{sec:texte:awk:motifs}

Comme nous l'avons déjà vu, les motifs dans \icode{awk} contrôlent
l'exécution des règles: l'action d'une règle est exécutée lorsque le
motif correspond à l'enregistrement courant. Il existe six types de
motifs dans \icode{awk}, dont quatre que nous avons déjà rencontrés
dans les exemples de la section précédente.
...
\item[\code{/\meta{expression régulière}/}]  \mbox{}\\
  correspond lorsque l'expression régulière
  correspond à l'enregistrement courant.
...
\item[\meta{vide}] un motif vide correspond à tous les
  enregistrements.
\end{ttscript}
\Index{BEGIN@\code{BEGIN} (\code{awk})}
\Index{END@\code{END} (\code{awk})}
\Index{BEGINFILE@\code{BEGINFILE} (\code{awk})}
\Index{ENDFILE@\code{ENDFILE} (\code{awk})}
...
et %
\index{!\string~@\code{\string~} (\code{awk})}«\code{!\string~}» %
mentionnés à la \autoref{sec:texte:awk:commandes}. Vous trouverez à la
section~6.2 de \citet{gawk:5.0} une liste exhaustive des opérateurs de
\icode{awk}.
...
Illustrons certains des concepts ci-dessus, notamment l'importance de
l'ordre des règles dans un programme, à l'aide d'un exemple. La
...
La seconde règle du programme affecte la valeur 1 à une variable
d'«état» --- d'où le nom que j'ai choisi de lui donner --- lorsque
\icode{awk} rencontre la première ligne blanche. La première règle,
quant à elle, cause la sortie du programme lorsque \code{awk}
rencontre une autre ligne blanche et que la variable d'état est égale
à 1, donc à la seconde ligne blanche. Tous les autres enregistrements
sont extraits avec \icode[\code{awk}]{print}. Si l'ordre des deux
premières règles était inversé, la commande n'extrairait que le
...
\item l'expression \code{/\string^\$/} est équivalente à
  ...
  \index{\string~@\code{\string~} (\code{awk})}«\code{\string~}»;
\item l'instruction \icode[\code{awk}]{print} étant implicite
  lorsqu'une action est absente, la troisième règle peut être
  remplacée par un simple motif identité qui est toujours vrai.
\end{itemize}
\begin{Schunk}
\begin{Verbatim}
$ awk '$/^$/ && state == 1 { exit }
...
L'action d'une règle dans un programme \icode{awk} indique à
l'interpréteur quoi faire lorsqu'un enregistrement correspond à un
motif. Une action est constituée d'une ou plusieurs instructions
\icode{awk} placées entre accolades «\icode[\code{awk}]{\{~\}}» et
séparées les unes des autres par un retour à la ligne ou par un
point-virgule «\code{;}». Chaque instruction d'une action indique une
chose à faire.
...
Les séparateurs d'enregistrements et de champs par défaut sont,
respectivement, le retour à la ligne et l'espace. Ces symboles sont
stockés dans les variables prédéfinies \icode[\code{awk}]{RS}
(\emph{record separator}) et \icode[\code{awk}]{FS} (\emph{field
  separator}). Pour changer l'un ou l'autre de ces séparateurs, il
suffit d'affecter un nouveau symbole ou une expression régulière dans
la variable correspondante, habituellement dans une règle spéciale
...
lors de l'écriture des résultats. Ceux-ci sont stockés dans les
...
Ces exemples utilisent des fichiers distribués avec le présent
ouvrage. Vous pouvez évaluer les commandes ci-dessous telles quelles à
une ligne de commande \index{Unix}Unix.

Débutons en dressant la liste de toutes les utilisations de la
fonction \code{matrix} dans les fichiers d'exemples. C'est un travail
pour \icode{grep}.
<<echo=FALSE>>=
exec("grep", c("'matrix'", "*.R"), eval = FALSE)
@
...
\code{matrix}.
<<echo=FALSE>>=
exec("grep", c("-l", "'matrix'", "*.R"), eval = FALSE)
@
...
égard à la casse pour attraper les deux cas. L'option \code{-i} de
\icode{grep} permet d'ignorer la casse.
<<echo=FALSE>>=
exec("grep", c("-i", "'letters'", "*.R"), eval = FALSE)
@
...
<<echo=FALSE>>=
exec("grep", c("-E", "'( |^)g\\('", "*.R"), eval = FALSE)
@

Remplaçons maintenant les symboles de commentaires triples
«\code{\#\#\#}» en début de ligne (à la Emacs) dans le fichier
\code{pratiques.R} par un symbole unique «\code{\#}» (à la RStudio).
Pour faire bonne mesure, plaçons le résultat dans un nouveau fichier
\code{pratiques-new.R} à l'aide de l'opérateur de redirection Unix
\index{>@\code{>} (Unix)}«\code{>}». C'est un travail pour \icode{sed}.
<<echo=FALSE>>=
exec("sed", c("'s/^###/#/'", "pratiques.R > pratiques-new.R"), eval = FALSE)
@

Le fichier \code{100metres.data} contient la liste des 31 meilleurs
temps enregistrés au 100~mètres homme entre 1964 et 2005 (voir
l'\autoref{ex:tri:100metres}). Effectuons dans un premier temps
l'extraction des temps des records. C'est le second champ du fichier.
Voilà un travail tout désigné pour \icode{awj}.
<<echo=FALSE>>=
exec("awk", c("'{ print $2 }'", "100metres.data"), eval = FALSE)
@

Nous pouvons aussi aisément inverser les deux colonnes du fichier avec
\icode{awk}.
<<echo=FALSE>>=
exec("awk", c("'{ print $2, $1 }'", "100metres.data"), eval = FALSE)
@
...
fichier en format CSV. Je fournis deux solutions, avec \icode{awk} (la
plus simple) et avec \icode{sed}.
<<echo=FALSE>>=
exec("awk", c("'{ print $1 \",\" $2 }'", "100metres.data"), eval = FALSE)
exec("sed", c("'s/ /,/'", "100metres.data"), eval = FALSE)
@
...
champs de \code{awk} pour l'espace et le tiret. Ainsi, \code{awk} va
non seulement séparer les deux colonnes du fichier, mais aussi les
champs de la date. Il suffit ensuite de replacer les champs dans
l'ordre voulu avec les bons séparateurs. La fonction
\icode[\code{awk}]{substr} de \code{awk} permet de sélectionner une
partie d'une chaine de caractères \citep[section~9.1.3]{gawk:5.0}.
\begin{Schunk}
\begin{Verbatim}
$ awk 'BEGIN { FS = "[ -]" } \
...
remplacement d'expressions régulières. Le motif recherche, en
capturant chaque élément trouvé (sauf les tirets): les nombres 19 ou
20; deux chiffres; un tiret; deux chiffres; un tiret; deux chiffres;
tout le reste de la ligne. Il s'agit ensuite de replacer les éléments
capturés dans l'ordre souhaité avec des barres obliques \code{/} entre
les éléments de dates. Comme le symbole \code{/} est utilisé dans la
chaine de sortie, il vaut mieux utiliser un autre symbole pour séparer
les champs de la commande \code{sed}. J'ai utilisé le symbole
«\code{\string~}» ici.
\begin{Schunk}
\begin{Verbatim}
...
Le fichier \code{NEWS} contient l'historique des versions du document.
La commande suivante dresse la liste des numéros de versions.
<<echo=FALSE>>=
exec("grep", c("-E", "'^# [[:digit:]]'", "NEWS"), eval = FALSE)
@

Un dernier exemple similaire à celui de la
\autoref{sec:texte:awk:motifs}. Nous souhaitons extraire du fichier
...
marqué par «\code{\#\aspace}». Le meilleur outil pour effectuer des
opérations sur plusieurs lignes d'un fichier est \icode{awk}. Nous
avons recours à une variable d'état pour identifier les lignes que
vous souhaitons extraire.
\begin{Schunk}
\begin{Verbatim}
$ awk '(state == 1) && /^# / { exit }
...
\tipbox{Le motif dans la dernière règle dans la commande précédente
  est équivalent à «\code{state~==~1}» ou «\code{state~!=~0}» puisque
  \icode{awk} évalue l'expression et détermine que le motif correspond
...
<<echo=FALSE>>=
exec("sed", c("'s/\\./,/'", "100metres.data > 100metres-dec.data"), eval = FALSE)
@
  \end{sol}
\end{exercice}
...
\begin{exercice}
  ...
  informations suivantes du fichier \code{100metres.data}.
  \begin{enumerate}
  ...
  \begin{sol}
    ...
    \item C'est un travail pour \icode{grep}. La solution la plus
      simple serait:
<<echo=FALSE>>=
exec("grep", c("'-08-'", "100metres.data"), eval = FALSE)
@
      ...
      rechercher:
<<echo=FALSE>>=
exec("grep", c("-e", "'-08-'", "100metres.data"), eval = FALSE)
@
...
@
    \item Le plus simple, ici, consiste à utiliser \icode{awk} puisque
      ...
      disponibles.
<<echo=FALSE>>=
exec("awk", c("'$2 < 10'", "100metres.data"), eval = FALSE)
@
...
<<echo=FALSE>>=
exec("grep", c("' 9\\.'", "100metres.data"), eval = FALSE)
@
    \item Nous pouvons combiner les deux commandes ainsi:
<<echo=FALSE>>=
exec("grep", c("-e", "'-08-'", "100metres.data | awk '$2 < 10'"), eval = FALSE,
     env = "LC_NUMERIC=en_US.UTF-8")
@
      ou
<<echo=FALSE>>=
exec("grep", c("-e", "'-08-'", "100metres.data | grep ' 9\\.'"), eval = FALSE)
@
      C'est aussi possible de tout faire en un seul appel à \icode{awk}:
<<echo=FALSE>>=
exec("awk", c("'/-08-/ && $2 < 10'", "100metres.data"), eval = FALSE,
     env = "LC_NUMERIC=en_US.UTF-8")
@
    \end{enumerate}
  \end{sol}
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
    \item Utilisons la fonction \icode{grep}. Le motif de l'expression
